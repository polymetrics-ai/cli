...
- Dispatch `pm-gsd-worker` (mutating, `thinking: xhigh`, inherits the parent session model, scoped
  to `read,bash,edit,write,grep,find,ls`) for each independent ready sub-issue with disjoint write
  scope. Give every mutating worker its own `cwd` (prefer a git worktree).
- Dispatch `pm-scout` (read-only, `thinking: high`, inherits the parent session model) for
  reconnaissance sidecars.
