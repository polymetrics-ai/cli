...
export async function captureTargetEvidence(
	...
	if (!validNumber(request?.pr)) throw new TargetEvidenceError("pull request must be a positive bounded integer");
	...
	const [rawHead, rawBranch, rawStatus, rawPR] = await Promise.all([
