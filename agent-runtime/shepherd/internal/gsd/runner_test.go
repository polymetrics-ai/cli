...
func TestRunnerRejectsUnsupportedCommandAndModel(t *testing.T) {
	...
	result = runner.Run(context.Background(), "research-milestone", nil, Observer{})
	if result.Terminal != TerminalSuccess {
		t.Fatalf("research milestone terminal=%s error=%v", result.Terminal, result.Err)
	}
	result = runner.Run(context.Background(), "plan-milestone", nil, Observer{})
	if result.Terminal != TerminalSuccess {
		t.Fatalf("plan milestone terminal=%s error=%v", result.Terminal, result.Err)
	}
