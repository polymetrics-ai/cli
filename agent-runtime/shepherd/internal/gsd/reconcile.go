...
func Reconcile(registry UnitRegistry, command string, result Result, before, snapshot WorkflowSnapshot) (Terminal, error) {
	...
	if (command == "next" || registry.IsCanonicalCommand(command)) && before.Phase == snapshot.Phase && before.Next == snapshot.Next {
	...
	case "dispatch":
		if command == "next" || command == "new-milestone" || command == "status" || command == "discuss" || registry.IsCanonicalCommand(command) {
