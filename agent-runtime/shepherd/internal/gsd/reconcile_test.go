...
func TestReconcileAcceptsOneUnitProgress(t *testing.T) {
	...
	terminal, err := Reconcile(mustDecodeRegistryFixture(t), "new-milestone", Result{Terminal: TerminalSuccess}, WorkflowSnapshot{}, WorkflowSnapshot{
...
func TestReconcileAcceptsTargetedDiscussUnit(t *testing.T) {
	...
	terminal, err := Reconcile(mustDecodeRegistryFixture(t), "discuss", Result{Terminal: TerminalSuccess}, WorkflowSnapshot{}, WorkflowSnapshot{
...
func TestReconcileAcceptsCompleteQuery(t *testing.T) {
	...
	terminal, err := Reconcile(mustDecodeRegistryFixture(t), "auto", Result{Terminal: TerminalSuccess}, WorkflowSnapshot{}, WorkflowSnapshot{Phase: "complete", Next: NextDispatch{Action: "stop"}})
	if terminal != TerminalSuccess || err != nil {
		t.Fatalf("terminal=%s err=%v", terminal, err)
	}
}
...
func TestReconcileRejectsFalseGreenCanonicalUnit(t *testing.T) {
	...
	terminal, err := Reconcile(mustDecodeRegistryFixture(t), "research-milestone", Result{Terminal: TerminalSuccess}, before, before)
	...
	terminal, err = Reconcile(mustDecodeRegistryFixture(t), "research-milestone", Result{Terminal: TerminalSuccess}, before, after)
	if terminal != TerminalSuccess || err != nil {
...
func TestReconcileClassifiesMutatingSkipAsARecoverableFence(t *testing.T) {
	...
	terminal, err := Reconcile(mustDecodeRegistryFixture(t), "plan-slice", Result{Terminal: TerminalSuccess}, before, after)
