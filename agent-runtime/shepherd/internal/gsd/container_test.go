...
func TestContainerRuntimeHidesHostPlanningAndCredentialSurface(t *testing.T) {
	...
	config := ContainerConfig{Engine: "podman", Image: "localhost/gsd-pi:1.11.0", GSDStateDir: filepath.Join(t.TempDir(), "gsd"), PlanningDir: filepath.Join(t.TempDir(), "planning"), SessionsDir: filepath.Join(t.TempDir(), "sessions"), BackgroundDir: filepath.Join(t.TempDir(), "bg-shell"), BackupDir: filepath.Join(t.TempDir(), "gsd-backups"), AuthFile: auth, SettingsFile: settings, PolicyDir: policyDir, GitCommonDir: gitCommonDir}
	if err := config.Validate(root); err != nil {
		t.Fatal(err)
	}
	joined := strings.Join(config.commandArgs(root, []string{"headless", "query"}), " ")
	for _, required := range []string{root + ":" + root + ":rw", root + "/.gsd", root + "/.planning", root + "/.bg-shell", root + "/.gsd-backups", "/home/shepherd/.pi/agent/sessions:rw", gitCommonDir + ":" + gitCommonDir + ":rw", "GIT_CONFIG_KEY_2=safe.directory", "GIT_CONFIG_VALUE_2=" + root, "auth.json:ro", "settings.json:ro", "--pull=never", "--interactive"} {
...
func validContainerConfig(t *testing.T) (ContainerConfig, string) {
	...
	return ContainerConfig{Engine: "podman", Image: "localhost/gsd-pi:1.11.0", GSDStateDir: filepath.Join(t.TempDir(), "gsd"), PlanningDir: filepath.Join(t.TempDir(), "planning"), SessionsDir: filepath.Join(t.TempDir(), "sessions"), BackgroundDir: filepath.Join(t.TempDir(), "bg-shell"), BackupDir: filepath.Join(t.TempDir(), "gsd-backups"), AuthFile: auth, SettingsFile: settings, PolicyDir: policyDir, GitCommonDir: gitCommonDir}, root
}
