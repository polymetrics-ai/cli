...
.PHONY: fmt vet tidy-check test build icons-generate docs-check docs-check-no-build install uninstall smoke smoke-no-build verify verify-parallel verify-duckdb perf-free perf-runtime runtime-doctor runtime-up runtime-down runtime-reset clean lint connectorgen-validate cli-architecture-v2-skill-check pi-model-routing-check pi-shepherd-guards-check
...
cli-architecture-v2-skill-check:
	scripts/tests/cli-architecture-v2-delivery-skill.sh

pi-model-routing-check:
	scripts/tests/pi-model-routing.sh

pi-shepherd-guards-check:
	scripts/tests/pi-shepherd-loop-stall-guard.sh
	scripts/tests/pi-shepherd-loop-verdict-guard.sh

verify: fmt tidy-check vet test build docs-check smoke lint connectorgen-validate pi-model-routing-check pi-shepherd-guards-check
