...
- A terminal `run.status=delivered_reconciliation_required` means the provider effect, read-back, and checkpoint are already durable. Preserve its exact `ETLRun` and categorized nonzero exit; rerun only the same saved connection and stream so pm repairs its declared `delivery_reconciliation` locally before endpoint resolution. Never replace it with a normal retry or a raw provider action.
