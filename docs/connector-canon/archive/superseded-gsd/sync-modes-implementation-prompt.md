# Prompt: Connector Registry-Style Sync Modes for PM
...
connector registry models sync behavior as a combination of source read mode and destination write mode:
...
connector registry code reviewed locally from `connector-registryhq/connector-registry` commit `124f26099572d954ea6f257780145b8e1fb5bd68`:
...
Primary goal:
Implement connector registry-style sync-mode semantics for PM ETL with dependency-free local storage first. Preserve PM's explicit Go connector design. Do not add a plugin runtime or a web UI.
...
Phase name:
connector-registry-style-sync-modes
...
Required connector registry research anchors:
- connector registry Sync Modes docs
- connector registry Incremental Append + Deduped docs
- connector registry Full Refresh Overwrite + Deduped docs
- connector registry source files:
  - SyncMode.yaml
