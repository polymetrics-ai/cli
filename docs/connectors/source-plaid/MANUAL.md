...
```text
NAME
  pm connectors inspect source-plaid - Plaid connector manual

SYNOPSIS
  pm connectors inspect source-plaid
  pm connectors inspect source-plaid --json
  pm credentials add <name> --connector source-plaid [--config key=value] [--from-env field=ENV] [--value-stdin field]

DESCRIPTION
  Plaid catalog connector. Native implementation status: planned_native_port.
