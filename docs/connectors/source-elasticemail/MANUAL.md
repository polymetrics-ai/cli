...
```text
NAME
  pm connectors inspect source-elasticemail - Elasticemail connector manual

SYNOPSIS
  pm connectors inspect source-elasticemail
  pm connectors inspect source-elasticemail --json
  pm credentials add <name> --connector source-elasticemail [--config key=value] [--from-env field=ENV] [--value-stdin field]

DESCRIPTION
  Elasticemail catalog connector. Native implementation status: planned_native_port.
