...
COMMAND SURFACE
  ...
  User Commands
    ...
    user fetch - Fetch Freshchat users by id list [intent=direct_read availability=implemented]; notes: Typed direct-read body support is limited to the Freshchat users/fetch ids array with Freshchat's 100-id API limit; this is not a raw HTTP POST escape hatch.; flags: --id
  ...
  Agent Commands
    ...
    agent create - Create a Freshchat agent [intent=reverse_etl availability=implemented write=create_agent]; approval: reverse ETL writes require plan, preview, approval, execute and admin confirmation.; risk: Creates a Freshchat agent account.; flags: --email, --first-name, --last-name, --role-id, --group
    agent update - Update a Freshchat agent [intent=reverse_etl availability=implemented write=update_agent]; approval: reverse ETL writes require plan, preview, approval, execute and admin confirmation.; risk: Updates an existing Freshchat agent.; flags: --agent-id, --first-name, --last-name, --role-id, --group, --is-deactivated
    agent status-update - Update a Freshchat agent availability status [intent=reverse_etl availability=implemented write=update_agent_status]; approval: reverse ETL writes require plan, preview, approval, execute and admin confirmation.; risk: Updates an agent's Freshchat availability status.; flags: --agent-id, --status
  ...
  Report And Metric Commands
    report extract - Request a Freshchat raw report extract [intent=reverse_etl availability=implemented write=extract_report]; approval: reverse ETL writes require plan, preview, approval, execute and sensitive confirmation.; risk: Requests generation of a Freshchat raw report extract.; flags: --start, --end, --event, --format
