...
```text
NAME
  pm connectors inspect source-hardcoded-records - Hardcoded Records connector manual

SYNOPSIS
  pm connectors inspect source-hardcoded-records
  pm connectors inspect source-hardcoded-records --json
  pm credentials add <name> --connector source-hardcoded-records [--config key=value] [--from-env field=ENV] [--value-stdin field]
