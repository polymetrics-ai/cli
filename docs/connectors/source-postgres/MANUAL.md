...
```text
NAME
  pm connectors inspect source-postgres - Postgres connector manual

SYNOPSIS
  pm connectors inspect source-postgres
  pm connectors inspect source-postgres --json
  pm credentials add <name> --connector source-postgres [--config key=value] [--from-env field=ENV] [--value-stdin field]
