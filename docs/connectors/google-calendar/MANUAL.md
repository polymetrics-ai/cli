...
COMMAND SURFACE
  ...
  Event reads and reverse-ETL actions
    ...
    events import - Plan import of a private event copy. [intent=reverse_etl availability=implemented write=import_event]; approval: Plan first, inspect preview output, then execute only with the generated approval token.; risk: Imports a private event copy; requires plan, preview, approval, and execute.; flags: --calendar-id (required), --summary (required), --start-date-time (required), --end-date-time (required), --description, --location, --color-id
