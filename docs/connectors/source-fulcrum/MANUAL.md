# pm connectors inspect source-fulcrum

```text
NAME
  pm connectors inspect source-fulcrum - Fulcrum connector manual

SYNOPSIS
  pm connectors inspect source-fulcrum
  pm connectors inspect source-fulcrum --json
  pm credentials add <name> --connector source-fulcrum [--config key=value] [--from-env field=ENV] [--value-stdin field]
...
CONFIGURATION
  api_key (string) required secret: API key to use. Find it at https://web.fulcrumapp.com/settings/api
