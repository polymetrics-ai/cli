...
```text
NAME
  pm connectors inspect source-outlook - Outlook connector manual

SYNOPSIS
  pm connectors inspect source-outlook
  pm connectors inspect source-outlook --json
  pm credentials add <name> --connector source-outlook [--config key=value] [--from-env field=ENV] [--value-stdin field]
