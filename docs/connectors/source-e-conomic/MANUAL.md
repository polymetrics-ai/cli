...
```text
NAME
  pm connectors inspect source-e-conomic - e-conomic connector manual

SYNOPSIS
  pm connectors inspect source-e-conomic
  pm connectors inspect source-e-conomic --json
  pm credentials add <name> --connector source-e-conomic [--config key=value] [--from-env field=ENV] [--value-stdin field]
