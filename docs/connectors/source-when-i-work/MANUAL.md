# pm connectors inspect source-when-i-work

```text
NAME
  pm connectors inspect source-when-i-work - When_i_work connector manual

SYNOPSIS
  pm connectors inspect source-when-i-work
  pm connectors inspect source-when-i-work --json
  pm credentials add <name> --connector source-when-i-work [--config key=value] [--from-env field=ENV] [--value-stdin field]
...
CONFIGURATION
  email (string) required secret: Email of your when-i-work account
  password (string) required secret: Password for your when-i-work account
  secret fields: email, password
