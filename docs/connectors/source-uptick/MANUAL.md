...
```text
NAME
  pm connectors inspect source-uptick - Uptick connector manual

SYNOPSIS
  pm connectors inspect source-uptick
  pm connectors inspect source-uptick --json
  pm credentials add <name> --connector source-uptick [--config key=value] [--from-env field=ENV] [--value-stdin field]

DESCRIPTION
  Uptick catalog connector. Native implementation status: planned_native_port.
