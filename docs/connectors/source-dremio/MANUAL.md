...
```text
NAME
  pm connectors inspect source-dremio - Dremio connector manual

SYNOPSIS
  pm connectors inspect source-dremio
  pm connectors inspect source-dremio --json
  pm credentials add <name> --connector source-dremio [--config key=value] [--from-env field=ENV] [--value-stdin field]
