...
```text
NAME
  pm connectors inspect source-encharge - Encharge connector manual

SYNOPSIS
  pm connectors inspect source-encharge
  pm connectors inspect source-encharge --json
  pm credentials add <name> --connector source-encharge [--config key=value] [--from-env field=ENV] [--value-stdin field]
