# pm connectors inspect source-oncehub

```text
NAME
  pm connectors inspect source-oncehub - Oncehub connector manual

SYNOPSIS
  pm connectors inspect source-oncehub
  pm connectors inspect source-oncehub --json
  pm credentials add <name> --connector source-oncehub [--config key=value] [--from-env field=ENV] [--value-stdin field]
...
CONFIGURATION
  api_key (string) required secret: API key to use. Find it in your OnceHub account under the API & Webhooks Integration page.
