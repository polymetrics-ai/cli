...
```text
NAME
  pm connectors inspect source-track-pms - Track PMS connector manual

SYNOPSIS
  pm connectors inspect source-track-pms
  pm connectors inspect source-track-pms --json
  pm credentials add <name> --connector source-track-pms [--config key=value] [--from-env field=ENV] [--value-stdin field]
