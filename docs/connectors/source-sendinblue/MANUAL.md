...
```text
NAME
  pm connectors inspect source-sendinblue - Sendinblue connector manual

SYNOPSIS
  pm connectors inspect source-sendinblue
  pm connectors inspect source-sendinblue --json
  pm credentials add <name> --connector source-sendinblue [--config key=value] [--from-env field=ENV] [--value-stdin field]
