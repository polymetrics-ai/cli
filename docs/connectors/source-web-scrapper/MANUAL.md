# pm connectors inspect source-web-scrapper

```text
NAME
  pm connectors inspect source-web-scrapper - Web Scrapper connector manual

SYNOPSIS
  pm connectors inspect source-web-scrapper
  pm connectors inspect source-web-scrapper --json
  pm credentials add <name> --connector source-web-scrapper [--config key=value] [--from-env field=ENV] [--value-stdin field]
...
CONFIGURATION
  api_token (string) required secret: API token to use. Find it at https://cloud.webscraper.io/api
