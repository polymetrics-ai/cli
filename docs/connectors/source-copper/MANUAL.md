...
```text
NAME
  pm connectors inspect source-copper - Copper connector manual

SYNOPSIS
  pm connectors inspect source-copper
  pm connectors inspect source-copper --json
  pm credentials add <name> --connector source-copper [--config key=value] [--from-env field=ENV] [--value-stdin field]

DESCRIPTION
  Copper catalog connector. Native implementation status: planned_native_port.
