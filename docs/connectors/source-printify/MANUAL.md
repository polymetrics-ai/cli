...
```text
NAME
  pm connectors inspect source-printify - Printify connector manual

SYNOPSIS
  pm connectors inspect source-printify
  pm connectors inspect source-printify --json
  pm credentials add <name> --connector source-printify [--config key=value] [--from-env field=ENV] [--value-stdin field]
