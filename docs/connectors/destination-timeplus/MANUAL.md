# pm connectors inspect destination-timeplus

```text
NAME
  pm connectors inspect destination-timeplus - Timeplus connector manual

SYNOPSIS
  pm connectors inspect destination-timeplus
  pm connectors inspect destination-timeplus --json
  pm credentials add <name> --connector destination-timeplus [--config key=value] [--from-env field=ENV] [--value-stdin field]
...
CONFIGURATION
  apikey (string) required secret: Personal API key
