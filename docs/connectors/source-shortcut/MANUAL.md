...
```text
NAME
  pm connectors inspect source-shortcut - Shortcut connector manual

SYNOPSIS
  pm connectors inspect source-shortcut
  pm connectors inspect source-shortcut --json
  pm credentials add <name> --connector source-shortcut [--config key=value] [--from-env field=ENV] [--value-stdin field]
