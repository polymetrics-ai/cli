# pm connectors inspect source-mux

```text
NAME
  pm connectors inspect source-mux - Mux connector manual

SYNOPSIS
  pm connectors inspect source-mux
  pm connectors inspect source-mux --json
  pm credentials add <name> --connector source-mux [--config key=value] [--from-env field=ENV] [--value-stdin field]
