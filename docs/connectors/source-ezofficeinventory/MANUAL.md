...
```text
NAME
  pm connectors inspect source-ezofficeinventory - EZOfficeInventory connector manual

SYNOPSIS
  pm connectors inspect source-ezofficeinventory
  pm connectors inspect source-ezofficeinventory --json
  pm credentials add <name> --connector source-ezofficeinventory [--config key=value] [--from-env field=ENV] [--value-stdin field]
