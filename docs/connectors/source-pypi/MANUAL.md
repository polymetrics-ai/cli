...
```text
NAME
  pm connectors inspect source-pypi - PyPI connector manual

SYNOPSIS
  pm connectors inspect source-pypi
  pm connectors inspect source-pypi --json
  pm credentials add <name> --connector source-pypi [--config key=value] [--from-env field=ENV] [--value-stdin field]

DESCRIPTION
  PyPI catalog connector. Native implementation status: planned_native_port.
