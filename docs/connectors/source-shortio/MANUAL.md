# pm connectors inspect source-shortio

```text
NAME
  pm connectors inspect source-shortio - Shortio connector manual

SYNOPSIS
  pm connectors inspect source-shortio
  pm connectors inspect source-shortio --json
  pm credentials add <name> --connector source-shortio [--config key=value] [--from-env field=ENV] [--value-stdin field]
...
CONFIGURATION
  ...
  secret_key (string) required secret: Short.io Secret Key
