...
```text
NAME
  pm connectors inspect source-mention - Mention connector manual

SYNOPSIS
  pm connectors inspect source-mention
  pm connectors inspect source-mention --json
  pm credentials add <name> --connector source-mention [--config key=value] [--from-env field=ENV] [--value-stdin field]

DESCRIPTION
  Mention catalog connector. Native implementation status: planned_native_port.
