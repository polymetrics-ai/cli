...
```text
NAME
  pm connectors inspect source-airbyte - Airbyte connector manual

SYNOPSIS
  pm connectors inspect source-airbyte
  pm connectors inspect source-airbyte --json
  pm credentials add <name> --connector source-airbyte [--config key=value] [--from-env field=ENV] [--value-stdin field]
