...
```text
NAME
  pm connectors inspect source-nasa - Nasa connector manual

SYNOPSIS
  pm connectors inspect source-nasa
  pm connectors inspect source-nasa --json
  pm credentials add <name> --connector source-nasa [--config key=value] [--from-env field=ENV] [--value-stdin field]

DESCRIPTION
  Nasa catalog connector. Native implementation status: planned_native_port.
