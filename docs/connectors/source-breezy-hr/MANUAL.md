...
```text
NAME
  pm connectors inspect source-breezy-hr - Breezy HR connector manual

SYNOPSIS
  pm connectors inspect source-breezy-hr
  pm connectors inspect source-breezy-hr --json
  pm credentials add <name> --connector source-breezy-hr [--config key=value] [--from-env field=ENV] [--value-stdin field]

DESCRIPTION
  Breezy HR catalog connector. Native implementation status: planned_native_port.
