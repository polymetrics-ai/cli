...
- [alpha-vantage](alpha-vantage/MANUAL.md): Reads daily, weekly, monthly, and intraday OHLCV time series plus latest global quotes through fixed Alpha Vantage query operations.
...
- [amazon-sqs](amazon-sqs/MANUAL.md): Retains evidence-backed visibility for DeleteQueue and PurgeQueue while their configured-queue actions lack a schema-4 typed record binding. No Amazon SQS provider action is executable from this bundle.
...
- [apify-dataset](apify-dataset/MANUAL.md): Reads Apify dataset items and dataset metadata through fixed Apify API v2 routes.
...
- [asana](asana/MANUAL.md): Maps 107 bounded operation-backed direct reads, 12 stream-backed interactive reads with a global one-provider-request budget, and 131 approval-gated one-record direct writes across all 130 mutation endpoints. All 64 source-backed collection streams support saved full-refresh ETL; project-scoped tasks additionally support source-backed event-token incremental modes; all 131 write actions support warehouse-table reverse ETL. Every operation in the pinned OpenAPI ledger has an executable covered_by route, including POST /batch through a closed declaration-backed action selector.
- [ashby](ashby/MANUAL.md): Reads Ashby applicant-tracking REST resources and exposes reviewed reverse-ETL/direct-read surfaces from the official Ashby OpenAPI. Fixture-only; not live-certified.
...
- [aws-cloudtrail](aws-cloudtrail/MANUAL.md): Reads CloudTrail management events through fixed, declaration-bound LookupEvents requests signed with AWS Signature Version 4.
- [babelforce](babelforce/MANUAL.md): Reads Babelforce call reporting, recordings, numbers, and users through fixed Babelforce v2 REST routes.
- [bahmni](bahmni/MANUAL.md): Reads clinical EMR data from a Bahmni deployment, including local Bahmni/bahmni-docker setups, through the OpenMRS REST, Bahmni-core REST, and OpenMRS FHIR2 R4 APIs — patients, encounters, observations, visits, concepts, locations, providers, orders, lab results, appointments, and diagnoses — executes bounded direct reads and a schema-gated typed GET patient search, and models retained OpenMRS/Bahmni clinical mutations as approval-gated, schema-bound reverse-ETL actions. Output is bounded; secret-shaped fields and typed patient-search declared sensitive fields are redacted by the current runtime, while broad clinical PHI field redaction remains a separate engine policy decision.
...
- [basecamp](basecamp/MANUAL.md): Reads Basecamp 3 projects, people, and account events through fixed account-bound REST routes.
...
- [bitbucket](bitbucket/MANUAL.md): Reads Bitbucket Cloud workspace, repository, pull request, issue, commit, pipeline, deployment, snippet, and project resources; exposes closed-schema repository creation and path-only deletes while blocking untyped JSON-body and multipart writes.
...
- [bunny-inc](bunny-inc/MANUAL.md): Reads Bunny subscription-billing data through declared per-tenant GraphQL connection routes.
...
- [canny](canny/MANUAL.md): Reads Canny boards, posts, comments, categories, and companies through fixed Canny REST form requests.
...
- [copper](copper/MANUAL.md): Reads Copper CRM records through fixed typed search routes.
...
- [dixa](dixa/MANUAL.md): Reads Dixa conversation export records through fixed bearer-authenticated export routes.
...
- [fastbill](fastbill/MANUAL.md): Reads FastBill billing records through fixed JSON SERVICE envelopes.
...
- [feishu](feishu/MANUAL.md): Reads Feishu/Lark Bitable records, tables, and field schemas through declared Bitable REST routes and a bounded tenant token exchange.
...
- [free-agent-connector](free-agent-connector/MANUAL.md): Reads FreeAgent contacts, invoices, bills, projects, and tasks through fixed FreeAgent v2 REST routes and OAuth2 refresh-token authentication.
- [freightview](freightview/MANUAL.md): Reads Freightview shipments, quotes, and tracking events through fixed Freightview v2.0 REST routes using client-credentials authentication.
...
- [github](github/MANUAL.md): Reads GitHub repository, issue, pull request, code, release, collaboration, Actions, security (code scanning/dependabot/secret scanning/advisories), webhook, deploy key, environment, and ruleset data, and plans approval-gated reverse ETL actions through fixed GitHub REST and GraphQL operations.
- [gitlab](gitlab/MANUAL.md): Runs 582 bounded direct reads, four GitLab REST ETL streams, and 381 provider mutations through direct-write and approval-gated reverse-ETL commands; the groups deletion action also has one execution-bound, response-captured single-attempt warehouse-to-destination route.
...
- [google-analytics-data-api](google-analytics-data-api/MANUAL.md): Reads fixed Google Analytics 4 reports from the Analytics Data API runReport endpoint through declared response-header projection.
...
- [google-classroom](google-classroom/MANUAL.md): Reads Classroom courses and course-scoped resources through fixed REST routes and OAuth2 refresh-token authentication.
...
- [google-pagespeed-insights](google-pagespeed-insights/MANUAL.md): Reads Lighthouse PageSpeed Insights reports for a bounded Cartesian product of configured HTTPS URLs and mobile or desktop strategies through the fixed PageSpeed Insights v5 API.
...
- [less-annoying-crm](less-annoying-crm/MANUAL.md): Reads Less Annoying CRM users, contacts, tasks, notes, and events through fixed v2 RPC requests.
...
- [lokalise](lokalise/MANUAL.md): Reads Lokalise project keys, languages, translations, contributors, and comments through fixed API v2 routes.
...
- [mendeley](mendeley/MANUAL.md): Reads documents, folders, groups, and annotations through fixed Mendeley REST routes and OAuth refresh-token authentication.
...
- [mercado-ads](mercado-ads/MANUAL.md): Reads Mercado Ads advertiser and campaign metrics through fixed Advertising API routes.
...
- [metabase](metabase/MANUAL.md): Reads Metabase cards, dashboards, collections, databases, and users through the Metabase REST API using a declared session token.
...
- [mode](mode/MANUAL.md): Reads Mode workspace collections through fixed HAL+JSON REST routes.
...
- [my-hours](my-hours/MANUAL.md): Reads My Hours clients, projects, team members, tags, and bounded time-log windows through fixed REST routes.
- [mysql](mysql/MANUAL.md): Native MySQL source connector for wire-protocol checks, dynamic schemas, and bounded reads. Read-only source.
...
- [pocket](pocket/MANUAL.md): Reads saved Pocket items through the fixed v3 retrieve API.
...
- [rootly](rootly/MANUAL.md): Reads Rootly incidents, services, and users through fixed JSON:API routes.
...
- [safetyculture](safetyculture/MANUAL.md): Reads SafetyCulture audits, templates, and users through fixed REST routes.
...
- [yahoo-finance-price](yahoo-finance-price/MANUAL.md): Reads public Yahoo Finance chart prices as declaration-bound OHLCV records. Read-only.
