...
```text
NAME
  pm connectors inspect source-workable - Workable connector manual

SYNOPSIS
  pm connectors inspect source-workable
  pm connectors inspect source-workable --json
  pm credentials add <name> --connector source-workable [--config key=value] [--from-env field=ENV] [--value-stdin field]
