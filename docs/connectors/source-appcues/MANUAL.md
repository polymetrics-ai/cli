...
```text
NAME
  pm connectors inspect source-appcues - Appcues connector manual

SYNOPSIS
  pm connectors inspect source-appcues
  pm connectors inspect source-appcues --json
  pm credentials add <name> --connector source-appcues [--config key=value] [--from-env field=ENV] [--value-stdin field]

DESCRIPTION
  Appcues catalog connector. Native implementation status: planned_native_port.
