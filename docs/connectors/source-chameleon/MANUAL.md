# pm connectors inspect source-chameleon

```text
NAME
  pm connectors inspect source-chameleon - Chameleon connector manual

SYNOPSIS
  pm connectors inspect source-chameleon
  pm connectors inspect source-chameleon --json
  pm credentials add <name> --connector source-chameleon [--config key=value] [--from-env field=ENV] [--value-stdin field]
...
CONFIGURATION
  ...
  filter (string): Filter for using in the `segments_experiences` stream
