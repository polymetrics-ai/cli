# pm connectors inspect source-svix

```text
NAME
  pm connectors inspect source-svix - Svix connector manual

SYNOPSIS
  pm connectors inspect source-svix
  pm connectors inspect source-svix --json
  pm credentials add <name> --connector source-svix [--config key=value] [--from-env field=ENV] [--value-stdin field]
