...
COMMAND SURFACE
  Run Recurly's declared streams and reverse-ETL actions.
  Usage: pm recurly <command> [flags]
  Read streams
  Reverse ETL writes
  Other Commands
    a billing info remove - Delete Recurly remove a billing info via typed reverse ETL. [intent=reverse_etl availability=implemented write=remove_a_billing_info]; approval: requires plan, preview, approval, and execute; risk: critical — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required), --billing_info_id (required)
    ...
    account acquisition remove - Delete Recurly remove account acquisition via typed reverse ETL. [intent=reverse_etl availability=implemented write=remove_account_acquisition]; approval: requires plan, preview, approval, and execute; risk: critical — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required)
    ...
    account create - Create Recurly create account via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_account]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --code (required)
    ...
    account deactivate - Delete Recurly deactivate account via typed reverse ETL. [intent=reverse_etl availability=implemented write=deactivate_account]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required), --redact (required)
    account external account create - Create Recurly create account external account via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_account_external_account]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required), --external_account_code (required), --external_connection_type (required)
    account external account delete - Delete Recurly delete account external account via typed reverse ETL. [intent=reverse_etl availability=implemented write=delete_account_external_account]; approval: requires plan, preview, approval, and execute; risk: critical — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required), --external_account_id (required)
    ...
    account external account update - Update Recurly update account external account via typed reverse ETL. [intent=reverse_etl availability=implemented write=update_account_external_account]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required), --external_account_id (required)
    ...
    account note create - Create Recurly create account note via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_account_note]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required), --message (required)
    account note get - Get Recurly account note as an ETL record. [intent=etl availability=implemented stream=get_account_note]
    account note remove - Delete Recurly remove account note via typed reverse ETL. [intent=reverse_etl availability=implemented write=remove_account_note]; approval: requires plan, preview, approval, and execute; risk: critical — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required), --account_note_id (required)
    ...
    account reactivate - Update Recurly reactivate account via typed reverse ETL. [intent=reverse_etl availability=implemented write=reactivate_account]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required)
    account redact - Update Recurly redact account via typed reverse ETL. [intent=reverse_etl availability=implemented write=redact_account]; approval: requires plan, preview, approval, and execute; risk: critical — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required)
    ...
    account update - Update Recurly update account via typed reverse ETL. [intent=reverse_etl availability=implemented write=update_account]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required)
    ...
    billing info cvv verify - Create Recurly verify billing info cvv via typed reverse ETL. [intent=reverse_etl availability=implemented write=verify_billing_info_cvv]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required)
    billing info remove - Delete Recurly remove billing info via typed reverse ETL. [intent=reverse_etl availability=implemented write=remove_billing_info]; approval: requires plan, preview, approval, and execute; risk: critical — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required)
    ...
    billing info verify - Create Recurly verify billing info via typed reverse ETL. [intent=reverse_etl availability=implemented write=verify_billing_info]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required)
    billing infos cvv verify - Create Recurly verify billing infos cvv via typed reverse ETL. [intent=reverse_etl availability=implemented write=verify_billing_infos_cvv]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required), --billing_info_id (required)
    ...
    billing infos verify - Create Recurly verify billing infos via typed reverse ETL. [intent=reverse_etl availability=implemented write=verify_billing_infos]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required), --billing_info_id (required)
    ...
    coupon create - Create Recurly create coupon via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_coupon]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --code (required), --discount_type (required)
    coupon deactivate - Delete Recurly deactivate coupon via typed reverse ETL. [intent=reverse_etl availability=implemented write=deactivate_coupon]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --coupon_id (required)
    coupon get - Get Recurly coupon as an ETL record. [intent=etl availability=implemented stream=get_coupon]
    coupon redemption by id remove - Delete Recurly remove coupon redemption by id via typed reverse ETL. [intent=reverse_etl availability=implemented write=remove_coupon_redemption_by_id]; approval: requires plan, preview, approval, and execute; risk: critical — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required), --coupon_redemption_id (required)
    coupon redemption create - Create Recurly create coupon redemption via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_coupon_redemption]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required), --coupon_id (required)
    ...
    coupon redemption remove - Delete Recurly remove coupon redemption via typed reverse ETL. [intent=reverse_etl availability=implemented write=remove_coupon_redemption]; approval: requires plan, preview, approval, and execute; risk: critical — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required)
    coupon restore - Update Recurly restore coupon via typed reverse ETL. [intent=reverse_etl availability=implemented write=restore_coupon]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --coupon_id (required)
    coupon update - Update Recurly update coupon via typed reverse ETL. [intent=reverse_etl availability=implemented write=update_coupon]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --coupon_id (required)
    ...
    dunning-campaigns bulk-update - Update Recurly put dunning campaign bulk update via typed reverse ETL. [intent=reverse_etl availability=implemented write=put_dunning_campaign_bulk_update]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --dunning_campaign_id (required)
    ...
    export files get - List of the export files that are available to download. [intent=binary_download availability=implemented operation=get_export_files]; notes: Bounded Recurly download; the provider response is written only beneath an explicit --dest-root.; flags: --export-date, --dest-root (required), --file-name, --max-bytes
    external invoice create - Create Recurly create external invoice via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_external_invoice]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --currency (required), --external_id (required), --external_subscription_id (required), --purchased_at (required), --state (required), --total (required)
    ...
    external product create - Create Recurly create external product via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_external_product]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --name (required)
    external product external product reference create - Create Recurly create external product external product reference via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_external_product_external_product_reference]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --external_product_id (required)
    external product external product reference deactivate - Delete Recurly deactivate external product external product reference via typed reverse ETL. [intent=reverse_etl availability=implemented write=deactivate_external_product_external_product_reference]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --external_product_id (required), --external_product_reference_id (required)
    ...
    external product update - Update Recurly update external product via typed reverse ETL. [intent=reverse_etl availability=implemented write=update_external_product]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --external_product_id (required), --plan_id (required)
    external products deactivate - Delete Recurly deactivate external products via typed reverse ETL. [intent=reverse_etl availability=implemented write=deactivate_external_products]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --external_product_id (required)
    ...
    external subscription create - Create Recurly create external subscription via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_external_subscription]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.
    ...
    external-subscriptions put - Update Recurly put external subscription via typed reverse ETL. [intent=reverse_etl availability=implemented write=put_external_subscription]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --external_subscription_id (required)
    general ledger account create - Create Recurly create general ledger account via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_general_ledger_account]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.
    ...
    general ledger account update - Update Recurly update general ledger account via typed reverse ETL. [intent=reverse_etl availability=implemented write=update_general_ledger_account]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --general_ledger_account_id (required)
    ...
    gift card create - Create Recurly create gift card via typed reverse ETL. [intent=reverse_etl availability=not_implemented write=create_gift_card]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; notes: named_dependency=engine.reverse_etl_scalar_flag_contract: the reverse-ETL command surface cannot faithfully expose this action's required object or array record fields as scalar flags
    gift card get - Get Recurly gift card as an ETL record. [intent=etl availability=implemented stream=get_gift_card]
    gift card redeem - Create Recurly redeem gift card via typed reverse ETL. [intent=reverse_etl availability=not_implemented write=redeem_gift_card]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; notes: named_dependency=engine.reverse_etl_scalar_flag_contract: the reverse-ETL command surface cannot faithfully expose this action's required object or array record fields as scalar flags
    ...
    invoice collect - Update Recurly collect invoice via typed reverse ETL. [intent=reverse_etl availability=implemented write=collect_invoice]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --invoice_id (required)
    ...
    invoice create - Create Recurly create invoice via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_invoice]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required), --currency (required)
    invoice failed mark - Update Recurly mark invoice failed via typed reverse ETL. [intent=reverse_etl availability=implemented write=mark_invoice_failed]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --invoice_id (required)
    ...
    invoice pdf get - Fetch an invoice as a PDF [intent=binary_download availability=implemented operation=get_invoice_pdf]; notes: Bounded Recurly download; the provider response is written only beneath an explicit --dest-root.; flags: --invoice-id, --dest-root (required), --file-name, --max-bytes
    invoice refund - Create Recurly refund invoice via typed reverse ETL. [intent=reverse_etl availability=implemented write=refund_invoice]; approval: requires plan, preview, approval, and execute; risk: critical — refund_invoice moves money by refunding an invoice; requires destructive confirmation and reverse ETL plan/preview/approval/execute. Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --invoice_id (required), --type (required)
    invoice reopen - Update Recurly reopen invoice via typed reverse ETL. [intent=reverse_etl availability=implemented write=reopen_invoice]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --invoice_id (required)
    ...
    invoice update - Update Recurly update invoice via typed reverse ETL. [intent=reverse_etl availability=implemented write=update_invoice]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --invoice_id (required)
    ...
    invoices apply-credit-balance - Update Recurly apply credit balance via typed reverse ETL. [intent=reverse_etl availability=implemented write=apply_credit_balance]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --invoice_id (required)
    invoices list - List Recurly invoices as ETL records. [intent=etl availability=implemented stream=invoices]
    invoices preview - Preview new invoice for pending line items [intent=direct_read availability=implemented operation=preview_invoice]; notes: Bounded Recurly read/preview operation; fixed method and path, typed request fields, and redacted JSON output.; flags: --account-id, --currency (required), --page, --page-cursor
    invoices record-external-transaction - Create Recurly record external transaction via typed reverse ETL. [intent=reverse_etl availability=implemented write=record_external_transaction]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --invoice_id (required)
    invoices retries create - Create Recurly create invoice retry via typed reverse ETL. [intent=reverse_etl availability=not_implemented write=create_invoice_retry]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; notes: named_dependency=engine.reverse_etl_scalar_flag_contract: the reverse-ETL command surface cannot faithfully expose this action's required object or array record fields as scalar flags
    item create - Create Recurly create item via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_item]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --code (required), --name (required)
    item deactivate - Delete Recurly deactivate item via typed reverse ETL. [intent=reverse_etl availability=implemented write=deactivate_item]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --item_id (required)
    item get - Get Recurly item as an ETL record. [intent=etl availability=implemented stream=get_item]
    item reactivate - Update Recurly reactivate item via typed reverse ETL. [intent=reverse_etl availability=implemented write=reactivate_item]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --item_id (required)
    item update - Update Recurly update item via typed reverse ETL. [intent=reverse_etl availability=implemented write=update_item]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --item_id (required)
    items list - List Recurly items as ETL records. [intent=etl availability=implemented stream=list_items]
    line item create - Create Recurly create line item via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_line_item]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required), --currency (required), --type (required), --unit_amount (required)
    line item get - Get Recurly line item as an ETL record. [intent=etl availability=implemented stream=get_line_item]
    line item remove - Delete Recurly remove line item via typed reverse ETL. [intent=reverse_etl availability=implemented write=remove_line_item]; approval: requires plan, preview, approval, and execute; risk: critical — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --line_item_id (required)
    line items list - List Recurly line items as ETL records. [intent=etl availability=implemented stream=list_line_items]
    measured unit create - Create Recurly create measured unit via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_measured_unit]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --display_name (required), --name (required)
    ...
    measured unit remove - Delete Recurly remove measured unit via typed reverse ETL. [intent=reverse_etl availability=implemented write=remove_measured_unit]; approval: requires plan, preview, approval, and execute; risk: critical — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --measured_unit_id (required)
    measured unit update - Update Recurly update measured unit via typed reverse ETL. [intent=reverse_etl availability=implemented write=update_measured_unit]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --measured_unit_id (required)
    ...
    plan add on create - Create Recurly create plan add on via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_plan_add_on]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --code (required), --name (required), --plan_id (required)
    plan add on get - Get Recurly plan add on as an ETL record. [intent=etl availability=implemented stream=get_plan_add_on]
    plan add on remove - Delete Recurly remove plan add on via typed reverse ETL. [intent=reverse_etl availability=implemented write=remove_plan_add_on]; approval: requires plan, preview, approval, and execute; risk: critical — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --add_on_id (required), --plan_id (required)
    plan add on update - Update Recurly update plan add on via typed reverse ETL. [intent=reverse_etl availability=implemented write=update_plan_add_on]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --add_on_id (required), --plan_id (required)
    plan add ons list - List Recurly plan add ons as ETL records. [intent=etl availability=implemented stream=list_plan_add_ons]
    plan create - Create Recurly create plan via typed reverse ETL. [intent=reverse_etl availability=not_implemented write=create_plan]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; notes: named_dependency=engine.reverse_etl_scalar_flag_contract: the reverse-ETL command surface cannot faithfully expose this action's required object or array record fields as scalar flags
    plan get - Get Recurly plan as an ETL record. [intent=etl availability=implemented stream=get_plan]
    plan remove - Delete Recurly remove plan via typed reverse ETL. [intent=reverse_etl availability=implemented write=remove_plan]; approval: requires plan, preview, approval, and execute; risk: critical — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --plan_id (required)
    plan update - Update Recurly update plan via typed reverse ETL. [intent=reverse_etl availability=implemented write=update_plan]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --plan_id (required)
    ...
    purchase create - Create Recurly create purchase via typed reverse ETL. [intent=reverse_etl availability=not_implemented write=create_purchase]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; notes: named_dependency=engine.reverse_etl_scalar_flag_contract: the reverse-ETL command surface cannot faithfully expose this action's required object or array record fields as scalar flags
    purchases authorize - Create Recurly create authorize purchase via typed reverse ETL. [intent=reverse_etl availability=not_implemented write=create_authorize_purchase]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; notes: named_dependency=engine.reverse_etl_scalar_flag_contract: the reverse-ETL command surface cannot faithfully expose this action's required object or array record fields as scalar flags
    purchases cancel - Create Recurly cancel purchase via typed reverse ETL. [intent=reverse_etl availability=implemented write=cancel_purchase]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --transaction_id (required)
    purchases capture - Create Recurly create capture purchase via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_capture_purchase]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --transaction_id (required)
    purchases create-pending - Create Recurly create pending purchase via typed reverse ETL. [intent=reverse_etl availability=not_implemented write=create_pending_purchase]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; notes: named_dependency=engine.reverse_etl_scalar_flag_contract: the reverse-ETL command surface cannot faithfully expose this action's required object or array record fields as scalar flags
    ...
    shipping address create - Create Recurly create shipping address via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_shipping_address]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --account_id (required), --city (required), --country (required), --first_name (required), --last_name (required), --postal_code (required), --street1 (required)
    ...
    shipping method create - Create Recurly create shipping method via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_shipping_method]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --code (required), --name (required)
    shipping method deactivate - Delete Recurly deactivate shipping method via typed reverse ETL. [intent=reverse_etl availability=implemented write=deactivate_shipping_method]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --shipping_method_id (required)
    ...
    shipping method update - Update Recurly update shipping method via typed reverse ETL. [intent=reverse_etl availability=implemented write=update_shipping_method]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --shipping_method_id (required)
    ...
    subscription cancel - Update Recurly cancel subscription via typed reverse ETL. [intent=reverse_etl availability=implemented write=cancel_subscription]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --subscription_id (required)
    subscription change create - Create Recurly create subscription change via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_subscription_change]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --subscription_id (required)
    ...
    subscription change remove - Delete Recurly remove subscription change via typed reverse ETL. [intent=reverse_etl availability=implemented write=remove_subscription_change]; approval: requires plan, preview, approval, and execute; risk: critical — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --subscription_id (required)
    ...
    subscription coupon redemption remove - Delete Recurly remove subscription coupon redemption via typed reverse ETL. [intent=reverse_etl availability=implemented write=remove_subscription_coupon_redemption]; approval: requires plan, preview, approval, and execute; risk: critical — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --coupon_redemption_id (required), --subscription_id (required)
    ...
    subscription create - Create Recurly create subscription via typed reverse ETL. [intent=reverse_etl availability=not_implemented write=create_subscription]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; notes: named_dependency=engine.reverse_etl_scalar_flag_contract: the reverse-ETL command surface cannot faithfully expose this action's required object or array record fields as scalar flags
    ...
    subscription pause - Update Recurly pause subscription via typed reverse ETL. [intent=reverse_etl availability=implemented write=pause_subscription]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --remaining_pause_cycles (required), --subscription_id (required)
    subscription reactivate - Update Recurly reactivate subscription via typed reverse ETL. [intent=reverse_etl availability=implemented write=reactivate_subscription]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --subscription_id (required)
    ...
    subscription terminate - Delete Recurly terminate subscription via typed reverse ETL. [intent=reverse_etl availability=implemented write=terminate_subscription]; approval: requires plan, preview, approval, and execute; risk: critical — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --charge (required), --refund (required), --subscription_id (required)
    subscription update - Update Recurly update subscription via typed reverse ETL. [intent=reverse_etl availability=implemented write=update_subscription]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --subscription_id (required)
    subscriptions convert-trial - Update Recurly convert trial via typed reverse ETL. [intent=reverse_etl availability=implemented write=convert_trial]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --subscription_id (required)
    subscriptions list - List Recurly subscriptions as ETL records. [intent=etl availability=implemented stream=subscriptions]
    subscriptions preview change - Preview a new subscription change [intent=direct_read availability=implemented operation=preview_subscription_change]; notes: Bounded Recurly read/preview operation; fixed method and path, typed request fields, and redacted JSON output.; flags: --subscription-id, --page, --page-cursor
    subscriptions preview renewal - Fetch a preview of a subscription's renewal invoice(s) [intent=direct_read availability=implemented operation=get_preview_renewal]; notes: Bounded Recurly read/preview operation; fixed method and path, typed request fields, and redacted JSON output.; flags: --subscription-id, --page, --page-cursor
    ...
    unique coupon code deactivate - Delete Recurly deactivate unique coupon code via typed reverse ETL. [intent=reverse_etl availability=implemented write=deactivate_unique_coupon_code]; approval: requires plan, preview, approval, and execute; risk: high — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --unique_coupon_code_id (required)
    ...
    unique coupon code reactivate - Update Recurly reactivate unique coupon code via typed reverse ETL. [intent=reverse_etl availability=implemented write=reactivate_unique_coupon_code]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --unique_coupon_code_id (required)
    ...
    unique-coupon-codes generate - Create Recurly generate unique coupon codes via typed reverse ETL. [intent=reverse_etl availability=implemented write=generate_unique_coupon_codes]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --coupon_id (required)
    unique-coupon-codes generate-sync - Create Recurly generate unique coupon codes sync via typed reverse ETL. [intent=reverse_etl availability=implemented write=generate_unique_coupon_codes_sync]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --coupon_id (required), --number_of_unique_codes (required)
    usage create - Create Recurly create usage via typed reverse ETL. [intent=reverse_etl availability=implemented write=create_usage]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --add_on_id (required), --subscription_id (required)
    ...
    usage remove - Delete Recurly remove usage via typed reverse ETL. [intent=reverse_etl availability=implemented write=remove_usage]; approval: requires plan, preview, approval, and execute; risk: critical — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --usage_id (required)
    usage update - Update Recurly update usage via typed reverse ETL. [intent=reverse_etl availability=implemented write=update_usage]; approval: requires plan, preview, approval, and execute; risk: medium — Recurly supports provider idempotency for POST/PUT/DELETE through the Idempotency-Key header; keep reverse ETL in plan/preview/approve/execute and the runtime binds one fresh key to each approved record and reuses it only for that record's automatic retries.; flags: --usage_id (required)
