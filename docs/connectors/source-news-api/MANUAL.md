...
```text
NAME
  pm connectors inspect source-news-api - News Api connector manual

SYNOPSIS
  pm connectors inspect source-news-api
  pm connectors inspect source-news-api --json
  pm credentials add <name> --connector source-news-api [--config key=value] [--from-env field=ENV] [--value-stdin field]

DESCRIPTION
  News Api catalog connector. Native implementation status: planned_native_port.
