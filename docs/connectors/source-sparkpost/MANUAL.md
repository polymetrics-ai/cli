...
```text
NAME
  pm connectors inspect source-sparkpost - SparkPost connector manual

SYNOPSIS
  pm connectors inspect source-sparkpost
  pm connectors inspect source-sparkpost --json
  pm credentials add <name> --connector source-sparkpost [--config key=value] [--from-env field=ENV] [--value-stdin field]
