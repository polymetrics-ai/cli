# pm connectors inspect source-zonka-feedback

```text
NAME
  pm connectors inspect source-zonka-feedback - Zonka Feedback connector manual

SYNOPSIS
  pm connectors inspect source-zonka-feedback
  pm connectors inspect source-zonka-feedback --json
  pm credentials add <name> --connector source-zonka-feedback [--config key=value] [--from-env field=ENV] [--value-stdin field]
...
CONFIGURATION
  auth_token (string) required secret: Auth token to use. Generate it by navigating to Company Settings > Developers > API in your Zonka Feedback account.
  datacenter (string) required: The identifier for the data center, such as 'us1' or 'e' for EU.
