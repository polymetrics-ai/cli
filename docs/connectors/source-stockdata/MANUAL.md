# pm connectors inspect source-stockdata

```text
NAME
  pm connectors inspect source-stockdata - StockData connector manual

SYNOPSIS
  pm connectors inspect source-stockdata
  pm connectors inspect source-stockdata --json
  pm credentials add <name> --connector source-stockdata [--config key=value] [--from-env field=ENV] [--value-stdin field]
...
CONFIGURATION
  ...
  industries (array): Specify the industries of entities which have been identified within the article.
