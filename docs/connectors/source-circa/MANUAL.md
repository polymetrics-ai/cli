# pm connectors inspect source-circa

```text
NAME
  pm connectors inspect source-circa - Circa connector manual

SYNOPSIS
  pm connectors inspect source-circa
  pm connectors inspect source-circa --json
  pm credentials add <name> --connector source-circa [--config key=value] [--from-env field=ENV] [--value-stdin field]
...
CONFIGURATION
  api_key (string) required secret: API key to use. Find it at https://app.circa.co/settings/integrations/api
