...
```text
NAME
  pm connectors inspect source-rollbar - Rollbar connector manual

SYNOPSIS
  pm connectors inspect source-rollbar
  pm connectors inspect source-rollbar --json
  pm credentials add <name> --connector source-rollbar [--config key=value] [--from-env field=ENV] [--value-stdin field]
