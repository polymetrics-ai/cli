...
COMMAND SURFACE
  ...
  Permissions
    permissions profiles list - List Gong permission profiles as ETL records. [intent=etl availability=implemented stream=permission_profiles]; flags: --workspaceId
  ...
  CRM
    ...
    crm entities get - Get CRM objects (/v2/crm/entities) [intent=direct_read availability=planned]; risk: blocked until shared direct-read query serialization supports OpenAPI FORM/explode repeated query values for the required objectsCrmIds array; notes: Do not dispatch this command through the scalar query runner; a valid request must encode objectsCrmIds as repeated query parameters.; flags: --integrationId, --objectType, --objectsCrmIds
  ...
  Engage Flows
    flows list - List Gong Engage flows as ETL records. [intent=etl availability=implemented stream=flows]; flags: --flowOwnerEmail
    flows folders list - List Gong Engage flow folders as ETL records. [intent=etl availability=implemented stream=flow_folders]; flags: --flowFolderOwnerEmail
