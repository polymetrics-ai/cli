...
- upload_call_media:
  ...
  - risk: high: uploads call media from a bounded project-local file path; requires reverse ETL approval and destructive confirmation
- upload_crm_entities:
  - endpoint: POST /v2/crm/entities
  - required fields: data_file_path
  - risk: high: uploads CRM data from a bounded project-local file path; requires reverse ETL approval and destructive confirmation
- upload_crm_entity_schema:
  - endpoint: POST /v2/crm/entity-schema
  - required fields: selected_fields
  - risk: high: uploads CRM entity schema as a top-level JSON array; requires reverse ETL approval and destructive confirmation
