# pm connectors inspect source-statsig

```text
NAME
  pm connectors inspect source-statsig - Statsig connector manual

SYNOPSIS
  pm connectors inspect source-statsig
  pm connectors inspect source-statsig --json
  pm credentials add <name> --connector source-statsig [--config key=value] [--from-env field=ENV] [--value-stdin field]
