...
```text
NAME
  pm connectors inspect source-metabase - Metabase connector manual

SYNOPSIS
  pm connectors inspect source-metabase
  pm connectors inspect source-metabase --json
  pm credentials add <name> --connector source-metabase [--config key=value] [--from-env field=ENV] [--value-stdin field]
