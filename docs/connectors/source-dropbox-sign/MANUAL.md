...
```text
NAME
  pm connectors inspect source-dropbox-sign - Dropbox Sign connector manual

SYNOPSIS
  pm connectors inspect source-dropbox-sign
  pm connectors inspect source-dropbox-sign --json
  pm credentials add <name> --connector source-dropbox-sign [--config key=value] [--from-env field=ENV] [--value-stdin field]

DESCRIPTION
  Dropbox Sign catalog connector. Native implementation status: planned_native_port.
