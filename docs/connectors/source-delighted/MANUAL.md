...
```text
NAME
  pm connectors inspect source-delighted - Delighted connector manual

SYNOPSIS
  pm connectors inspect source-delighted
  pm connectors inspect source-delighted --json
  pm credentials add <name> --connector source-delighted [--config key=value] [--from-env field=ENV] [--value-stdin field]
