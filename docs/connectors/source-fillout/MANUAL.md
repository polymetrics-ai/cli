...
```text
NAME
  pm connectors inspect source-fillout - Fillout connector manual

SYNOPSIS
  pm connectors inspect source-fillout
  pm connectors inspect source-fillout --json
  pm credentials add <name> --connector source-fillout [--config key=value] [--from-env field=ENV] [--value-stdin field]

DESCRIPTION
  Fillout catalog connector. Native implementation status: planned_native_port.
