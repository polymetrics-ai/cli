# pm connectors inspect source-dwolla

```text
NAME
  pm connectors inspect source-dwolla - Dwolla connector manual

SYNOPSIS
  pm connectors inspect source-dwolla
  pm connectors inspect source-dwolla --json
  pm credentials add <name> --connector source-dwolla [--config key=value] [--from-env field=ENV] [--value-stdin field]
...
CONFIGURATION
  ...
  environment (string) required: The environment for the Dwolla API, either 'api-sandbox' or 'api'.
