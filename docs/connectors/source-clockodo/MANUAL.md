# pm connectors inspect source-clockodo

```text
NAME
  pm connectors inspect source-clockodo - Clockodo connector manual

SYNOPSIS
  pm connectors inspect source-clockodo
  pm connectors inspect source-clockodo --json
  pm credentials add <name> --connector source-clockodo [--config key=value] [--from-env field=ENV] [--value-stdin field]
...
CONFIGURATION
  api_key (string) required secret: API key to use. Find it in the 'Personal data' section of your Clockodo account.
  ...
  years (array) required: 2024, 2025
