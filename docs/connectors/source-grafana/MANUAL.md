...
```text
NAME
  pm connectors inspect source-grafana - Grafana connector manual

SYNOPSIS
  pm connectors inspect source-grafana
  pm connectors inspect source-grafana --json
  pm credentials add <name> --connector source-grafana [--config key=value] [--from-env field=ENV] [--value-stdin field]
