...
```text
NAME
  pm connectors inspect source-defillama - Curve connector manual

SYNOPSIS
  pm connectors inspect source-defillama
  pm connectors inspect source-defillama --json
  pm credentials add <name> --connector source-defillama [--config key=value] [--from-env field=ENV] [--value-stdin field]
