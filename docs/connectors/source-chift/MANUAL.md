# pm connectors inspect source-chift

```text
NAME
  pm connectors inspect source-chift - Chift connector manual

SYNOPSIS
  pm connectors inspect source-chift
  pm connectors inspect source-chift --json
  pm credentials add <name> --connector source-chift [--config key=value] [--from-env field=ENV] [--value-stdin field]
...
CONFIGURATION
  ...
  secret fields: account_id, client_id, client_secret
