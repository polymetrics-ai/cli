...
```text
NAME
  pm connectors inspect destination-ragie - Ragie connector manual

SYNOPSIS
  pm connectors inspect destination-ragie
  pm connectors inspect destination-ragie --json
  pm credentials add <name> --connector destination-ragie [--config key=value] [--from-env field=ENV] [--value-stdin field]
