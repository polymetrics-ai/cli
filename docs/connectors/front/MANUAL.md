...
COMMAND SURFACE
  ...
  Other Commands
    analytics exports create - Create a new analytics export with bounded JSON output. [intent=direct_read availability=implemented operation=front.create_analytics_export]; approval: none for read/query; no provider write execution is performed by these direct-read commands; risk: high: analytics report/export metadata may expose operational content; output is bounded and redacted; notes: Fixed provider search/query endpoint; distinct from warehouse pm query.; flags: --columns (required), --type (required), --start (required), --end (required)
