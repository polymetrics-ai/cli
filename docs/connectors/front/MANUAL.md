...
COMMAND SURFACE
  ...
  Reverse ETL writes
    ...
    create-teammate-discussion - Create a discussion for one or more teammates. [intent=reverse_etl availability=implemented write=create_conversation]; approval: reverse ETL plan -> preview -> explicit approval -> execute; typed destructive confirmation required by the action metadata; risk: high: fixed Front POST /conversations mutation; reverse ETL requires plan -> preview -> explicit approval -> execute; notes: Discussions require a starter comment and exactly one destination. This variant sends teammate-ids and never inbox-id; task-only fields are unavailable.; flags: --type (required), --subject (required), --teammate-ids (required), --comment-body (required), --comment-author-id
    ...
    create-teammate-task - Create a task for one or more teammates. [intent=reverse_etl availability=implemented write=create_conversation]; approval: reverse ETL plan -> preview -> explicit approval -> execute; typed destructive confirmation required by the action metadata; risk: high: fixed Front POST /conversations mutation; reverse ETL requires plan -> preview -> explicit approval -> execute; notes: Tasks require exactly one destination. This variant sends teammate-ids and never inbox-id; description and due-at are task-only fields.; flags: --type (required), --subject (required), --teammate-ids (required), --description, --due-at
