...
COMMAND SURFACE
  ...
  Other Commands
    analytics exports create - Create a message analytics export with bounded JSON output. [intent=direct_read availability=implemented operation=front.create_analytics_export]; approval: none for read/query; no provider write execution is performed by these direct-read commands; risk: high: analytics report/export metadata may expose operational content; output is bounded and redacted; notes: Message exports accept only documented message columns. Front permits columns: [], but the required CLI flag cannot transmit it pending cli-engine-empty-required-array-r1. Event exports use the fixed-event or parameterized-event commands; no raw request body is accepted.; flags: --columns (required), --type (required), --start (required), --end (required), --timezone, --filter-tag-ids, --filter-teammate-ids, --filter-channel-ids, --filter-inbox-ids, --filter-team-ids, --filter-account-ids
    analytics exports create-events - Create a fixed-column event analytics export with bounded JSON output. [intent=direct_read availability=implemented operation=front.create_analytics_export]; approval: none for read/query; no provider write execution is performed by these direct-read commands; risk: high: analytics report/export metadata may expose operational content; output is bounded and redacted; notes: Event exports with a fixed column use this command. Front permits columns: [], but the required CLI flag cannot transmit it pending cli-engine-empty-required-array-r1. Time-in-status, transitions, custom-field, updated-custom-field, and Smart QA columns require the typed parameterized commands.; flags: --columns (required), --type (required), --start (required), --end (required), --timezone, --filter-tag-ids, --filter-teammate-ids, --filter-channel-ids, --filter-inbox-ids, --filter-team-ids, --filter-account-ids
