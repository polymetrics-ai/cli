# pm connectors inspect source-fleetio

```text
NAME
  pm connectors inspect source-fleetio - Fleetio connector manual

SYNOPSIS
  pm connectors inspect source-fleetio
  pm connectors inspect source-fleetio --json
  pm credentials add <name> --connector source-fleetio [--config key=value] [--from-env field=ENV] [--value-stdin field]
...
CONFIGURATION
  account_token (string) required secret
  api_key (string) required secret
  secret fields: account_token, api_key
