# pm connectors inspect source-gainsight-px

```text
NAME
  pm connectors inspect source-gainsight-px - Gainsight Px connector manual

SYNOPSIS
  pm connectors inspect source-gainsight-px
  pm connectors inspect source-gainsight-px --json
  pm credentials add <name> --connector source-gainsight-px [--config key=value] [--from-env field=ENV] [--value-stdin field]
...
CONFIGURATION
  api_key (string) required secret: The Aptrinsic API Key which is recieved from the dashboard settings (ref - https://app.aptrinsic.com/settings/api-keys)
