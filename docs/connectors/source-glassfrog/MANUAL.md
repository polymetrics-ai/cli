# pm connectors inspect source-glassfrog

```text
NAME
  pm connectors inspect source-glassfrog - Glassfrog connector manual

SYNOPSIS
  pm connectors inspect source-glassfrog
  pm connectors inspect source-glassfrog --json
  pm credentials add <name> --connector source-glassfrog [--config key=value] [--from-env field=ENV] [--value-stdin field]
...
CONFIGURATION
  api_key (string) required secret: API key provided by Glassfrog
