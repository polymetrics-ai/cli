...
```text
NAME
  pm connectors inspect source-chargebee - Chargebee connector manual

SYNOPSIS
  pm connectors inspect source-chargebee
  pm connectors inspect source-chargebee --json
  pm credentials add <name> --connector source-chargebee [--config key=value] [--from-env field=ENV] [--value-stdin field]
