...
```text
NAME
  pm connectors inspect source-stigg - Stigg connector manual

SYNOPSIS
  pm connectors inspect source-stigg
  pm connectors inspect source-stigg --json
  pm credentials add <name> --connector source-stigg [--config key=value] [--from-env field=ENV] [--value-stdin field]
