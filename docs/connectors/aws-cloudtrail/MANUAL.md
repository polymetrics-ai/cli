...
COMMAND SURFACE
  ...
  Other Commands
    ...
    query cancel - Cancel a running CloudTrail Lake query by query id. [intent=reverse_etl availability=implemented write=cancel_query]; approval: reverse ETL writes require plan -> preview -> explicit approval -> execute; risk: cancels a running CloudTrail Lake query; flags: --event-data-store, --event-data-store-owner-account-id, --query-id (required)
