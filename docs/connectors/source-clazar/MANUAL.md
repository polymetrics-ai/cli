...
```text
NAME
  pm connectors inspect source-clazar - Clazar connector manual

SYNOPSIS
  pm connectors inspect source-clazar
  pm connectors inspect source-clazar --json
  pm credentials add <name> --connector source-clazar [--config key=value] [--from-env field=ENV] [--value-stdin field]
