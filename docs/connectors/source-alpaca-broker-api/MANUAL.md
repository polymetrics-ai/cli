...
```text
NAME
  pm connectors inspect source-alpaca-broker-api - Alpaca Broker API connector manual

SYNOPSIS
  pm connectors inspect source-alpaca-broker-api
  pm connectors inspect source-alpaca-broker-api --json
  pm credentials add <name> --connector source-alpaca-broker-api [--config key=value] [--from-env field=ENV] [--value-stdin field]
