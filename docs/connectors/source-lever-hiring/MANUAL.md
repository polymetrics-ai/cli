...
```text
NAME
  pm connectors inspect source-lever-hiring - Lever Hiring connector manual

SYNOPSIS
  pm connectors inspect source-lever-hiring
  pm connectors inspect source-lever-hiring --json
  pm credentials add <name> --connector source-lever-hiring [--config key=value] [--from-env field=ENV] [--value-stdin field]

DESCRIPTION
  Lever Hiring catalog connector. Native implementation status: planned_native_port.
