...
COMMAND SURFACE
  ...
  Other Commands
    ...
    audit logs list - List audit log events [intent=direct_read availability=implemented operation=dockerhub.auditlogs_listauditlogs]; approval: none; risk: Docker Hub provider authorization is enforced at runtime.; flags: --account (required), --action, --name, --actor, --from, --to, --page, --page-cursor
    scim users list - List users [intent=direct_read availability=implemented operation=dockerhub.get__v2_scim_2.0_users]; approval: none; risk: Docker Hub provider authorization is enforced at runtime.; flags: --filter, --attributes, --sortOrder, --sortBy, --page, --page-cursor
    scim user create - Create user [intent=direct_write availability=implemented operation=dockerhub.post__v2_scim_2.0_users]; approval: Direct writes require plan, preview, approval, and execute.; risk: Docker Hub SCIM user provisioning; direct write requires plan, preview, approval, and execute.; flags: --schemas (required), --user-name (required), --given-name, --family-name
    scim user update - Update a user [intent=direct_write availability=implemented operation=dockerhub.put__v2_scim_2.0_users__id_]; approval: Direct writes require plan, preview, approval, and execute.; risk: Docker Hub SCIM user update; direct write requires plan, preview, approval, and execute.; flags: --id (required), --schemas (required), --given-name, --family-name, --enabled
