...
COMMAND SURFACE
  ...
  Other Commands
    ...
    auth 2fa-login create - Second factor authentication (completes a 2FA-challenged login) [intent=reverse_etl availability=planned]; notes: Docker Hub's documented second-factor login completion. Not implemented: this command's only inputs are a live intermediate token and a TOTP code and its only output is a live session JWT, and the reverse-ETL path it would run on maps flags to a plan record that is written to the project state file in plaintext and echoed in plan output. Blocked on a named dependency — requires secure secret input (stdin/env/vault-reference), encrypted or ephemeral plan storage, and a secure sink for the returned token.
    auth login create - Create an authentication token (username+password/PAT session login) [intent=reverse_etl availability=planned]; notes: Docker Hub's documented username+password/PAT session login. Not implemented: this command's only input is a live credential and its only output is a live session JWT, and the reverse-ETL path it would run on maps flags to a plan record that is written to the project state file in plaintext and echoed in plan output. Blocked on a named dependency — requires secure secret input (stdin/env/vault-reference), encrypted or ephemeral plan storage, and a secure sink for the returned token. The same exchange is already performed internally, without a plan record, by the dockerhub AuthHook when docker_pat is configured.
    auth token create - Create access token (identifier+secret to short-lived access token exchange) [intent=reverse_etl availability=planned]; notes: Docker Hub's documented identifier+secret to short-lived access token exchange. Not implemented: this command's only input is a live credential and its only output is a live token, and the reverse-ETL path it would run on maps flags to a plan record that is written to the project state file in plaintext and echoed in plan output. Blocked on a named dependency — requires secure secret input (stdin/env/vault-reference), encrypted or ephemeral plan storage, and a secure sink for the returned token. The same exchange is already performed internally, without a plan record, by the dockerhub AuthHook when docker_pat is configured.
