...
**30 connectors per batch**: it retains a reviewable failure-isolated report
and one PR/CI merge cost. Increase to 40 only after another 30-connector run
shows that its generated artifact diff and per-candidate report remain
reviewable; the limit is review throughput, not the tool's loop.
