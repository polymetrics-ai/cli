...
so the capability stays false rather than faking a runnable CDC surface. Its `database.json` is
the reference strict policy-only declaration described below; it does not register a driver or
promote a capability. The package has **no `init()`/`RegisterFactory`/
