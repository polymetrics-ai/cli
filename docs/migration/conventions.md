...
A `file` part may also declare `allowed_media_types`, which bounds what the file's **own bytes** may sniff as (`http.DetectContentType`), checked before any request is made. Three consequences worth knowing: it is the only restriction on a part's **sniffed media type**, so **a single-entry list is how a bundle demands exactly one type**; when it is present the part's wire `Content-Type` header is set from the sniffed type rather than from the declared `content_type`, because the sniffed type has just been proven to be one the bundle accepts and asserting the declared one over disagreeing bytes would be a claim we had already falsified; and when it is absent the declared `content_type` is sent untouched, since nothing verified the sniff and `http.DetectContentType` is coarse (every CSV sniffs as `text/plain`). Omit the key to leave a part unconstrained — a present-but-empty list is a load-time error rather than a silent "allow anything", and a `content_type` outside its own `allowed_media_types` is rejected at load time as an unsatisfiable declaration.

For a provider's `.json only` file rule, do **not** misuse `allowed_media_types`: ordinary JSON has no magic signature and Go sniffs it as text/plain. Instead declare the file part's exact wire `content_type: "application/json"`, `content_validation: "json"`, and `allowed_file_extensions: [".json"]`. The extension list is case-insensitive, closed, and checked before a request; the approved bounded snapshot is then parsed as exactly one JSON document before it can reach the provider. For a provider's CSV-only rule, use the equivalent `content_type: "text/csv"`, `content_validation: "csv"`, and `allowed_file_extensions: [".csv"]`; the bounded snapshot is parsed as CSV grammar before dispatch, while the provider-specific column semantics stay in that operation's own schema. JSON and CSV validation each require a positive per-file or aggregate multipart cap, so neither can turn an otherwise unbounded upload into an unbounded parse. These keys are file-part-only, never caller flags, and are reusable only where the provider documents the same constraint.
