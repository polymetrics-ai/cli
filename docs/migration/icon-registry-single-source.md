...
- Website script invariants (bare registry keys, canonical-vs-generated asset parity, icon sync path containment) are gated by `pnpm run test:scripts` in the website workflow, which also runs when `internal/connectors/icon_data.json` changes.
