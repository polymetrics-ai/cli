...
- Website script invariants (bare registry keys, canonical-vs-generated asset parity, icon sync path containment, Simple Icons fetch-boundary slug/path validation) are gated by `pnpm run test:scripts` in the website workflow. That workflow runs for both canonical inputs of the generated website icon tree — `internal/connectors/icon_data.json` and `docs/connectors/icons/**` — so a refreshed or replaced canonical SVG cannot merge while `website/public/connectors/icons/**` stays stale.
