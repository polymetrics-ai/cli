...
**Human-first entry:** `pm query` with no action selected starts the read-only query workspace only
when stdin and stdout are TTYs and no bypass applies. In non-TTY/CI contexts, or with `--json`,
`--plain`, `--no-input`, PM_NO_TUI, or TERM=dumb, it renders deterministic contextual help and exits
0. `pm query --help` always renders help. Invalid actions still return usage errors.

**`pm query grid` (explicit alias):** opens the same alt-screen browser under the same gate.
...
- Bypass and accessibility: bare `pm query` and `pm query grid` with `--plain`, `--json`, or
  `--no-input` never run a
  ...
  consuming scripted input. On those bypass paths, bare `pm query` renders contextual help and
  exits 0; it does not initialize the workspace.
...
### 2.9 Human-first guided reverse-ETL session (`pm reverse`; alias `pm reverse guide`)

Bare `pm reverse` starts the guided session only on an eligible dual-TTY. On every bypass/non-TTY
path it renders deterministic contextual help and exits 0; `pm reverse --help` always renders help.
Invalid reverse actions still return usage errors.

`pm reverse guide` is an explicit alias for the same workspace. Both entries walk the existing
gate — the gate itself is untouched:
