...
The `pm` binary and the website are independent products. This document describes only `pm` binary
release preparation; it does not change or authorize website release work.

This document authorizes no tag, no GitHub Release, no website release work, and no merge. It
prepares the binary release path only.
...
| Job | Trigger | Purpose |
| --- | --- | --- |
| `release-please` | push to `main` / manual workflow run | Opens or updates the release PR; on release PR merge, creates the tag and GitHub Release. |
...
There is intentionally no website release job in this binary release flow.
