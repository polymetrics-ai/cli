...
DuckDB is the common materialization and query engine. The durable warehouse is a
connection-owned JSONL WAL plus one derived Parquet table file per table. Each sync rebuilds that
file from the WAL and atomically replaces the prior file only after materialization succeeds.
...
6. Atomically replace the resulting final Parquet table file.
...
1. Pin an immutable warehouse workset and its materialized final table.
