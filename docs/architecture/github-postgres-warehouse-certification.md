...
DuckDB is the common materialization and query engine. The durable warehouse is a connection-owned JSONL WAL plus one derived Parquet file per table.
...
The warehouse owns the connection-scoped JSONL WAL, fsync before acknowledgement, DuckDB transformation and validation, single-file Parquet materialization and atomic replacement, row counts and content hashes, durable receipts, and readback for outbound plans.
...
6. Publish the resulting single-file Parquet table atomically.
...
1. Pin an immutable warehouse workset.
...
| --- | --- |
| `full_overwrite` | Build and validate a new single-file Parquet table in staging, then atomically replace the active table. |
| `full_append` | Combine the active table and complete new snapshot into a new single-file table, then replace it atomically. |
