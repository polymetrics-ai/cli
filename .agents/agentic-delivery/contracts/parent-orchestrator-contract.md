...
the single owner for shared parent artifacts, merge arbitration, local automated review coverage,
...
- parent PR is missing or lacks local review coverage
- sub-PR merge arbitration is needed
- local automated review coverage gap blocks integration
...
- launching or assigning local automated review disposition work
- recording local review coverage for integrated batches when needed
...
- `sub_pr_reviewed`: local automated review coverage exists for the sub-issue head, or an explicit
  blocker is recorded.
- `provisionally_integrated`: sub-PR merged into parent branch, but integrated-batch local review
  coverage is still pending.
- `parent_review_pending`: local review coverage is pending for an integrated batch.
- `parent_review_clean`: integrated batch has no unresolved actionable local review findings.
...
- local automated review findings on the sub-PR are resolved or dispositioned
- the diff is within the sub-issue scope
- no requested-changes human review is open
- no human gate is triggered

Remote PR-bot review is not required for non-`main` sub-PRs or parent-PR commit ranges. The
orchestrator records local review coverage for the sub-issue head or integrated batch
before marking that sub-issue review-complete.
...
- primary route: `local_reviewer`, `local_security`, `local_verifier`, `human`, or `blocked`
- coverage route: `sub_issue_head`, `parent_batch`, `human`, or `blocked`
- fallback route: `human` or `none`
- review status: `pending`, `clean`, `comments_addressed`, or `blocked`
- disposition summary artifact, PR body section, or comment
