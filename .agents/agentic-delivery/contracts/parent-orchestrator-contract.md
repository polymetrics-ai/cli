...
the single owner for shared parent artifacts, merge arbitration, CodeRabbit coverage routing, and
final human approval readiness.
...
- launching or assigning CodeRabbit disposition work
...
- CodeRabbit findings on the sub-PR are resolved, or the parent PR fallback path is recorded
...
- which checks ran
- which CodeRabbit route covered each sub-issue
