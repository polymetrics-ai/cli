...
10. If CodeRabbit skips the sub-PR because the base branch is not `main`, record that skip as a
    ...
    when the parent PR exists and the coordinator immediately requests or observes CodeRabbit review
    on the parent PR commit range that includes the sub-issue.
