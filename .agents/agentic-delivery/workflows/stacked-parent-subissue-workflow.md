...
Use `.agents/agentic-delivery/workflows/automated-review-routing-loop.md` before recording local
...
8. Run the local review loop and record a disposition for every actionable finding.
...
10. Merge the sub-PR into the parent branch without human approval only if every automated gate is
    green, local automated review coverage is recorded for the sub-issue head or integrated batch,
    and no human gate is triggered.
11. After merging a sub-PR into the parent branch, push the parent branch and update the parent
    PR's integrated-subissue list. Record integrated-batch local review coverage when needed using
    `workflows/automated-review-routing-loop.md`.
12. Comment on the sub-issue with the merged sub-PR, commit, verification, local review coverage
...
4. Record local review coverage for integrated sub-issue batches when needed.
5. Resolve or disposition all local review findings using the local review loop.
...
- local automated review has unresolved actionable findings
- required local review coverage is missing
