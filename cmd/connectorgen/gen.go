...
// prepareHookset validates each package-owned explicit hook factory and renders
// the closed sorted catalog used by production construction.
func prepareHookset(hooksRoot string) (string, error) {
...
func prepareManifestIndex(defsRoot, hooksRoot string) (string, error) {
	...
	projections, err := buildSourceVisibility(context.Background(), filepath.Dir(filepath.Dir(filepath.Dir(defsRoot))))
	if err != nil {
		return "", fmt.Errorf("generate source visibility: %w", err)
	}
	for i := range entries {
		if projection, ok := projections[entries[i].Connector]; ok {
			entries[i].SourceVisibility = projection
			delete(projections, entries[i].Connector)
		} else {
			entries[i].SourceVisibility = connectors.SourceVisibilityArtifact{SchemaVersion: 1, Connector: entries[i].Connector, Coverage: "not_in_cohort"}
		}
	}
	if len(projections) != 0 {
		return "", fmt.Errorf("source cohort connector absent from manifest inventory")
	}
...
func renderManifestIndexFile(entries []generatedManifestIndexEntry) (string, error) {
	...
	b.WriteString("// Code generated by connectorgen gen. DO NOT EDIT.\n")
	b.WriteString("//\n")
	b.WriteString("// Execution identity comes from rendered JSON; source visibility is separate authoring-derived diagnostic metadata.\n")
...
func runGenAt(_ []string, stdout, stderr io.Writer, hooksRoot string) int {
	...
	repo := filepath.Dir(filepath.Dir(connectorsRoot))
	capabilities, err := generateSourceCapabilities(context.Background(), repo)
	if err != nil {
		logln(stderr, "connectorgen gen:", err)
		return 1
	}
	hookSource, err := prepareHookset(hooksRoot)
	if err != nil {
		logln(stderr, "connectorgen gen:", err)
		return 1
	}
	indexSource, err := prepareManifestIndex(filepath.Join(connectorsRoot, "defs"), hooksRoot)
	if err != nil {
		logln(stderr, "connectorgen gen:", err)
		return 1
	}
	// All source, capability and execution metadata must validate before any
	// generated output is replaced. Invalid inputs preserve every prior output.
	for _, output := range []struct{ path, source string }{
		{filepath.Join(hooksRoot, "hookset", "hookset_gen.go"), hookSource},
		{filepath.Join(connectorsRoot, "manifestindex", "index_gen.go"), indexSource},
		{filepath.Join(connectorsRoot, "source_capabilities_gen.go"), capabilities},
	} {
		if err := writeGenFile(output.path, output.source); err != nil {
			logln(stderr, "connectorgen gen:", err)
			return 1
		}
	}
