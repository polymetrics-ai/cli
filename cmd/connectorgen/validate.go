...
func checkAPISurface(b engine.Bundle) []Finding {
	...
	if b.CLISurface != nil {
		for _, cmd := range b.CLISurface.Commands {
			// Operation-backed read-only commands consume an api_surface endpoint
			// through the same covered_by bookkeeping as a direct read, so they
			// satisfy that coverage too.
