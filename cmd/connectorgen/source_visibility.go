...
func projectSourceVisibility(manifest sourceLaneManifest, cohort sourceLaneCohort, cohortHash string, atlas sourceFoundationAtlas) (map[string]connectors.SourceVisibilityArtifact, error) {
	...
	for name, v := range catalogs {
		...
		// Zero time and empty optional fields are the gzip defaults; fix OS too
		// so generated bytes do not depend on the authoring host.
		writer.OS = 255
		...
		result[name] = connectors.SourceVisibilityArtifact{SchemaVersion: 1, Connector: name, Coverage: "in_cohort", CohortID: cohort.CohortID, Bytes: len(raw), SHA256: sourceBytesHash(raw), KeySHA256: v.KeySHA256, Encoding: "gzip", Payload: compressed.String()}
	}
	return result, nil
}
