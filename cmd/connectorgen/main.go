...
func usage() string {
  ...
  connectorgen ownership [repo-root] [--json] [--base <ref>] [--scope-file <path>]
	...
	connectorgen certification-matrix [repo-root] (--connector <name> | --all | --check)
	connectorgen certification-evidence (transport|transport-capability-write|change-capture) --connector <name> --report <path> --binary-sha <sha256> --from-env password=<ENV> --run-id <id> --record-prefix <id> [--repo-root <path>]
