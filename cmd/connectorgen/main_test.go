	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			op := engine.OperationSpec{
				...
				REST: &engine.RESTOperationSpec{CallerSuppliedIdentifierSets: []engine.CallerSuppliedIdentifierSetSpec{{
					Name: "ids", ElementShape: "opaque_string", Wire: tt.wire, MinItems: 1, MaxItems: 1,
				}}},
			}
			findings := checkCLISurfaceCallerSuppliedIdentifierSets(engine.Bundle{Name: "acme"}, 0, engine.CLICommand{Path: "identifiers lookup", Flags: []engine.CLIFlag{
				{Name: "ids", Type: "string_array", Required: true, MapsTo: "identifier_set.ids"},
				{Name: "shadow", Type: "string", MapsTo: tt.mapsTo},
			}}, op)
			for _, finding := range findings {
				if strings.Contains(finding.Message, "must not map to "+tt.mapsTo) {
					return
				}
			}
			t.Fatalf("findings = %+v, want %s shadow rejection", findings, tt.mapsTo)
		})
	}
}
...
func TestValidate_CallerSuppliedIdentifierSetRejectsSiblingGenericDirectRead(t *testing.T) {
	op := engine.OperationSpec{
		ID:   "acme.coins.lookup",
		Kind: "rest_read",
		REST: &engine.RESTOperationSpec{
	...
	bundle := engine.Bundle{
		...
		Surface: &engine.APISurface{Endpoints: []engine.SurfaceEndpoint{{
			Method: "GET", Path: "/coins", CoveredBy: &engine.SurfaceCoverage{DirectReads: []string{"coins lookup", "coins generic"}},
		}}},
		CLISurface: &engine.CLISurface{Commands: []engine.CLICommand{
			{
...
func TestValidate_CallerSuppliedIdentifierSetRejectsDynamicBaseGenericDirectRead(t *testing.T) {
	...
	bundle := engine.Bundle{
		Name:       "acme",
		HTTP:       engine.HTTPBase{URL: "https://{{ config.host }}/v1"},
		Operations: []engine.OperationSpec{op},
		CLISurface: &engine.CLISurface{Commands: []engine.CLICommand{
			{
				Path: "coins lookup", Intent: "direct_read", Availability: "implemented", Operation: op.ID, OutputPolicy: "json_redacted",
	...
	t.Fatalf("findings = %+v, want dynamic-base generic direct-read endpoint rejection", findings)
}
