# Typed body request placement233
...
2. Literal2/2/1 pages five identities, exact initial/next JSON bodies, direct one-send resume, saved placement using common helper.
3. Defaults/explicit size, required/type/bounds, conflicts, capsule definition/filter/body drift, malformed later cursor before send, negative oracle perturbations.
4. Focused native normal/race, old GET/Requester compatibility, loader admission, vet and scoped lint.

## Evidence table
| Criterion | Evidence | Observable assertion or fake reason |
| --- | --- | --- |
| Source schema admission | live | actual Load/Validate rejects unsupported contracts and admits valid body-only query-free case |
| Direct paging | fake | local HTTP exact bodies/IDs/send count; provider live not authorized and deterministic fault injection required |
| Saved paging | fake | local HTTP exact five IDs and body location; provider live not authorized |
