...
- [ ] G3 first proof candidates: `gitlab-primary-delete-api-v4-admin-ci-variables-key-{direct-write,reverse-etl}-g3-285` carry the portable current four-leaf C2 receipt and exact request/response target projections. A source-lane generation spelling RED/Green makes the generated `sha256:` identity representable without normalizing equality. The fresh report is valid but retains only the expected missing-independent-review deficits; no catalog is authored by this implementation task and neither lane is implemented/admitted.
