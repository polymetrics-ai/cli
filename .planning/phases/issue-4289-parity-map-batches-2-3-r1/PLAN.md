...
Required foundation change: permit a bounded, declaration-owned REST structured JSON body only when it is compiled and validated against that operation's exact `rest.body_schema`, retains declared redaction and size limits, and cannot choose a URL, method, operation, or arbitrary raw request. This is not a generic HTTP writer. Until a keyed foundation decision authorizes that shared change, affected operations remain `declaration-pending`; this phase will not mislabel them as engine incapability or fabricate a command.
