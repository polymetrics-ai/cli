...
**Red:** source and action evidence alone can report a reverse-ETL action as eligible while no installed CLI command selects it. The seven-surface assertion now derives `implemented` `reverse_etl` command-to-`write` coverage from every bundle's `cli_surface.json` and fails for every missing, duplicate, or orphan action. Current evidence finds 621 actions, with full existing command coverage only for Google Calendar and Amazon SQS; Gmail is missing three, Gong is missing twenty-four, and the other target write bundles have no corresponding installed action commands. These are declaration gaps to be authored only from the pinned operation schema, not a reason to weaken eligibility or invent parameters.
