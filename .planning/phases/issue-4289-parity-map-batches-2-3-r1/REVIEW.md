...
- The verifier proves 5,014 currently found source inventory rows have exactly one corrected Batch-1-shaped disposition and a bound API-surface endpoint. It also requires `counts.total`, per-kind counts, an old/new API-surface report, and a confidence basis for every lock.
