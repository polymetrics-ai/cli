...
The classification correction is deliberate: typed write actions are enabled `direct_write`; `reverse_etl` is represented only as nested and action-level eligibility metadata. All 621 existing schema-backed write actions are semantically eligible and individually representable as closed destination action shapes. Exact stream-to-required-input mappings have not been evidenced, persisted App/CLI dispatch does not select the already-composed destination (`internal/app/transport_dispatch.go:53-67`), and one descriptor can select only one named action per sync mode (`internal/connectors/sync_transport.go:388-415`). ETL remains a real endpoint class but is connector-local `declaration-pending` until a source `sync_transport.json` meets the PR #4286 contract. No source field, destination binding, selector, or action was invented.
