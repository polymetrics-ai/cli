...
Current main's generic `connectorgen validate` rejects the restored v2 lock's `source_operation` field. That shared source-projection compatibility gap is recorded in `VERIFICATION.md`; it is intentionally not repaired in this connector-local mapping slice.
