...
- RC-11 shared-admission fixture correction (Verify runs `32589551092` and `32592490944`, 2026-08-22): the 97-stage GitHub candidate fixture is a real release-gate failure under the aggregate `internal/cli` run. The final declaration witness establishes the reason: all seven GitHub `certification-*` rate-limit scopes declare `coordination=require_shared`, unchanged from `origin/main`. A fresh `pm connectors certify github` child correctly refuses `shared_coordinator_unavailable` before provider I/O if it inherits any unreachable runtime coordinator alias. Do not remove or weaken that declared policy, and do not claim a process-local fallback. Instead start a disposable local Redis-compatible coordinator in the fixture, inject only its address into the child under the primary configuration name, prove all 97 stages execute their declared request/output checks, and prove shared-admission state was written to that coordinator. The build-tagged coordination integration test remains the Dragonfly-backed cross-process proof; this is hermetic local behavioral evidence only.
- RC-12 exact-gate lint correction (current I5, 2026-08-23): `make lint` with the CI-pinned `golangci-lint v2.12.2` directly fails staticcheck S1008 at `cmd/connectorgen/sourceprojection.go:1971`: a boolean predicate is written as `if predicate { return true }; return false`. `git blame` identifies the PR's `f9f279cbd` projection repair as the source, while I5 itself did not touch the file. This exact gate failure is release-relevant; make only the equivalent direct return, then prove generation and real binary behavior remain unchanged. Do not alter the projection’s candidate/surface policy, counts, declarations, or artifacts.
- RC-13 generated-manual correction (current I6, 2026-08-23): the exact `make docs-check-no-build` gate directly reports `connector github manual is stale; run pm docs generate`. This is generated parity, not authority to alter the command surface. Run only the owning manual generator, inspect the artifact-only diff, rerun it for idempotence, then validate docs/help parity. Do not hand-edit a generated manual or change declaration/runtime behavior to satisfy it.
...
| RC-11 keeps the 97-stage fixture under its declared shared-admission policy | live | With either documented coordinator alias set to `127.0.0.1:1` in the parent, the fixture starts its own local Redis-compatible coordinator and injects that address into the fresh child. All 97 stages must execute authenticated declared direct-read request/output checks and leave shared-admission state in that coordinator. |
