...
| The native PostgreSQL connector still passes its database integration lane | live | The opt-in Docker/Colima test performs the connector's real database setup and asserts its declared native behavior against PostgreSQL; it is run with the supplied local endpoint. |
...
- The current embedded production definitions contain no `polling_watermark.json`. Surfaces therefore must say polling-watermark eligibility is unavailable/not declared for a connector binding, never display it as implemented merely because the shared executor exists.
...
- [ ] `pm connectors` exits successfully with contextual help.
- [ ] `pm help connectors` and `pm connectors --help` accurately describe polling truth.
- [ ] `pm connectors inspect <name> --json` and catalog output expose no unavailable mode as implemented.
- [ ] `docs/cli/**` and `website/**` cover the same bounded polling contract.
- [ ] Generated help/manual and website artifacts are regenerated from a freshly built binary.
- [ ] No generic shell, HTTP write, SQL write, credential, or reverse-ETL execution surface is added.
