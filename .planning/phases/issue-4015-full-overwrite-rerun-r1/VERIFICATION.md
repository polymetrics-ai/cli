...
- [ ] Red unit test proves full-refresh modes incorrectly receive prior checkpoints before the fix.
- [ ] Red live binary test reproduces run-two `0/0` and stale target state with exact counts.
- [ ] Green unit test proves `full_append` and `full_overwrite` omit prior checkpoints.
- [ ] Green unit test proves all four requested incremental modes retain prior checkpoints.
- [ ] Green live binary test independently proves target replacement after source deletion/update/insertion.
- [ ] Green live incremental test proves unchanged `incremental_upsert` still skips with `0/0`.
