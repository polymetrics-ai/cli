...
- Focused clean-branch verification passed: `git log --oneline origin/main..HEAD`, `git merge-base HEAD origin/main`, `git diff --name-status origin/main..HEAD`, `git diff --exit-code` for release/website/conventions/issue-guard paths, `git show --no-patch --format=%B 7d019a11`, and changed-doc `rg` checks for forbidden website-coupling markers.
