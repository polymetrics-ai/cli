...
## GitLab managed-destination single-attempt slice (2026-08-30)

- Issue/PR: `Refs #4283`; extend the existing Batch-1 branch and PR #4294 only.
- Base and delivery: start at remote `origin/fm/cli-top100-declaration-batch-r1` (`58686f557`), make one clean atomic commit in an isolated temporary worktree, and normal-fast-forward push only that existing remote ref. No new branch, issue, PR, merge, rebase, force-push, provider credentials, or provider-live I/O.
- Scope: a reusable, opt-in declaration-selected `single_attempt` destination adapter in the existing declarative transport foundation plus GitLab's exact source-backed adoption. It accepts the existing saved connection and plan → preview → approval lifecycle, makes one declared write per row, captures the bounded provider response, and checkpoint-acks only after that write succeeds. It does not introduce generic HTTP transport or connector-name dispatch.
- Non-goals and safety boundary: it does **not** claim provider idempotency, retry safety, automatic replay, read-back confirmation, or a generic writer. HTTP 4xx/5xx/timeout is terminal for the attempted workset: it has no checkpoint, retries are disabled, and the consumed authorization is revoked so a repeated saved run stops before source/provider I/O. A new plan and approval are required for another operator attempt. Existing `declarative_typed_destination` remains the separate `retry_safe` path with its keyed idempotency and provider read-back contract unchanged.
- Atlas classification: `transport.sync-contract.v1` and `runtime.provider-extension-seams.v1` are constrained extensions. The new adapter is selected only by an exact `sync_transport.json` executor and accepted evidence, and it has no connector-specific branch.
- Required guidance updated with the implementation: the shared `.agents/skills/connector-lane-build-order/SKILL.md` distinguishes explicit single-attempt delivery from retry-safe/read-back delivery. Per the 2026-08-30 scope guard, broad generated connector manuals and connector Skill artifacts are not part of this foundation slice.

| TDD stage | Evidence |
| --- | --- |
| Red | `TestGitLabManagedDestinationDuckDBLifecycle` stages one fake GitLab group, then proves the current branch refuses `destination_action` before all provider I/O because GitLab has no destination declaration. |
| Green | The exact declaration-selected adapter admits that saved stream and produces one declared `DELETE /groups/group-one` request after plan, preview, approval, and destructive confirmation; HTTP 202 returns provider response evidence and a non-empty checkpoint. |
| Failure proof | The same fake server returns 4xx, 5xx, or waits for cancellation. Each has exactly one attempted DELETE, returns an error, persists no checkpoint, and makes no automatic replay. |
| Regression/refactor | Existing typed-destination tests continue to require keyed idempotency and read-back. Connector validation and `surface-sync --check` prove the GitLab adoption remains source-bound and generated-surface clean. |

### Lifecycle record

`scripts/gsd doctor`, `scripts/gsd sources {discuss-phase,plan-phase,execute-phase,verify-work,code-review}`, and `go run ./cmd/agentcontractgen check` passed before this slice. The generated prompts were resolved with `scripts/gsd prompt` for all five stages. Manual inline execution is the documented fallback because this assigned integration lane may not spawn lifecycle roles; this plan and the focused Red/Green commands below are the GSD/TDD record.

### 2026-08-30 verification record

- **Red:** before the declaration and shared opt-in adapter, `TestGitLabManagedDestinationDuckDBLifecycle` refused the GitLab `destination_action` before provider I/O because GitLab was not a declaration-owned destination.
- **Green:** `go test -timeout 20m ./internal/cli -run '^(TestGitLabSourceTransportMaterializesConnectionOwnedDuckDB|TestGitLabSourceTransportFixtureStopsOnDeclaredNonSuccess|TestGitLabManagedDestinationDuckDBLifecycle)$' -count=1` passed. The fake server proves `DELETE /groups/group-one` exactly once after plan → preview → approval: HTTP 202 writes a checkpoint; 400, 500, and cancellation write no checkpoint and a subsequent saved run is refused by revoked authorization before any request.
- **Shared-contract proof:** `go test -timeout 20m ./internal/connectors/engine -run '^TestDeclarativeWriteRetryPolicy$' -count=1`, `go test -timeout 20m ./internal/connectors -run '^TestSourceTransportDescriptorRejectsSingleAttemptDelivery$' -count=1`, `go test -timeout 20m ./internal/synctransport -run '^(TestTransportPreflight_EnforcesDeliveryGuaranteeCompatibility|TestTransportPreflight_AdmitsSingleAttemptOnlyForFullAppend)$' -count=1`, and `go test -timeout 20m ./internal/app` passed.
- **Narrow generated CLI artifacts:** `go run ./cmd/pm docs generate --dir docs/cli --connectors-dir docs/connectors` and `go test -timeout 20m ./internal/cli -run '^TestGoldenDocsGenerateMatchesTrackedCLIManuals$' -count=1` passed. Only `docs/cli/connections.md`, `docs/cli/etl.md`, and the eight matching golden transcripts are retained.
- **Independent baseline gates (not normalized in this slice):** full `connectorgen validate internal/connectors/defs` fails because the base branch lacks canonical source descriptors for Bitbucket, CircleCI, Docker Hub, Jira, Notion, Sentry, Stripe, and Vercel. `surface-sync --check` additionally reports a pre-existing Asana source-projection update and then the same missing Bitbucket descriptor. Full transcript regeneration similarly contains unrelated root-manual drift. These are recorded, not folded into this GitLab-only change.
- **Required full gate:** `make verify` was run after formatting, focused vet, and build. Its repository-wide `go test -timeout 20m ./...` stopped before later Make targets on unrelated current-base failures: `cmd/connectorgen` GitLab source-lock count plus Asana source-projection assertions, `internal/connectors/conformance`, and `internal/connectors/defs` Asana embed inventory; `internal/cli` also reached its configured 20-minute timeout while retrying unavailable local Redis test endpoints. The focused GitLab lifecycle, engine, transport, app, and narrow CLI-doc gates above passed independently. No failure was addressed by changing an unrelated connector or generated connector documentation.
