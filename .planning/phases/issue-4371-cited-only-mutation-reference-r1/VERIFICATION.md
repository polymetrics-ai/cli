...
- [ ] CI and GitHub's configured automated reviewer run after normal branch
  publication.
- [x] Separate fresh-context Codex exact-head audit completed against
  `8ce9632aac9ebfccb7ce1db127e73b982bc4d8c6` (the full implementation,
  test-matrix, and verification head). It reported no High/Medium/Low finding
  and no merge blocker. Its focused source-projection/source-reference and
  commandrunner missing-foundation-before-dispatch checks passed; `git diff
  --check` passed and the worktree remained clean. The next commit records
  this audit only and changes no audited implementation/test/generated byte.
