# Google Ads documented-operation parity — plan

> **Generated mechanically** from `data/cli-top50-fixed-schema-sweep-r1/MASTER-PLAN.json`
> (`google-ads`, landing order 10). The operation count below was derived in the sweep-wide
...
- **Artifact**: `https://googleads.googleapis.com/$discovery/rest?version=v22`
- **Kind**: google_discovery, OAS `discovery (discoveryVersion=v1)`
- **Retrieved**: 2026-08-07, 2936865 bytes
- **Documented operations: 163**
- **By method**: DELETE 1, GET 11, POST 151
- **Read / write split**: 45 read, 118 write
...
**How it was counted.** version=v22 resolved live (HTTP 200, id='googleads:v22', revision=20260721, discoveryVersion='v1', 2,936,865 bytes) -- not stale, so no fallback version needed. Parsed with json.load, then walked resources RECURSIVELY: for each key under top-level 'resources', collected its 'methods' dict (1 entry = 1 operation) and then recursed into its own nested 'resources' dict at depth+1, repeating until no further nesting. Also checked for a top-level 'methods' key outside 'resources' (none present -- all methods live under resources.*). Max nesting depth observed = 2 (e.g. resources.customers.resources.adGroupAds.methods.mutate). Depth distribution: depth 1 = 39 methods, depth 2 = 124 methods. Total = 163, matching ledger_total exactly. Verified 163 unique (httpMethod, flatPath) pairs -- zero true collisions at the fully-expanded-path level.
...
- **Nested-resource hazard**: resources nest, so operations must be walked RECURSIVELY. A flat one-level walk undercounts.
...
write, or a binary transfer, **and** be individually reachable as its own `pm google-ads <command>`.
...
Parent **#3021** (new generation); children are expected at **#3022–#3028** (new-generation pattern) — **CONFIRM with `gh-axi` before using them in a PR body**, this is inferred from the pattern, not verified.
...
1. **RED** — write `cmd/connectorgen/google-ads_api_surface_test.go` **against this connector's real
   ...
   surface test (`ls cmd/connectorgen/ | grep '^google-ads_'`) — gong carried two, and a targeted
