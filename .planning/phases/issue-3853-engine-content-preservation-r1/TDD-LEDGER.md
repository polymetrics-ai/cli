...
| R5 | CLI/manual/golden/website wording describes the true connector-engine boundary | The tracked help/manual/golden/website wording asserted masking before the source change | `go test ./internal/cli -run '^(TestGoldenTranscripts|TestGoldenDocsGenerateMatchesTrackedCLIManuals)$' -count=1` passed after regeneration | Token omission and the separate source-table sample boundary remain explicit |
