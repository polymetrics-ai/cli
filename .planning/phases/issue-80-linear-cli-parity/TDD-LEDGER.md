...
| #100 | Operation ledger | `TestLinearOperationLedgerInventoriesGraphQLOperations` failed before ledger v1 was present/loadable; refreshed `TestLinearMutationOperationsModeledAsTypedWrites` failed while 317 mutation rows remained generic blocked rows | `api_surface.json` inventories 466 Linear GraphQL rows with 465 covered rows and only raw arbitrary GraphQL blocked | `go run ./cmd/connectorgen validate internal/connectors/defs --json` and `make verify` passed |
