...
## Accepted review correction at `748f6bdbf754840912796d8f7ff8aee9b8fbc00a`

All four findings in `/tmp/pm-397-review-436.log` are accepted. Terminal verification is reset before production edits. This bounded correction will:

1. Open and hold a project-rooted, RLM-specific warehouse effect scope before analyzer construction, pass that narrow scope through `rlm.RunRequest`, and use it for both input and atomic output effects. A deterministic temp test will replace the warehouse directory with an external symlink from a controlled analyzer-factory hook and prove no external read/write can occur.
2. Preserve base parser ownership for literal `--`, assigned/malformed unknowns, and help-like tail tokens without weakening intentional direct/trailing help. Differential cases include `extract -- --help --request=...` and malformed/assigned unknowns followed by positional `help`.
3. Distinguish the exact bare `extract` invocation from unknown-only/help-like invocations discarded by pflag, so only exact bare invocation renders help and unknown-only forms remain usage errors.
