...
Verification result: reset at correction start `748f6bdbf754840912796d8f7ff8aee9b8fbc00a`. All four findings from `/tmp/pm-397-review-436.log` are accepted. Planned correction: hold a narrow project-rooted RLM warehouse scope before analyzer construction and through read/write effects; preserve base parser ownership after literal `--` and malformed/help-like tails; keep unknown-only invocations as usage errors; reject the complete RLM bare-table contract before the factory. Deterministic RED tests and full rerun pending; prior completion evidence is historical only.
