...
- Current child: Refs #4424 — S1A API generic-engine migration
...
- Task: complete the captain-authorized S1A API cohort through schema-4 source locks, rendered execution JSON, and existing PM-CLI runtime only: preserve Amazon SQS as its bounded disposition, preserve DynamoDB/MySQL/PostgreSQL native behavior, remove every API legacy hook/native execution route, regenerate the shared surfaces, and publish one independently reviewed parent-branch checkpoint.
- Verification: record truthful RED/GREEN evidence before every family; run focused source-lock/production-registry/engine/CLI checks, complete render and generated-surface checks, residual and secret/local-state scans, and the fresh exact-SHA review procedure in `VERIFICATION.md` and `REVIEW-CONVERGENCE.md`.
