...
| --- | --- | --- | --- | --- |
| 1 | F-04-R, F-08-R | Bounded helper A→FIFO/symlink/directory A→B observation; directory-open pre-flock readiness must not count as contention; exact sent OS signal is distinct from any signaled wait status. | Same opened descriptor supplies identity/type/bytes or refuses before unrelated read; real EWOULDBLOCK acknowledgement precedes actual-main SIGINT/SIGTERM; direct waits, no-success/state and retry hold. | Planned before code |
