...
## Firstmate094 complete oracle migration and candidate-ready checks

Red: `oracle-red-receipt.json` is the original four-control exit1, wall9.645836875s, committed393c0e2a. CorrectN, correct intended payload and genuine movedA positives disconfirm a reject-everything checker. Complete audited091 ledger preceded these edits atede1950b.

Implementation: cloned pure fixture-derived pointers/controls; explicit ordered transitions; payload/shape checks before acquisition of stage/anchor/capture/record identities; complete expected connector tree; all cleanup, held-reader and three pending-prepared helper consumers migrated. Exact external early-stage A compared before restoration after both Recover and Check. The four actual caller verdicts reject coherentQ, well-formed wrong intended payload, changed-nonempty externalA and same-byte replacementA. No production edit.

Green chronology: initial complete closure normal51.207315292s passed. A stricter rejection-reason assertion failed once because Q correctly failed earlier at the prepared JOURNAL anchor, not at stage acquisition; receipt focused-normal-receipt.json preserves this assertion error, exit1wall50.330411083s. Corrected exact JOURNAL mismatch assertion then focused normal50.771331334s/race70.214342250s passed. Introduced lint then required only QF1003's equivalent tagged switch; final candidate-ready checks use *-ready receipts, not those earlier source hashes. Full final package execution is separate and pending until frozen commit.

Owner assertion inspection and all contract-to-evidence dimensions are in CP11-PROOF-CORRECTION-094.md, with current exact source bindings in its JSON companion. Mechanical symbol/assertion enumeration is navigation and does not replace that inspection or independent review.
