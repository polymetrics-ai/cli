...
| Nullable object-wrapper order | Red: `TestSourceProjectionClosedCompositionPreservesNullableObjectWrapperRegardlessOfTypeOrder` failed because a `type: ["null", "object"]` composition wrapper accepted `{}` after projection. | Green: the same focused test proves the object wrapper's closed properties and required field survive regardless of source type order. | Converted every declared type kind instead of selecting only the first; composition-object wrappers now require an explicit closed boundary. | green |
