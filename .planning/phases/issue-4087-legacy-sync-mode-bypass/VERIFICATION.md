...
- [x] A nonempty runtime incremental cursor must match its schema cursor; Chargebee `coupons` now declares `updated_at` in both places and again projects all five canonical public modes, while valid empty-cursor incremental declarations remain supported.
