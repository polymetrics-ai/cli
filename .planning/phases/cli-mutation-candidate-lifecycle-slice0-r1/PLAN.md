...
6. **Generated artifacts and accounting.** Regenerate candidate and certification sweep surfaces, run the generator twice, and record byte stability. Show `279 + 577 = 856` and the complete sweep total of `1,571` in the PR body.
7. **Verification and review.** Run the targeted tests, their consumers, no-I/O generator checks, binary build/help checks, required individual `make verify` gates, full `make verify` if environment time permits, inline security/code review, then write verification and review evidence.
