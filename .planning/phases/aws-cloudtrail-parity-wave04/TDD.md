...
- Connector-local contract tests prove:
  ...
  - 8 stream-backed read operations are implemented; 11 parameterized read operations, 10 direct/provider-query operations, and 31 write/admin actions are blocked/planned.
...
- Scope correction reverted the shared interface forwarding/validator/cache edits and reclassified the exposed CloudTrail surface to 8 implemented streams plus 52 blocked/planned operations.
- Focused green tests:
...
- The operation ledger and generated docs/catalog/help show 8 implemented streams, 0 executable direct reads, 0 executable writes, and 52 blocked/planned operations.
...
- `go run ./cmd/pm connectors catalog --json` reports AWS CloudTrail as read-only with 8 streams and 0 write actions.
