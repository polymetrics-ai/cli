...
- Scope-corrected implemented classification: 8 ETL/read streams, 0 bounded direct/provider query commands, 0 typed reverse-ETL write/admin actions, 0 binary operations, 0 CDC operations, 52 blocked/planned operations.
- Blocked/planned breakdown: 10 provider query/direct-read actions, 11 parameterized read actions, and 31 write/admin actions require shared promoted-native command-surface, manifest, write-validation, dry-run, and operation-direct-read forwarding that was reverted from this connector-local branch.
