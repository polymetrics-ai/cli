...
The initial commit verified a 60/19/10/31 implementation with shared promoted-native support. That shared support was later ruled out of scope by the user, so the current corrective head narrows executable reads further to 60/8/0/0 with 52 blocked/planned operations.
...
- 8 implemented ETL/read streams.
- 0 implemented direct/provider query commands.
- 0 implemented reverse-ETL write/admin actions.
- 52 blocked/planned operations: 11 parameterized read + 10 direct/provider query + 31 write/admin.
...
- [x] `go run ./cmd/pm connectors catalog --json` -> AWS CloudTrail reports read-only, 8 streams, 0 write actions.
...
- [x] `cd website && node scripts/gen-connector-bundles.mjs && node scripts/gen-connector-catalog.mjs` -> AWS CloudTrail website data reports 8 streams, 0 write actions, `cli_surface: null`.
