# Trello documented-operation parity — plan

> **Generated mechanically** from `data/cli-top50-fixed-schema-sweep-r1/MASTER-PLAN.json`
> (`trello`, landing order 16). The operation count below was derived in the sweep-wide
...
- **Artifact**: `https://developer.atlassian.com/cloud/trello/swagger.v3.json`
- **Kind**: openapi, OAS `3.0.0`
- **Retrieved**: 2026-08-07, 261671 bytes
- **Documented operations: 261**
- **By method**: DELETE 37, GET 128, POST 45, PUT 51
- **Read / write split**: 128 read, 133 write
...
**How it was counted.** Loaded the (single-line, minified) JSON with json.load, walked `paths`, counted one operation per (path, method) pair for real HTTP methods only. Reviewed all 45 POST paths by name, including /members/{id}/savedSearches (creates a saved-search resource, does not execute a search) -- none match the search/query/batchGet read pattern, so all POSTs are writes; all 128 GETs are reads. Cross-checked with a duplicate-key-detecting JSON loader (0 duplicate keys) and an independent recount; both reproduced 261/128/51/45/37 exactly.
...
- **Webhook management endpoints: 10** — these stay **in scope** and are counted in the
...
write, or a binary transfer, **and** be individually reachable as its own `pm trello <command>`.
...
Parent **#3086** (new generation); children are expected at **#3087–#3093** (new-generation pattern) — **CONFIRM with `gh-axi` before using them in a PR body**, this is inferred from the pattern, not verified.
...
1. **RED** — write `cmd/connectorgen/trello_api_surface_test.go` **against this connector's real
   ...
   surface test (`ls cmd/connectorgen/ | grep '^trello_'`) — gong carried two, and a targeted
