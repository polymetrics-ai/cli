...
| Source declaration | Batch-1 `api_surface.json` plus retained source lock | Canonical provider method/path stays visible and has per-endpoint provenance. The foundation will add a source-cited per-binding identity record; no provider route is rewritten. |
