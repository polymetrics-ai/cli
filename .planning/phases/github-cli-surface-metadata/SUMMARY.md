...
- Fixed subagent review findings by embedding `cli_surface.json` in `defs.FS`, blocking unsafe
  ...
  command map.
- Fixed Claude review finding on PR #48 by ensuring CLI-surface API endpoint references are
  validated whenever `api_surface.json` is present, including an intentionally empty endpoint list.
- Hardened the agentic delivery system so stacked sub-PRs cannot count skipped Claude status as
