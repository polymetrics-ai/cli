...
- Base branch: main
- Merges into: main
- Delivery: PR #4296 remains open against `main` (API verified). `cli-rendered-reference-citation-contract-r1` is merged through `origin/main` `3c66c33b8`; the batch remains blocked only because `operation-evidence` cannot yet project that valid schema-v3 document-owned inventory. No red-gate branch push is authorized.
