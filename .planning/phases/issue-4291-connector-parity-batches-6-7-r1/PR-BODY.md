...
- Adds public-documentation source locks and complete six-class declaration-disposition ledgers for batches 6 and 7 (20 canonical connector IDs).
- Makes enabled status reachable: an endpoint is enabled only with a bound API-surface direct-read command or typed write action. The reconciled total is **2,099 documented / 575 enabled / 348 commands / 448 writes / 211 deletes**.
...
- TDD evidence: source-lock/ledger absence checks supplied the red state; the strict complete-map invariant check supplied green state.
