...
- **RED — Square static extraction:** after 40/40 group pages were fetched, seven yielded zero static operation cards, although browser-rendered Locations proves such a page has four operations. The crawl correctly downgraded to `partial_incomplete_extraction` at 40 fetched / 33 extracted, cleared `operations_found`, and refused to generate a source lock. The 255-row preliminary map was discarded; rendered-browser extraction is the required resume path.
