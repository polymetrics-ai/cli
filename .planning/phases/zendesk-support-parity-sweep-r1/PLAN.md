# Zendesk Support documented-operation parity — plan

> **Generated mechanically** from `data/cli-top50-fixed-schema-sweep-r1/MASTER-PLAN.json`
> (`zendesk-support`, landing order 26). The operation count below was derived in the sweep-wide
...
- **Artifact**: `https://developer.zendesk.com/zendesk/oas.yaml`
- **Kind**: openapi, OAS `3.0.3`
- **Retrieved**: 2026-08-07, 1701930 bytes
- **Documented operations: 625**
- **By method**: DELETE 86, GET 325, HEAD 0, OPTIONS 0, PATCH 14, POST 111, PUT 89, TRACE 0
- **Read / write split**: 333 read, 292 write
...
**How it was counted.** Fetched with curl, HTTP 200 on first attempt (1,701,930 bytes). PyYAML's SafeLoader could NOT parse the file as-is: it contains bare `=` scalar values (e.g. `change: =` inside an example block around line 36047), which YAML 1.1 resolves to the special tag `tag:yaml.org,2002:value` that SafeLoader has no registered constructor for, raising a ConstructorError. Registered a constructor that treats that tag as a plain string; the full document then parsed cleanly. Counted one operation per (METHOD, path) pair under top-level `paths`, same method/read-write rules as the other two connectors in this batch. Manual description review corrected 3 keyword false positives where the matched word was a stored-resource NAME, not a read action: POST /api/v2/saved_searches [CreateSavedSearch] matched 'search' via 'saved_searches'; POST /api/v2/task_list_templates [CreateTaskListTemplate] and POST /api/v2/tickets/{ticket_id}/task_lists [CreateTaskList] both matched 'list' via 'task_list(s)'. All three literally 'Creates a ...' per their description -> moved to write.
...
write, or a binary transfer, **and** be individually reachable as its own `pm zendesk-support <command>`.
...
Parent **#156** (old generation); children **#157–#162** (verified)
...
1. **RED** — write `cmd/connectorgen/zendesk-support_api_surface_test.go` **against this connector's real
   ...
   surface test (`ls cmd/connectorgen/ | grep '^zendesk-support_'`) — gong carried two, and a targeted
