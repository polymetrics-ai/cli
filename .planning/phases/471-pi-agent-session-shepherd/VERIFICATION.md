...
| Parent live preflight | correction active | Unprotected/null-app policy fix is green at `95a4bd697` (3/3 selected, 660/660 affected, live policy read). Read-only canary generation 1 then exposed concurrent embedded OAuth runtime construction; one bounded single-flight correction is planned before dispatch. |
| #480 recovery/cutover preparation | worker-ready but dispatch deferred | Dependency #479 is integrated. Dispatch remains deferred only until the parent-owned embedded-session preflight is green; then a persistent Shepherd worktree must capture RED before recovery/audit/cutover production edits. |
| #481 CLI Architecture canary | dependency-blocked; live reconciliation attempted | Requires #480. Generation 1 reconciled exact #438 head read-only: scout evidence succeeded, validator hit the parent OAuth preflight defect, and #438 stayed draft/unmerged/unchanged. Deprecation remains inactive. |
