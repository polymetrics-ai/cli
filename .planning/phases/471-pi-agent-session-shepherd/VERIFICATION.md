...
| Parent live preflight | green; commit/publish pending | Unprotected/null-app policy fix is green at `95a4bd697`. Shared embedded runtime correction passes 5/5 focused, 168/168 affected, strict Pi typecheck, family/RPC checks, and live canary generation 2 with both model lanes; no credential value was accessed or transferred. |
| #480 recovery/cutover preparation | worker-ready | Dependency #479 and parent live preflight are green. A persistent Shepherd worktree must capture RED before recovery/audit/cutover production edits. |
| #481 CLI Architecture canary | dependency-blocked; exact live target reconciled | Requires #480. Generation 2 completed both read-only model lanes and failed closed only because unchanged external #438 remains draft and GitHub `DIRTY`; #438 stayed unmerged/unchanged. This is no passing deprecation receipt. |
