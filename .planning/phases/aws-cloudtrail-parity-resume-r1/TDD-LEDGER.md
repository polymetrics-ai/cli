...
| Registry wrapper forwarding | `definitionConnector` erases the native optional interfaces, so the `aws-cloudtrail` factory cannot satisfy `CommandSurfaceProvider` or `OperationDirectReader`. | Every enumerated runtime interface (`DirectReader`, `OperationDirectReader`, `OperationBinaryDownloader`, `WriteValidator`, `DryRunWriter`, `Querier`, `CDCReader`, `StatefulReader`, `SchemaMapper`, `LiveConformanceProvider`, `LocalWarehouseMaterializer`) delegates through the generic wrapper; absent capabilities fail with `ErrUnsupportedOperation` rather than disappearing silently. | Captain-authorized exception: `internal/connectors/native/nativeset/promoted.go` only; each interface gets an individual red/green subtest. |
