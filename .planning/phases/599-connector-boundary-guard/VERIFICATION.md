...
| Extracted `conventions.yml` branch-name run block | Blocked | Current source rejects legacy branch `fm/cli-connector-boundary-guard-r1`; supported migration to a conventional `<type>/<description>` branch is required before reporting this gate green. |
