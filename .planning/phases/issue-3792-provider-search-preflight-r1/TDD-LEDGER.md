...
| --- | --- | --- | --- | --- |
| R1 | An implemented operation-backed direct-read command cannot preflight through a generic reader alone. | Pending: focused commandrunner test must fail before the preflight interface is consumed. | Pending. | Must prove `OperationDirectRead` is not dispatched after rejected preflight. |
