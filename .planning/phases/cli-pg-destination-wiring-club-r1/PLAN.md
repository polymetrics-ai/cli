...
5. **Red/green — live production composition.** Through App and a built `pm`, run real PostgreSQL source → connection warehouse → real PostgreSQL target. Assert rows, types, private owner/control/ledger state, warehouse WAL/Parquet/manifests, baselines, receipts, checkpoints, LSNs, and slot state. Separately run the definition-selected authenticated GitHub issue source → the same warehouse → real PostgreSQL target and assert the exact API row plus target receipt/checkpoint; never substitute public access or a fake server for that proof.
