...
| --- | --- | --- | --- | --- |
| R1 | GitHub source eligibility is an explicit executable-stream allowlist | Production-composed `commits` preflight returns the current unsupported-stream error and the declaration contains only `issues`. | GitHub declaration names every executable bundle stream and no wildcard; production composition accepts `commits`. | planned |
| R2 | The allowlist still refuses absence before effects | No typed stream-ineligibility result exists and production-composition tests do not prove zero effects. | `errors.As` matches `SourceStreamIneligibleError`; source requests, stages, target sends/rows, and checkpoint updates all remain zero. | planned |
...
| R5 | PostgreSQL polling is definition-selected in production | `polling_watermark.json` is `planned` and no PostgreSQL factory reaches `PollingSourceExecutor`. | Loaded PostgreSQL definition, `app.Open` factory composition, and native adapter resolve exact polling references/evidence and invoke the shared executor. | planned |
