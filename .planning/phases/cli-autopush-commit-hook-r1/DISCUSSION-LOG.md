...
| Operation cleanup | Record a commit-scoped Git-resolved operation marker in `prepare-commit-msg` and consume it from `post-commit` before scheduling. | Review follow-up |
...
| Testing | Use a POSIX-shell harness with temporary local bare remotes. It must run a real two-commit rebase, clean, manually completed, and amended operation states, delayed children during and after an operation, stale and pushurl-specific remote defaults, configured push-target precedence, concurrent linked-worktree rate-ref compare-and-swap, and a real non-fast-forward rejection. | Task brief + review follow-up |
