...
| Safety boundary | Skip all in-progress Git operations, detached HEAD, `main`, and a locally known remote default branch; the detached child re-resolves operation paths, resolves every effective push endpoint's live default, and only sends its scheduling OID if the branch still names it; never force-push. | Task brief + review follow-up |
...
| Testing | Use a POSIX-shell harness with temporary local bare remotes. It must run a real two-commit rebase, manually completed operation states, delayed children during and after an operation, stale and pushurl-specific remote defaults, a concurrent linked worktree lease, and a real non-fast-forward rejection. | Task brief + review follow-up |
