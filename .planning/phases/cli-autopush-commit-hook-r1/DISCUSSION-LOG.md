...
| Testing | Use a POSIX-shell harness with temporary local bare remotes. It must run a real two-commit rebase, clean and manually completed operation states, delayed children during and after an operation, stale and pushurl-specific remote defaults, concurrent linked-worktree rate-ref compare-and-swap, and a real non-fast-forward rejection. | Task brief + review follow-up |
