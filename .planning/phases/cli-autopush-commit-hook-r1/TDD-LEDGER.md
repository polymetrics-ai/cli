...
| R11 | Amended operation refusal | An amended squash or clean no-commit completion replaces the old tip, so the old tip is not the new commit's parent. | `post-commit` consumes the prepared operation marker independent of commit topology; amended squash, cherry-pick, and revert cases record no push. |
