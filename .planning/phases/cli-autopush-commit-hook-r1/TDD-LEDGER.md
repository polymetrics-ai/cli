...
| R8 | Delayed operation refusal | A delayed child can push while a later Git operation is active or can push a merge, cherry-pick, revert, or rebase tip that replaced the commit which scheduled it. | The child re-resolves Git operation paths at its send boundary, captures the scheduling HEAD, requires the branch still equal that OID, and pushes the captured OID rather than a live branch ref. |
