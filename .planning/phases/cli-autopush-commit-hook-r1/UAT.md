...
| --- | --- | --- |
| Refusal and replay safety | `scripts/tests/post-commit-autopush.sh` runs a real two-commit rebase, manual, clean, amended, and interleaved ordinary/squash completion, delayed children during and after a manual merge, stale/default-pushurl, detached, and opt-out cases. | Pass |
| Push destination | The same harness proves branch-specific, global, and tracking remote precedence with isolated local bare remotes. | Pass |
