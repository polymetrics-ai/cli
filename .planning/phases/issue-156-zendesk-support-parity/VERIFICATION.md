...
- [x] Delete/destructive safety asserted: 86 official DELETE operations = 9 covered delete writes with `confirm: "destructive"` + 77 blocked typed operation rows; 94 blocked destructive-action rows include those 77 DELETE rows plus 17 non-DELETE destructive mutations; 22 sensitive/secret mutation rows also require typed destructive confirmation. `operations.json` has 116 blocked destructive/sensitive typed approvals, and `cli_surface.json` has 125 destructive-confirmation command approvals including the 9 covered delete write-plan commands.
