...
- Added 34 fixed direct-read operations, 104 guarded write actions, and 104 sanitized write fixtures.
- Replaced coarse legacy `api_surface.json` with v22 operation-ledger rows: 164 local rows (3 streams, 34 direct reads, 104 writes, 23 blocked/planned). The local row count is one greater than raw discovery because one official `customers.googleAds.search` method backs two fixed connector streams.
