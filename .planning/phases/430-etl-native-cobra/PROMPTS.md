...
Verification result: initial focused RED failed before production edits at `internal/cli/etl_cli_test.go:22:9: undefined: newETLCobraCommand`, as required. Focused GREEN passed in `13.396s`, and the broader ETL/router focused suite passed in `27.999s`. Local review then added a test-first invalid-action trailing-help case; it failed because Cobra rendered `CommandManual` and exited 0. The correction now bounds invalid actions before help parsing; focused/repeated/race and router/golden/help preservation pass. `RUN-STATE.json` remains non-terminal with `verificationPassed=false` until complete final gates pass.
