...
| Captain parity bar | `gated` conveyed generator/preflight admission but no per-connector documented-operation percentage, and 282 non-command provider operations plus 12 unmaterialized source-ledger rows had no fixed-vocabulary recovery record. | Schema-v2 progress ledger gives every connector a `coverage_percentage` (number or explicit `null` while unmeasured); the 20 batch candidates measure 99 delivered of 780 documented operations (12.69%), with 681 operations in the exhaustive rejection list. None is marked as meeting the >90% bar. | completed |
...
For the captain parity-bar gap, `scripts/gsd prompt plan-phase 4273 --gaps --tdd` was resolved and its role-only steps were performed inline because the canonical contract prohibits role spawning. RED is the prior ledger's absence of coverage/rejection fields; GREEN is the checked schema-v2 ledger/rejection invariant. No new scraper, pipeline, runtime, or shared schema was introduced.
