...
| `write` and `cdc` publication is truthful | live | `write` has a capability-scoped aggregate record from the complete six-mode profile; its twelve mode records remain on sync-mode cells. `cdc` has its own receipt-backed capability record. `query` stays false and the legacy unsupported direct `Write` entry point is never rebranded as a generic writer. |
...
5. The original mismatch was publication bookkeeping: `metadata.json` and native tests published `cdc=true, write=false, query=false`; the matrix initially tried to attach six mode records to the broad write cell. The final projection instead uses a separately emitted aggregate `capability:write` record, retains the twelve records at their exact sync-mode scope, and publishes `write=true` only for the definition-owned warehouse transport, never for the direct stub. The global matrix correctly remains incomplete because unrelated cells lack fixture/live proof; that aggregate is not a failed live transport certificate.
