...
- Follow-up review probe showed `pm gong --bogus` also rendered help successfully. Red captured; unknown flags now return usage errors. Subsequent reviews identified cross-connector shared flags, false-y preview semantics, and empty lifecycle values. Shared passive flags such as GitHub `--credential` and `--preview=false` render help, while `plan`, `approve`, and `confirm` are rejected by presence (including `--plan=`) and truthy `preview` is rejected as an active lifecycle request.
