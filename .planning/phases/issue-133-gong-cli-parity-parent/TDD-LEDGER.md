...
- Follow-up review probe showed `pm gong --bogus` also rendered help successfully. Red captured; unknown flags now return usage errors. Subsequent reviews identified cross-connector shared flags, false-y preview semantics, and empty lifecycle values. Shared passive flags such as GitHub `--credential` and `--preview=false` render help. Empty `plan`, `approve`, and `confirm` values now fail for both pathless and pathful invocations; focused pathful reds are green and prevent malformed write commands from silently creating a new plan or ignoring approval/confirmation.
