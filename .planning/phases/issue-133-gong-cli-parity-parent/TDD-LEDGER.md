...
- Follow-up review probe showed `pm gong --bogus` also rendered help successfully. Red captured; unknown flags now return usage errors. Subsequent reviews identified cross-connector shared flags, false-y preview semantics, and missing lifecycle values. Shared passive flags such as GitHub `--credential` and `--preview=false` render help. Focused reds are green: `plan`, `approve`, and `confirm` without values fail for both pathless and pathful invocations, covering explicit empties (`--plan=`), bare-flag `"true"` sentinels (`--plan`), and malformed repeated forms where a later value would otherwise mask an earlier missing value.
