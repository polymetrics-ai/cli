...
- Follow-up review probe showed `pm gong --bogus` also rendered help successfully. Red captured; unknown flags now return usage errors. Subsequent reviews identified two cross-connector semantics: shared passive flags such as GitHub `--credential` must work even if surface metadata omits them, and false-y `--preview=false` is not an action. Both focused reds are green: help detection uses the union of declared and shared flags and rejects only active lifecycle values (`plan`, `approve`, `confirm`, truthy `preview`).
