# Twilio documented-operation parity — plan

> **Generated mechanically** from `data/cli-top50-fixed-schema-sweep-r1/MASTER-PLAN.json`
> (`twilio`, landing order 12). The operation count below was derived in the sweep-wide
...
- **Artifact**: `https://raw.githubusercontent.com/twilio/twilio-oai/main/spec/json/twilio_api_v2010.json`
- **Kind**: openapi, OAS `3.0.1`
- **Retrieved**: 2026-08-07, 1869905 bytes
- **Documented operations: 197**
- **By method**: DELETE 32, GET 103, POST 62
- **Read / write split**: 103 read, 94 write
...
**How it was counted.** Loaded the JSON with json.load, walked the top-level `paths` object, and counted one operation per unique (path, HTTP method) pair where method is one of get/put/post/delete/patch/head/options/trace. All 62 POST paths were reviewed by name and none are search/query/list/batchGet-shaped (Twilio uses POST for both create and update, never PUT/PATCH) so all POSTs are writes; all 103 GETs are reads. Independently re-verified with a duplicate-key-detecting JSON loader (0 duplicate keys found) and by recomputing by_method from scratch; both passes reproduced 197/103/62/32 exactly.
...
write, or a binary transfer, **and** be individually reachable as its own `pm twilio <command>`.
...
Parent **#3167** (new generation); children are expected at **#3168–#3174** (new-generation pattern) — **CONFIRM with `gh-axi` before using them in a PR body**, this is inferred from the pattern, not verified.
...
1. **RED** — write `cmd/connectorgen/twilio_api_surface_test.go` **against this connector's real
   ...
   surface test (`ls cmd/connectorgen/ | grep '^twilio_'`) — gong carried two, and a targeted
