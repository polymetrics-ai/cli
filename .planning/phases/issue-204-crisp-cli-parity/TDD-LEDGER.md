...
| Crisp metadata/ledger scaffold | #205 | `go run ./cmd/connectorgen validate internal/connectors/defs/crisp` failed before files existed (`read root: open .`); post-scaffold `go test ./...` exposed fixed catalog-count expectations | Temp-root single-connector validate passed; `go run ./cmd/connectorgen validate internal/connectors/defs` passed with 548 connectors; conformance/inspect/docs smokes passed; `make verify` passed | Removed executable API refs from planned CLI commands; updated catalog counts/generated docs for the new non-dispatching bundle | full local green |
