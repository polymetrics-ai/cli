...
- Represented official Shopify Admin API surface from public docs as an operation ledger: 287 GraphQL queries, 518 GraphQL mutations, 317 REST non-delete rows, and 44 REST DELETE rows. Non-executable rows are blocked/planned with source evidence rather than exposed as generic passthrough.
- Covered 43 executable connector rows: fixture-backed `shop` read plus 42 typed destructive delete actions, all with `confirm: "destructive"` and no request body. The two documented REST DELETE shapes that require query identifiers (`inventory_levels?inventory_item_id&location_id`, `themes/{theme_id}/assets?asset[key]`) remain blocked/planned with source evidence rather than exposed unsafely.
