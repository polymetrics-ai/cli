# Segment documented-operation parity — plan

> **Generated mechanically** from `data/cli-top50-fixed-schema-sweep-r1/MASTER-PLAN.json`
> (`segment`, landing order 11). The operation count below was derived in the sweep-wide
...
- **Artifact**: `https://docs.segmentapis.com/redocly-state-0c69df4f659ddc0a96c4999793bd40dd5566a088.js`
- **Kind**: openapi, OAS `3.0.3`
- **Retrieved**: 2026-08-07, 2355809 bytes
- **Documented operations: 197**
- **By method**: DELETE 27, GET 97, PATCH 23, POST 43, PUT 7
- **Read / write split**: 98 read, 99 write
...
**How it was counted.** docs.segmentapis.com has no directly-fetchable spec file (probed openapi.json, openapi.yaml, swagger.json, api-description.{json,yaml}, spec.{json,yaml}, docs.{json,yaml} -- all 404). The page is rendered by Redocly reference-docs tooling; the full OpenAPI 3.0.3 document is embedded as an escaped JSON string inside `const __redoc_state = JSON.parse("...")` in https://docs.segmentapis.com/redocly-state-0c69df4f659ddc0a96c4999793bd40dd5566a088.js (HTTP 200, 2355809 bytes decompressed via curl --compressed). Extracted with python3: regex-matched the JSON.parse("...") string literal, json.loads() it once to undo JS string-escaping, then json.loads() again on the resulting text to get the real spec object at definition.data (openapi=3.0.3, info.version=73.1.0, title='Segment Public API', 125 paths). This extracted document (2249002 bytes) was then counted the same way as a normal OpenAPI file: every paths.<path>.<method> entry for method in get/put/post/delete/patch/head/options/trace = 197 operations, matching ledger_total exactly.
...
write, or a binary transfer, **and** be individually reachable as its own `pm segment <command>`.
...
Parent **#3150** (new generation); children are expected at **#3151–#3157** (new-generation pattern) — **CONFIRM with `gh-axi` before using them in a PR body**, this is inferred from the pattern, not verified.
...
1. **RED** — write `cmd/connectorgen/segment_api_surface_test.go` **against this connector's real
   ...
   surface test (`ls cmd/connectorgen/ | grep '^segment_'`) — gong carried two, and a targeted
