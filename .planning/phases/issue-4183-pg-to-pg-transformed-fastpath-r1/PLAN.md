...
### Lint follow-up: Arrow v18.7 API migration

The Arrow v18.7.0 security upgrade deprecates `arrow.Record` and `RecordBuilder.NewRecord`; `make verify` therefore fails in `make lint` with staticcheck `SA1019`, even though its test log contains no `--- FAIL:` records. Replace the types and constructors with `arrow.RecordBatch` and `NewRecordBatch` only. A direct scoped scan finds the six reported production/test occurrences plus two identical zero-row/null test constructors; migrate all eight so lint cannot reveal the same deprecated API on a later run. This is an API spelling/lifetime-preserving change, not a behavioural rewrite: no suppression, dependency downgrade, benchmark edit, or test weakening. Before push, run both `make lint` (the CI entry point) and `make test`; record their exact successful results in the status file and PR evidence.
