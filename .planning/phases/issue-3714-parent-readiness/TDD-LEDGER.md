...
| R4 | Parent head continues to contain the current `origin/main` after it advances | At `554235545`, `git merge-base --is-ancestor origin/main HEAD` returned nonzero for `4871df2f8` | Pending the normal merge of `4871df2f8` into this same branch | Re-run generator synchronization, its drift check, focused Recurly/engine tests, and the fresh no-mistakes parent validation run |
