...
- An initial broad parser also saw 34 Workday REST summaries ending in sentence punctuation. Their literal text does not match a declared `api_surface.json` endpoint, so they are not safely recoverable addresses. The final generic rule joins a parsed summary to the bundle's canonical endpoint inventory and does not guess or normalize punctuation.
