...
**Status:** frozen design; RED test-only checkpoint in progress. Base admission
is squash-aware content identity at
...
- `TestOpenInstallsGitHubWarehouseMediatedTransport` must prove production
  construction, exact GitHub descriptor/registration preflight, and a non-nil
  reopenable stage without provider I/O.
- `TestGitHubWarehouseStageReopensDurableReceiptAfterSourceReferencesAreDiscarded`
  must persist one record, erase/mutate every source-owned reference, and prove
  a second reopen returns the original durable Parquet value.
- `TestGitHubWarehouseStageRejectsTamperedReceipt` must independently reject
  owner, generation, manifest, and content mutations before emitting records.
- `TestOrchestratorReadsBackDestinationReceiptBeforeCheckpointCAS` must pin the
  exact ordering `stage, reopen, apply durable receipt, independent read-back,
  CAS`, and refuse CAS when read-back fails.
