...
5. **Accepted and fixed — leaf help reached `withApp` before its local guard.** The old `pm etl run --help` and `pm connections create --help` guards sat inside handlers whose Cobra wrappers opened the project first. `legacyLeafManualTopic` now recognizes only those valid static-manual leaves before `withApp`; the test changes into an empty directory so a repository-local project can never mask this regression. It retains approval-carrier validation before dispatch and does not turn unknown actions into help.

## Final assessment

No unresolved correctness, secret-handling, CLI-discovery, connector-boundary, ordering, checkpoint, or unbounded-retention issue remained after the fixes above. Focused race, package, production-binary, generated-artifact, and repository checks are recorded in `VERIFICATION.md`; the unmeasured 5 GB proof remains explicitly pending by captain decision.
