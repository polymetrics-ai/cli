...
5. **R5 — proof and review.** The captain has made the opt-in 5 GB binary proof explicitly pending rather than a merge gate; retain the prior result unchanged and make no throughput claim until a quiet-host proof can be recorded. Run `verify-work`, review the changed cross-package paths inline, and record findings/dispositions.
6. **R6 — PR #4190 CLI-help gap.** Make `pm etl run --help` and `pm connections create --help` dispatch their existing contextual manuals before the Cobra legacy handler opens the project. Red tests run those exact calls from a fresh directory containing no `.polymetrics`; green requires exit zero, expected flag text, and no stderr. Preserve approval-carrier validation and invalid-action usage errors.
