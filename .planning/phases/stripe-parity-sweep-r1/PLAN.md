# Stripe documented-operation parity — plan

> **Generated mechanically** from `data/cli-top50-fixed-schema-sweep-r1/MASTER-PLAN.json`
> (`stripe`, landing order 24). The operation count below was derived in the sweep-wide
...
- **Artifact**: `https://raw.githubusercontent.com/stripe/openapi/master/openapi/spec3.json`
- **Kind**: openapi, OAS `3.0.0`
- **Retrieved**: 2026-08-07, 7967776 bytes
- **Documented operations: 589**
- **By method**: DELETE 32, GET 263, HEAD 0, OPTIONS 0, PATCH 0, POST 294, PUT 0, TRACE 0
- **Read / write split**: 265 read, 324 write
- **Deprecated (still counted)**: 6
...
**How it was counted.** Counted every unique METHOD+path under `paths` in the OpenAPI 3.0.0 doc (294 POST, 263 GET, 32 DELETE, 0 PUT/PATCH = 589, exactly matching the ledger). Stripe's operationIds are path-derived (PostXxxYyy), not semantic, so summaries/descriptions were read directly: all Stripe Search API endpoints are GET (already read), so of 294 POSTs only `create_preview` (invoice preview, confirmed non-persisting) and `tax/calculations` (returns a Tax Calculation object, confirmed non-persisting) are reads; `create_from_calculation` looks similar by name but persists a real Tax Transaction record for filing, so it stayed a write, per this task's explicit 'Stripe POST create+update = write' rule.
...
write, or a binary transfer, **and** be individually reachable as its own `pm stripe <command>`.
...
Parent **#3005** (new generation); children are expected at **#3006–#3012** (new-generation pattern) — **CONFIRM with `gh-axi` before using them in a PR body**, this is inferred from the pattern, not verified.
...
1. **RED** — write `cmd/connectorgen/stripe_api_surface_test.go` **against this connector's real
   ...
   surface test (`ls cmd/connectorgen/ | grep '^stripe_'`) — gong carried two, and a targeted
