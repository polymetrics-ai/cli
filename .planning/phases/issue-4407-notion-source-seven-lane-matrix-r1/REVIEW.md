...
- Non-GET semantic reads are kept distinct from mutations. The four retained semantic POST reads are source-backed direct-read candidates because their cited request/response contracts prove bounded reads; arbitrary mutation POSTs remain not-applicable.
- `query-meeting-notes` is validly both a direct-read candidate and an ETL candidate; the ETL cell retains its missing-continuation mapping restriction.
