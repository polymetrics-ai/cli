...
- Every non-OpenAPI kind validates captured artifact and published-capture metadata. Rendered citations are HTTPS, absolute, same-origin provenance only; the code comment and fetch assertions prove they are never fetched.
- Rendered and bundle operations flow through the existing inventory, route, identity, count, deduplication, source-projection provenance, and final import-identity checks. The distinct importer branch only avoids treating declared non-OpenAPI bytes as an OpenAPI parser input.
