...
6. The branch contains the published #4304 foundation head but #4304 remains its open parent PR. Keep #4295 based on `fm/cli-reverse-etl-destination-r1` until that PR merges; only then retarget via `gh-axi api PATCH`. Do not fabricate a `transport_binding` or author a typed action without a provider-declared request contract; all other mutations stay identified as `direct_write` and `declaration-pending` until they have one.
...
- CLI help/manual/website parity: applicable to the source-cited Zendesk `delete_api_token` plan command. `pm help connectors`, bare `pm zendesk-support`, and `pm zendesk-support writes delete_api_token plan --help` are exercised without credentials; `docs/skills/pm-zendesk-support/SKILL.md` is regenerated. Connector-catalog website data is generated from bundle metadata rather than per-action commands; `make docs-check` validates the generated manual and website catalog parity.
