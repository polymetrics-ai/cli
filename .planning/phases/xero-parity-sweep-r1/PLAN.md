# Xero documented-operation parity — plan

> **Generated mechanically** from `data/cli-top50-fixed-schema-sweep-r1/MASTER-PLAN.json`
> (`xero`, landing order 13). The operation count below was derived in the sweep-wide
...
- **Artifact**: `https://raw.githubusercontent.com/XeroAPI/Xero-OpenAPI/master/xero_accounting.yaml`
- **Kind**: openapi, OAS `3.0.0`
- **Retrieved**: 2026-08-07, 927540 bytes
- **Documented operations: 235**
- **By method**: DELETE 10, GET 126, POST 46, PUT 53
- **Read / write split**: 126 read, 109 write
...
**How it was counted.** Parsed with yaml.safe_load, walked `paths`, counted one operation per (path, method) pair for real HTTP methods only. Reviewed all 46 POST paths by name -- all are Xero's create/update-by-POST convention (Contacts, Invoices, attachments, etc.) with none shaped like search/query/list/batchGet, so all POSTs classified as writes; all 126 GETs are reads. Cross-checked with a duplicate-key-detecting YAML loader (0 duplicate keys in the whole document) and an independent recount; both reproduced 235/126/53/46/10 exactly.
...
write, or a binary transfer, **and** be individually reachable as its own `pm xero <command>`.
...
Parent **#3102** (new generation); children are expected at **#3103–#3109** (new-generation pattern) — **CONFIRM with `gh-axi` before using them in a PR body**, this is inferred from the pattern, not verified.
...
1. **RED** — write `cmd/connectorgen/xero_api_surface_test.go` **against this connector's real
   ...
   surface test (`ls cmd/connectorgen/ | grep '^xero_'`) — gong carried two, and a targeted
