...
| Credential-bearing `base_url` admission | A schema-declared endpoint URI accepts URL user-info and a query component. | Shared configuration validation rejects those endpoint shapes before request construction while retaining an ordinary endpoint URI. | The error is the existing safe format classification and does not echo the rejected input; no provider I/O, output, persistence, or declaration changes occur. |

## Actual evidence

### 2026-08-24 — credential-bearing `base_url` admission safety slice

- Authorization: Firstmate authorized only this input-validation hardening while holding the `base_url` non-secret classification decision for the captain. The prior declaration scan found 532 `base_url` fields and no provider-declared user-info/query credential shape.
- Red: `go test -timeout 20m ./internal/connectors/engine -run '^TestSchemaValidateConfigurationRejectsCredentialBearingBaseURL$' -count=1` failed before production edits: both URL user-info and query-component cases returned nil validation errors. A second red test, `^TestSchemaValidateConfigurationRejectsCredentialBearingBaseURLWithoutDeclaredURIFormat$`, then failed because the 29 source-locked `base_url` fields without `format: uri` did not advertise or run a configuration constraint.
- Green: the shared configuration boundary now treats every declared `base_url` as endpoint metadata and requires an absolute URI without URL user-info or a query component. The format-declared and format-omitted tables each prove user-info/query rejection plus ordinary endpoint acceptance. `go test -timeout 20m ./internal/connectors/engine -run '^TestSchema(ValidateConfiguration|WithoutConfigurationConstraints|ValidateConfigurationReturnsSharedClassification)' -count=1` and `go test -timeout 20m ./internal/connectors/engine -count=1` passed.
- Credential-input boundary: `go test -timeout 20m ./internal/app -run '^(TestAddCredentialRejectsInvalidGitHubBaseURLAtConfigurationTime|TestAddCredentialRejectsCredentialBearingGitHubBaseURLAtConfigurationTime)$' -count=1` passed. It proves `AddCredential` rejects both shapes without storing a credential or echoing the rejected URL. The full app package passed in 393.089s under a completion-tracked session.
- Persistence decision remains independent: the unchanged GraphQL plaintext-state regression uses an ordinary endpoint URI (no user-info/query) and still requires the existing persistence-only receipt projection to pass. Admission hardening neither weakens nor replaces that test, and does not authorize a metadata classification or credential-storage redesign.
