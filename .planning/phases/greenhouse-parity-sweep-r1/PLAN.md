# Greenhouse documented-operation parity — plan

> **Generated mechanically** from `data/cli-top50-fixed-schema-sweep-r1/MASTER-PLAN.json`
> (`greenhouse`, landing order 7). The operation count below was derived in the sweep-wide
> artifact pass, not re-reasoned here. **Per-connector findings are NOT pre-planned** — if this
> connector surprises you during implementation, STOP and record it rather than forcing it into
> this shape.

Part of `cli-top50-fixed-schema-sweep-r1`. One connector, one PR.

## Operation surface, derived before authoring

- **Artifact**: `https://developers.greenhouse.io/harvest.html`
- **Kind**: html_reference
- **Retrieved**: 2026-08-07, 1636662 bytes
- **Documented operations: 138**
- **By method**: DELETE 14, GET 69, PATCH 19, POST 28, PUT 8
- **Read / write split**: 69 read, 69 write
- **Deprecated (still counted)**: 3

### Against the provider-artifact ledger

| | |
| --- | ---: |
| Ledger recorded | 138 |
| Re-derived | 138 |
| Delta | 0 |

The ledger's figure **reconciles exactly** with the live artifact.

**How it was counted.** No OpenAPI/Swagger spec exists for Greenhouse's Harvest API (checked the page for 'openapi'/'swagger' strings — none). This is a single Slate-generated static HTML page (1,636,662 bytes). Rather than counting <h2> headings or curl examples, extracted the canonical '<h3>HTTP Request</h3><p><code>METHOD https://harvest.greenhouse.io/vN/path</code></p>' declaration that follows every real endpoint section (one declaration per operation, independent of prose/heading wording). Initial strict regex found 137 matches; one endpoint (DELETE: Destroy a candidate tag) was missed because its HTTP Request code block contains a stray unescaped &#39; before the URL and its path placeholder has a literal space ({tag id} instead of {tag_id}) — both markup quirks in Greenhouse's own docs. A loosened, verified regex recovered it, giving 138 total matches with zero duplicates — an exact match to ledger_total. Cross-validated via a full heading/code document-order walk: 135 h2 sections carry a METHOD: prefix (pages_seen); 132 of them declare exactly 1 operation and 3 declare 2 operations each (v1+v2 pairs), 132 + 3*2 = 138, matching the direct count exactly.

## Hazards

- **HTML-derived count**: 135 documentation pages deduplicated to 138 unique actions. Counting pages instead of unique method+path actions is a known miscount in this programme.

## Webhook inventory (input for `cli-webhook-surface-sweep-r1`)

- **Webhook events: 0** — excluded from the operation total per the counting policy.
- **Webhook management endpoints: 0** — these stay **in scope** and are counted in the
  total. Create/list/update/delete of a webhook *subscription* is an ordinary REST operation; only
  webhook *events* are deferred.

## Required scopes — all five must be covered

Every documented operation must land as an ETL stream, a reverse-ETL write, a direct read, a direct
write, or a binary transfer, **and** be individually reachable as its own `pm greenhouse <command>`.
Every `api_surface.json` row must carry exactly one of `executable`,
`blocked-with-named-dependency`, or `unsupported-with-source-citation` — **never blank, and never
the legacy `excluded` category**.

## Issues

Parent **#3199** (new generation); children are expected at **#3200–#3206** (new-generation pattern) — **CONFIRM with `gh-axi` before using them in a PR body**, this is inferred from the pattern, not verified.

Use `Closes` only for what this PR genuinely completes and `Refs` for the rest.

## TDD sequence — the red test is NOT generated, by design

1. **RED** — write `cmd/connectorgen/greenhouse_api_surface_test.go` **against this connector's real
   bundle** and **watch it fail**. Paste the verbatim failure into `TDD-LEDGER.md` and set
   `tdd.red_confirmed` / `tdd.red_failure` in `RUN-STATE.json`. Check first for a pre-existing
   surface test (`ls cmd/connectorgen/ | grep '^greenhouse_'`) — gong carried two, and a targeted
   `-run` missed the second.
2. **GREEN** — author the bundle to satisfy it.
3. **REFACTOR** — docs, catalogs, operation endpoint ledger resync.
4. Gates, then no-mistakes.

`check_red_observed.py` refuses to let this connector proceed to implementation until the red
failure is real observed output.

## Safety notes

- Do not loosen `connectorgen validate`, the connector boundary gate, `certify`, or
  `TestEveryImplementedCommandPassesRuntimePreflight` to make this pass.
- Nothing is marked `implemented` unless its command genuinely runs; every block names a dependency.
- Run the WHOLE `cmd/connectorgen` package plus `internal/cli`, never just a targeted `-run`.
- Regenerating docs rewrites ~1,034 files of pre-existing `main` drift (finding F4) — revert every
  non-greenhouse path.
- Inspect the website catalog diff **by object, not by line**.
- No credential or token-derived value is ever emitted.
