...
| --- | --- | --- | --- |
| 1 target-scope core validator | Failing tests for exactly-one slug, auto-detected connector scope, shared runtime rejection, unrelated connector rejection, unrelated generated rejection, label omission bypass | `go test ./internal/connectors/boundary ./cmd/connectorgen` plus CLI fixture runs | Planned |
...
| 3 PM/no-mistakes integration | Failing docs/config/grep test or docs-only exemption | guidance files updated; grep proves connector lane says stop and split foundation PR | Planned |
