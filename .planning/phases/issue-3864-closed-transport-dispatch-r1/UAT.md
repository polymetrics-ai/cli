...
| Checkpoint advancement waits for durable acknowledgement and persists before post-ack failure | T15 plus `TestOrchestratorCommitsOnlyAfterDurableAcknowledgement`, `TestOrchestratorStopsOnCancellationBetweenWarehouseStageAndApply`, and `TestOrchestratorCommitsAcknowledgedPageBeforeReturningCancellation` | Pass: invalid acknowledgement or pre-apply cancellation yields zero commits; an acknowledged page is persisted before cancellation, source failure, and final state-save failure across all seven modes. |
