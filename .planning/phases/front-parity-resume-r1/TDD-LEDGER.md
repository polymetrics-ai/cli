...
| empty required-array CLI transport | trace `coerceFlagValue` and `validateRequiredCommandFlags` for Front's required `string_array` flags | blank `--teammate-ids` or `--columns` coerces to `[]`, which the shared required check rejects despite Front's no-`minItems` schemas | captain-approved external dependency `cli-engine-empty-required-array-r1`; Front help documents the limit for `create_conversation.teammate_ids`, `analytics exports create`, and `analytics exports create-events` without changing shared code |
