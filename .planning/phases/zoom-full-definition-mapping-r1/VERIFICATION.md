...
is now incorporated through `main`. #4332 has landed the rendered-reference citation contract, but
the next terminal source boundary is stable artifact attestation for the preserved 35 Zoom captures.
...
| `go test -count=1 -timeout 20m ./internal/connectors/defs/zoom` | NOT COMPLETED (2026-08-23) — stopped before a result when the full package process became CPU-bound on the shared host. This is not a passing claim; the focused Zoom contract tests above passed and the complete package test will be rerun after stable capture attestation. |
