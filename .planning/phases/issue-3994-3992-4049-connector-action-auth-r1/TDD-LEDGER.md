...
| R10 | live reverse-plan reload preserves in-memory typed policy tests | hosted run `31890181420` and focused local reproduction: `TestPostgresManagedTargetMissingAndConsumedApprovalErrorsAreTyped` got `state path is required` after branch-owned `GetReversePlan` began unconditionally loading the store | configured stores still call `loadReversePlan`; explicitly in-memory App lookup returns `*AuthorizationTokenReplayError`, leaves state unchanged, and the installed-flow fresh-binary proof remains green | green |
| R11 | prepared-execution directory durability handles cleanup errors | focused `golangci-lint` failed `errcheck` at `defer directory.Close()` in branch-owned `syncPreparedExecutionDir` | directory close is explicit, a sync error remains dominant, close failure is contextual, and focused App/certification lint passes with zero findings | green |
