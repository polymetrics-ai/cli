...
| Podman GSD startup | The governed query passed version/mount admission, then real GSD exited because the slim Node image has no `git` on `PATH` | Add the minimal pinned-image OS prerequisite only after explicit dependency approval, rebuild, and rerun the same query | BLOCKED_HUMAN_GATE |
