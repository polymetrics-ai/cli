...
| Green | Non-deprecated live mutation rows, including admin/destructive/sensitive-shaped operations, are typed fixed-document reverse-ETL actions with risk text and destructive confirmation where applicable; raw arbitrary GraphQL remains blocked by default and deprecated `integrationSettingsUpdate` is blocked with exact schema evidence. | done |
