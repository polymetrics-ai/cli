...
var allBoundEnvVars = []string{
	...
	"POLYMETRICS_OTEL_EXPORTER_OTLP_ENDPOINT", "OTEL_EXPORTER_OTLP_ENDPOINT", "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT",
	"POLYMETRICS_TELEMETRY_DIR", "PM_TELEMETRY_DIR",
	"POLYMETRICS_TELEMETRY_CAPTURE", "PM_TELEMETRY_CAPTURE",
}

func TestAllBoundEnvVarsIncludesOTLPTracesEndpoint(t *testing.T) {
	for _, name := range allBoundEnvVars {
		if name == "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT" {
			return
		}
	}
	t.Fatal("allBoundEnvVars missing OTEL_EXPORTER_OTLP_TRACES_ENDPOINT")
}
