...
func TestTransportPhaseMeasurementReportsLogicalInputThroughputAndEveryFastPathPhase(t *testing.T) {
	measurement := transportPhaseMeasurement(synctransport.Result{
		...
		ApplyElapsed: time.Second, IndexConstraintElapsed: 750 * time.Millisecond, PublishElapsed: 500 * time.Millisecond, CheckpointElapsed: 250 * time.Millisecond,
	...
	if measurement.SourceReadElapsedNanos != (4*time.Second).Nanoseconds() || measurement.TransformElapsedNanos != (3*time.Second).Nanoseconds() || measurement.ParquetCloseElapsedNanos != (2*time.Second).Nanoseconds() || measurement.BinaryCOPYElapsedNanos != (time.Second).Nanoseconds() || measurement.IndexConstraintBuildElapsedNanos != (750*time.Millisecond).Nanoseconds() || measurement.PublishReceiptElapsedNanos != (500*time.Millisecond).Nanoseconds() || measurement.CheckpointElapsedNanos != (250*time.Millisecond).Nanoseconds() || measurement.CriticalPathElapsedNanos != (10*time.Second).Nanoseconds() {
