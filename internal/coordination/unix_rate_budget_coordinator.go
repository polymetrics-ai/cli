...
// socket and removes both it and its private run directory on Close.
...
// UnixRateBudgetCoordinatorClient is a capability for the owner created in
// the same run. Its endpoint and epoch are intentionally unexported so they
// cannot accidentally become operator evidence.
...
// SharedCoordinatorError is deliberately closed: it reports only the safe
// failure class, never an endpoint, run path, epoch, request, or credential.
type SharedCoordinatorError struct {
	reason sharedCoordinatorErrorReason
}
...
// IsSharedCoordinatorUnavailable reports an owner-loss or transport failure
// that must fail closed under require_shared.
...
// IsSharedCoordinatorEpochMismatch reports a client from an earlier owner
// epoch trying to use a new owner. It is not retried or downgraded locally.
...
// StartUnixRateBudgetCoordinator creates a mode-0700, short /tmp run
// directory and a mode-0600 socket, starts the owner, and performs a versioned
// readiness handshake before returning either capability. The client endpoint
// is kept in memory only and is never emitted by this package.
...
// Close stops accepting new decisions, closes active connections, and removes
// only this owner's known socket and run directory. It is safe to call more
// than once.
...
func (o *UnixRateBudgetCoordinatorOwner) handle(conn *net.UnixConn) {
	...
	response := o.handleRequest(request)
	_ = writeUnixRateBudgetResponse(conn, response)
}
...
// Ready performs the narrow, versioned readiness handshake. There is no
// generic RPC operation exposed through this socket.
...
// Decide delegates exactly one atomic reservation batch to the run owner.
func (c *UnixRateBudgetCoordinatorClient) Decide(ctx context.Context, batch connsdk.ReservationBatch) (connsdk.AdmissionDecision, error) {
	...
	response, err := c.exchange(ctx, unixRateBudgetRequest{
		Version: unixRateBudgetProtocolVersion,
		Kind:    unixRateBudgetDecide,
		Epoch:   c.epoch,
		Batch:   batch,
	})
	if err != nil {
...
// Finish releases one owner-issued lease. A transport failure is deliberately
// returned to the caller: it cannot prove whether the owner received Finish,
// so the short lease TTL is the conservative concurrency recovery path.
func (c *UnixRateBudgetCoordinatorClient) Finish(ctx context.Context, lease connsdk.RateBudgetLease, observation connsdk.CompletionObservation) error {
	...
	_, err := c.exchange(ctx, unixRateBudgetRequest{
		Version:     unixRateBudgetProtocolVersion,
		Kind:        unixRateBudgetFinish,
		Epoch:       c.epoch,
		Lease:       lease,
...
func (c *UnixRateBudgetCoordinatorClient) exchange(ctx context.Context, request unixRateBudgetRequest) (unixRateBudgetResponse, error) {
	...
	if response.Error.Code != unixRateBudgetErrorNone {
