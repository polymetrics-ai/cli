...
func renderCodexProjection(contract *Contract, role string) ([]byte, error) {
	...
	if strings.Contains(instructions, `"""`) {
		return nil, fmt.Errorf("canonical contract: Codex developer instructions contain a TOML multiline-string delimiter")
	}
	...
	fmt.Fprintf(&output, "description = %s\n", strconv.Quote(roleSummary(contract, role)))
	fmt.Fprintln(&output, "developer_instructions = \"\"\"")
