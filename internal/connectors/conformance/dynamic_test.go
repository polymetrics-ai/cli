...
func TestCursorAdvances_ComparisonPrefixNumericCursorNormalized(t *testing.T) {
	b := loadTestBundle(t, "testdata/good", "acme-comparison-range-cursor")
	result := checkCursorAdvances(b)
	if !result.Passed {
		t.Fatalf("cursor_advances failed on comparison-prefix formatting with a numeric (Unix-seconds) cursor: %s", result.Error)
	}
}

func TestCursorAdvances_ComparisonPrefixNonUTCOffsetCursorNormalized(t *testing.T) {
	b := loadTestBundle(t, "testdata/good", "acme-comparison-range-cursor-offset")
	result := checkCursorAdvances(b)
	if !result.Passed {
		t.Fatalf("cursor_advances failed on comparison-prefix formatting with a non-UTC-offset RFC3339 cursor: %s", result.Error)
	}
}
