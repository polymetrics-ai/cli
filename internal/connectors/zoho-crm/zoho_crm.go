// Package zohocrm implements a read-only native Zoho CRM API connector.
package zohocrm
...
const (
	connectorName  = "zoho-crm"
	defaultBaseURL = "https://www.zohoapis.com/crm/v6"
...
func (Connector) Metadata() connectors.Metadata {
	return connectors.Metadata{Name: connectorName, DisplayName: "ZohoCRM", IntegrationType: "api", Description: "Reads Zoho CRM leads, accounts, and contacts through the Zoho CRM REST API.", Capabilities: connectors.Capabilities{Check: true, Catalog: true, Read: true, Write: false}}
}
...
var streamOrder = []string{"leads", "accounts", "contacts"}

var streams = map[string]streamEndpoint{
	"leads":    {path: "Leads", recordsPath: "data", desc: "Zoho CRM leads.", idKeys: []string{"id"}, nameKeys: []string{"Full_Name", "Lead_Name", "Company", "name"}, emailKeys: []string{"Email", "email"}, cursorKeys: []string{"Modified_Time", "updated_at"}},
	"accounts": {path: "Accounts", recordsPath: "data", desc: "Zoho CRM accounts.", idKeys: []string{"id"}, nameKeys: []string{"Account_Name", "name"}, cursorKeys: []string{"Modified_Time", "updated_at"}},
	"contacts": {path: "Contacts", recordsPath: "data", desc: "Zoho CRM contacts.", idKeys: []string{"id"}, nameKeys: []string{"Full_Name", "Last_Name", "name"}, emailKeys: []string{"Email", "email"}, cursorKeys: []string{"Modified_Time", "updated_at"}},
}
...
func readFixture(ctx context.Context, stream string, ep streamEndpoint, req connectors.ReadRequest, emit func(connectors.Record) error) error {
	for i := 1; i <= 2; i++ {
		...
		rec := mapRecord(ep, map[string]any{"id": fmt.Sprintf("%s_fixture_%d", stream, i), "name": fmt.Sprintf("%s fixture %d", strings.TrimSuffix(stream, "s"), i), "email": fmt.Sprintf("fixture+%d@example.com", i), "updated_at": fixtureTime})
...
func (c Connector) requester(cfg connectors.RuntimeConfig) (*connsdk.Requester, error) {
	...
	if token == "" {
		return nil, errors.New("zoho-crm connector requires secret access_token")
	}
...
func mapRecord(ep streamEndpoint, in map[string]any) connectors.Record {
	...
	if out["email"] == nil {
		out["email"] = firstValue(in, ep.emailKeys)
	}
