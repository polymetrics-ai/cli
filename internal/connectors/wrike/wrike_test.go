...
import (
	...
	"polymetrics.ai/internal/connectors/wrike"
)
...
func TestFixtureCatalogRegistrationAndWrite(t *testing.T) {
	...
	if cat.Connector != "wrike" || len(cat.Streams) < 3 {
		t.Fatalf("catalog = %+v, want wrike streams", cat)
	}
	var rows []connectors.Record
	err = c.Read(context.Background(), connectors.ReadRequest{Stream: "tasks", Config: fixture}, func(rec connectors.Record) error {
	...
	if _, ok := connectors.NewRegistry().Get("wrike"); !ok {
		t.Fatal("registry did not resolve wrike")
	}
...
func TestReadLiveUsesBearerAuthAndMapsData(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"data":[{"id":"task-1","title":"Launch","updatedDate":"2026-01-01T00:00:00Z"}]}`))
	}))
	defer srv.Close()
	...
	if len(got) != 1 || got[0]["id"] != "task-1" {
		t.Fatalf("records = %+v, want task-1", got)
	}
}
