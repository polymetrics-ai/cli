...
import (
	...
	"polymetrics.ai/internal/connectors/tickettailor"
)
...
func TestReadEventsPaginatesAndAuthenticates(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "1":
			_, _ = w.Write([]byte(`{"data":[{"id":"evt_1","name":"Launch"},{"id":"evt_2","name":"Workshop"}]}`))
		case "2":
			_, _ = w.Write([]byte(`{"data":[{"id":"evt_3","name":"Afterparty"}]}`))
		default:
	...
	c := tickettailor.New()
	cfg := connectors.RuntimeConfig{Config: map[string]string{"base_url": srv.URL, "page_size": "2"}, Secrets: map[string]string{"api_key": "tt-key"}}
...
func TestFixtureCatalogRegistrationAndWrite(t *testing.T) {
	c := tickettailor.New()
	assertConnector(t, c, "tickettailor")
}
