...
import (
	...
	"polymetrics.ai/internal/connectors/timely"
)

func TestReadUsersAuthenticatesAndUsesAccountPath(t *testing.T) {
	var sawAuth string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		sawAuth = r.Header.Get("Authorization")
		if r.URL.Path != "/123/users" {
			http.NotFound(w, r)
			return
		}
		_, _ = w.Write([]byte(`[{"id":1,"name":"Ada","email":"ada@example.com"},{"id":2,"name":"Ben","email":"ben@example.com"}]`))
	}))
	defer srv.Close()

	c := timely.New()
	cfg := connectors.RuntimeConfig{Config: map[string]string{"base_url": srv.URL, "account_id": "123", "start_date": "2026-01-01T00:00:00Z"}, Secrets: map[string]string{"bearer_token": "timely-token"}}
	...
	if sawAuth != "Bearer timely-token" {
		t.Fatalf("Authorization = %q", sawAuth)
	}
	if len(got) != 2 || got[0]["email"] == nil {
