// Package zohobilling implements a read-only native Zoho Billing API connector.
package zohobilling
...
const (
	connectorName  = "zoho-billing"
	defaultBaseURL = "https://www.zohoapis.com/billing/v1"
...
func (Connector) Metadata() connectors.Metadata {
	return connectors.Metadata{
		...
		IntegrationType: "api",
		Description:     "Reads Zoho Billing customers, subscriptions, and invoices through the Zoho Billing REST API.",
...
var streamOrder = []string{"customers", "subscriptions", "invoices"}

var streams = map[string]streamEndpoint{
	"customers": {
		path: "customers", recordsPath: "customers", desc: "Zoho Billing customers.",
		idKeys: []string{"customer_id", "id"}, nameKeys: []string{"display_name", "customer_name", "name"}, cursorKeys: []string{"updated_time", "last_modified_time", "updated_at"},
	},
	"subscriptions": {
		path: "subscriptions", recordsPath: "subscriptions", desc: "Zoho Billing subscriptions.",
		idKeys: []string{"subscription_id", "id"}, nameKeys: []string{"name", "plan_name"}, cursorKeys: []string{"updated_time", "last_modified_time", "updated_at"},
	},
	"invoices": {
		path: "invoices", recordsPath: "invoices", desc: "Zoho Billing invoices.",
		idKeys: []string{"invoice_id", "id"}, nameKeys: []string{"invoice_number", "number"}, cursorKeys: []string{"updated_time", "last_modified_time", "updated_at"},
	},
}
...
func (c Connector) Check(ctx context.Context, cfg connectors.RuntimeConfig) error {
	...
	q := baseQuery(cfg, 1, 1)
	if _, err := r.Do(ctx, http.MethodGet, streams[streamOrder[0]].path, q, nil); err != nil {
...
func harvest(ctx context.Context, r *connsdk.Requester, ep streamEndpoint, cfg connectors.RuntimeConfig, size, max int, emit func(connectors.Record) error) error {
	for page := 1; max == 0 || page <= max; page++ {
		q := baseQuery(cfg, page, size)
		resp, err := r.Do(ctx, http.MethodGet, ep.path, q, nil)
		if err != nil {
...
func (c Connector) requester(cfg connectors.RuntimeConfig) (*connsdk.Requester, error) {
	...
	if token == "" {
		return nil, errors.New("zoho-billing connector requires secret access_token")
	}
