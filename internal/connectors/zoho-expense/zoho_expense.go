// Package zohoexpense implements a read-only native Zoho Expense API connector.
package zohoexpense
...
const (
	connectorName  = "zoho-expense"
	defaultBaseURL = "https://www.zohoapis.com/expense/v1"
...
func (Connector) Metadata() connectors.Metadata {
	return connectors.Metadata{Name: connectorName, DisplayName: "Zoho Expense", IntegrationType: "api", Description: "Reads Zoho Expense reports, expenses, and users through the Zoho Expense REST API.", Capabilities: connectors.Capabilities{Check: true, Catalog: true, Read: true, Write: false}}
}
...
var streamOrder = []string{"reports", "expenses", "users"}

var streams = map[string]streamEndpoint{
	"reports":  {path: "expensereports", recordsPath: "expense_reports", desc: "Zoho Expense reports.", idKeys: []string{"report_id", "expense_report_id", "id"}, nameKeys: []string{"report_name", "name"}, cursorKeys: []string{"modified_time", "last_modified_time", "updated_at"}},
	"expenses": {path: "expenses", recordsPath: "expenses", desc: "Zoho Expense expenses.", idKeys: []string{"expense_id", "id"}, nameKeys: []string{"merchant_name", "description", "name"}, cursorKeys: []string{"modified_time", "last_modified_time", "updated_at"}},
	"users":    {path: "users", recordsPath: "users", desc: "Zoho Expense users.", idKeys: []string{"user_id", "id"}, nameKeys: []string{"name", "email"}, cursorKeys: []string{"modified_time", "updated_at"}},
}
...
func readFixture(ctx context.Context, stream string, ep streamEndpoint, req connectors.ReadRequest, emit func(connectors.Record) error) error {
	for i := 1; i <= 2; i++ {
		...
		rec := mapRecord(ep, map[string]any{"id": fmt.Sprintf("%s_fixture_%d", stream, i), "name": fmt.Sprintf("%s fixture %d", strings.TrimSuffix(stream, "s"), i), "merchant_name": "Fixture", "updated_at": fixtureTime})
...
func (c Connector) requester(cfg connectors.RuntimeConfig) (*connsdk.Requester, error) {
	...
	if token == "" {
		return nil, errors.New("zoho-expense connector requires secret access_token")
	}
