// Package zohoinvoice implements a read-only native Zoho Invoice API connector.
package zohoinvoice
...
const (
	connectorName  = "zoho-invoice"
	defaultBaseURL = "https://www.zohoapis.com/invoice/v3"
...
func (Connector) Metadata() connectors.Metadata {
	return connectors.Metadata{Name: connectorName, DisplayName: "Zoho Invoice", IntegrationType: "api", Description: "Reads Zoho Invoice customers, invoices, and payments through the Zoho Invoice REST API.", Capabilities: connectors.Capabilities{Check: true, Catalog: true, Read: true, Write: false}}
}
...
var streamOrder = []string{"customers", "invoices", "payments"}

var streams = map[string]streamEndpoint{
	"customers": {path: "customers", recordsPath: "customers", desc: "Zoho Invoice customers.", idKeys: []string{"customer_id", "contact_id", "id"}, nameKeys: []string{"customer_name", "display_name", "name"}, cursorKeys: []string{"last_modified_time", "updated_at"}},
	"invoices":  {path: "invoices", recordsPath: "invoices", desc: "Zoho Invoice invoices.", idKeys: []string{"invoice_id", "id"}, nameKeys: []string{"invoice_number", "number"}, cursorKeys: []string{"last_modified_time", "updated_at"}},
	"payments":  {path: "customerpayments", recordsPath: "customerpayments", desc: "Zoho Invoice payments.", idKeys: []string{"payment_id", "payment_id", "id"}, nameKeys: []string{"payment_number", "reference_number"}, cursorKeys: []string{"last_modified_time", "updated_at"}},
}
...
func (c Connector) requester(cfg connectors.RuntimeConfig) (*connsdk.Requester, error) {
	...
	if token == "" {
		return nil, errors.New("zoho-invoice connector requires secret access_token")
	}
