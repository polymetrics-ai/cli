...
import (
	...
	"polymetrics.ai/internal/connectors/safetyculture"
)

func TestReadAuditsPaginatesAuthenticatesAndMaps(t *testing.T) {
	var pages []string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch page {
		case "1":
			_, _ = w.Write([]byte(`{"audits":[{"audit_id":"a_1","name":"Store audit"},{"audit_id":"a_2","name":"Warehouse audit"}],"next_page":"/audits?page=2"}`))
		case "2":
			_, _ = w.Write([]byte(`{"audits":[{"audit_id":"a_3","name":"Vehicle audit"}],"next_page":""}`))
		default:
	...
	err := c.Read(context.Background(), connectors.ReadRequest{Stream: "audits", Config: cfg}, func(rec connectors.Record) error {
		got = append(got, rec)
		return nil
	})
	if err != nil {
	...
	if len(pages) != 2 || len(got) != 3 || got[0]["id"] != "a_1" {
...
func TestFixtureCatalogRegistrationAndReadOnly(t *testing.T) {
	...
	if cat.Connector != "safetyculture" || len(cat.Streams) == 0 {
	...
	if _, ok := connectors.NewRegistry().Get("safetyculture"); !ok {
		t.Fatal("registry did not resolve safetyculture")
	}
