...
// refresh-token grant used by the Gmail API. It exchanges the configured refresh
// token for a short-lived access token at the Google token endpoint, caches it
// until shortly before expiry, then sets Authorization: Bearer <token> on each
// request.
//
