...
import (
	...
	native "polymetrics.ai/internal/connectors/native/metabase"
)

func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("metabase")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "metabase" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "cards"}, connectors.ReadRequest{Stream: "cards", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
...
func TestReadStreamEmptyDefaultsToCards(t *testing.T) {
	h := Hooks{Connector: native.New()}
	cfg := connectors.RuntimeConfig{Config: map[string]string{"mode": "fixture"}}
	count := 0
