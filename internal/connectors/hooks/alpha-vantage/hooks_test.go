...
func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("alpha-vantage")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "alpha-vantage" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "time_series_daily"}, connectors.ReadRequest{Stream: "time_series_daily", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
