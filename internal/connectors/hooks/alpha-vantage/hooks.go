// Package alphavantage bridges the alpha-vantage quarantine bundle to the native connector.
...
import (
	...
	native "polymetrics.ai/internal/connectors/native/alpha-vantage"
)

func init() {
	engine.RegisterHooks("alpha-vantage", func() engine.Hooks { return New() })
}
...
// New returns a hook set backed by the native alpha-vantage connector.
func New() engine.Hooks { return Hooks{Connector: native.New()} }

func (h Hooks) ConnectorName() string { return "alpha-vantage" }
...
func (h Hooks) ReadStream(ctx context.Context, stream engine.StreamSpec, req connectors.ReadRequest, rt *engine.Runtime, emit func(connectors.Record) error) (bool, error) {
	...
	if req.Stream == "" {
		return true, fmt.Errorf("alpha-vantage" + " stream name is required")
	}
