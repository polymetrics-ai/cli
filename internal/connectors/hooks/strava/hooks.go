// Package strava implements the strava bundle's AuthHook (fresh migration,
// capability parity with internal/connectors/strava/strava.go's
// refreshTokenAuth): an OAuth 2.0 refresh-token-grant connsdk.Authenticator,
// porting legacy's refreshTokenAuth almost verbatim (~120 lines, well under
// the 300-line Tier-2 hook cap, conventions.md §1). Only one hook interface
// is implemented (AuthHook), well under the 2-interface cap.
//
// This is the identical shape to internal/connectors/hooks/gmail's pilot
...
func init() {
	engine.RegisterHooks("strava", func() engine.Hooks { return New() })
}

// Hooks is the strava hook set. It implements engine.AuthHook only.
type Hooks struct {
...
// New returns a fresh strava Hooks value as engine.Hooks.
func New() engine.Hooks { return &Hooks{} }

func (h *Hooks) ConnectorName() string { return "strava" }

// Authenticator resolves the OAuth2 refresh-token-grant connsdk.Authenticator
// for spec (mode "custom", hook "strava"). Templated AuthSpec fields
...
// convention): strava's AuthSpec has no dedicated "refresh_token" field, and
...
// strings.TrimSpace(...) == "" checks, strava.go:319-330).
func interpolateRequired(tmpl, field string, cfg connectors.RuntimeConfig) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("strava oauth: resolve %s: %w", field, err)
	}
	if strings.TrimSpace(val) == "" {
		return "", fmt.Errorf("strava oauth: %s is required", field)
	}
	return val, nil
}
...
// with a host, matching legacy's resolveHTTPURL (strava.go:494-510), which
// accepts plain http (for local test servers) as well as https.
func validateHTTPURL(raw, field string) error {
	parsed, err := url.Parse(raw)
	if err != nil {
		return fmt.Errorf("strava oauth: %s is invalid: %w", field, err)
	}
	if parsed.Scheme != "https" && parsed.Scheme != "http" {
		return fmt.Errorf("strava oauth: %s must use http or https, got %q", field, parsed.Scheme)
	}
	if parsed.Host == "" {
		return fmt.Errorf("strava oauth: %s must include a host", field)
	}
	return nil
}

// refreshTokenAuth implements connsdk.Authenticator for Strava's OAuth 2.0
// refresh-token grant, mirroring legacy strava.go's refreshTokenAuth
...
// 6-hour TTL when the response has neither expires_in nor expires_at,
// strava.go:418-424), then set Authorization: Bearer <token> on each
...
func (a *refreshTokenAuth) accessToken(ctx context.Context) (string, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
	// Refresh 60s before expiry to avoid edge races (legacy strava.go:372-375).
	if a.token != "" && a.timeNow().Add(60*time.Second).Before(a.expires) {
		return a.token, nil
	}
	if strings.TrimSpace(a.tokenURL) == "" {
		return "", errors.New("strava oauth: token URL is required")
	}
	if strings.TrimSpace(a.refreshToken) == "" {
		return "", errors.New("strava oauth: refresh_token is required")
	}
	if strings.TrimSpace(a.clientID) == "" {
		return "", errors.New("strava oauth: client_id is required")
	}
	...
	if err != nil {
		return "", fmt.Errorf("strava oauth: build token request: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("strava oauth: token request: %w", err)
	}
	defer func() { _ = resp.Body.Close() }()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return "", fmt.Errorf("strava oauth: token endpoint returned %s", resp.Status)
	}
	...
	if err := dec.Decode(&out); err != nil {
		return "", fmt.Errorf("strava oauth: decode token response: %w", err)
	}
	if strings.TrimSpace(out.AccessToken) == "" {
		return "", errors.New("strava oauth: token response missing access_token")
	}

	a.token = out.AccessToken
	// Legacy default TTL is 6 hours when neither expires_in nor expires_at
	// parses (strava.go:419), not gmail's 1-hour default — Strava's own
	// token responses always carry expires_in in practice, but the fallback
	// mirrors legacy exactly for parity.
	a.expires = a.timeNow().Add(6 * time.Hour)
