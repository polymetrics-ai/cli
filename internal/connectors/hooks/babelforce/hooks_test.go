...
func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("babelforce")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "babelforce" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "calls"}, connectors.ReadRequest{Stream: "calls", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
