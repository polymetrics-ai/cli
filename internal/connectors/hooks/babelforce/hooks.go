// Package babelforce bridges the babelforce quarantine bundle to the native connector.
package babelforce
...
import (
	...
	native "polymetrics.ai/internal/connectors/native/babelforce"
)

func init() {
	engine.RegisterHooks("babelforce", func() engine.Hooks { return New() })
}
...
// New returns a hook set backed by the native babelforce connector.
func New() engine.Hooks { return Hooks{Connector: native.New()} }

func (h Hooks) ConnectorName() string { return "babelforce" }
...
func (h Hooks) ReadStream(ctx context.Context, stream engine.StreamSpec, req connectors.ReadRequest, rt *engine.Runtime, emit func(connectors.Record) error) (bool, error) {
	...
	if req.Stream == "" {
		return true, fmt.Errorf("babelforce" + " stream name is required")
	}
