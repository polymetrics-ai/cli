...
func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("pocket")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "pocket" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "items"}, connectors.ReadRequest{Stream: "items", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
