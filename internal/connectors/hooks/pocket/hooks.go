// Package pocket bridges the pocket quarantine bundle to the legacy connector.
package pocket
...
import (
	...
	legacy "polymetrics.ai/internal/connectors/pocket"
)

func init() {
	engine.RegisterHooks("pocket", func() engine.Hooks { return New() })
}
...
// New returns a hook set backed by the legacy pocket connector.
func New() engine.Hooks { return Hooks{Connector: legacy.New()} }

func (h Hooks) ConnectorName() string { return "pocket" }
...
func (h Hooks) ReadStream(ctx context.Context, stream engine.StreamSpec, req connectors.ReadRequest, rt *engine.Runtime, emit func(connectors.Record) error) (bool, error) {
	...
	if req.Stream == "" {
		return true, fmt.Errorf("pocket" + " stream name is required")
	}
