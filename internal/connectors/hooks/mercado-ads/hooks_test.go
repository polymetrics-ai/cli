...
import (
	...
	legacy "polymetrics.ai/internal/connectors/mercado-ads"
)

func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("mercado-ads")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "mercado-ads" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "brand_advertisers"}, connectors.ReadRequest{Stream: "brand_advertisers", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
