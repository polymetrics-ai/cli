...
import (
	...
	native "polymetrics.ai/internal/connectors/native/bunny-inc"
)

func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("bunny-inc")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "bunny-inc" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "accounts"}, connectors.ReadRequest{Stream: "accounts", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
