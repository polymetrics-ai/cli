...
import (
	...
	native "polymetrics.ai/internal/connectors/native/copper"
)

func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("copper")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "copper" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "people"}, connectors.ReadRequest{Stream: "people", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
