// Package lessannoyingcrm bridges the less-annoying-crm quarantine bundle to the native connector.
...
import (
	...
	native "polymetrics.ai/internal/connectors/native/less-annoying-crm"
)

func init() {
	engine.RegisterHooks("less-annoying-crm", func() engine.Hooks { return New() })
}
...
// New returns a hook set backed by the native less-annoying-crm connector.
func New() engine.Hooks { return Hooks{Connector: native.New()} }

func (h Hooks) ConnectorName() string { return "less-annoying-crm" }
...
func (h Hooks) ReadStream(ctx context.Context, stream engine.StreamSpec, req connectors.ReadRequest, rt *engine.Runtime, emit func(connectors.Record) error) (bool, error) {
	...
	if req.Stream == "" {
		return true, fmt.Errorf("less-annoying-crm" + " stream name is required")
	}
