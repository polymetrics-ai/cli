...
func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("less-annoying-crm")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "less-annoying-crm" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "users"}, connectors.ReadRequest{Stream: "users", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
