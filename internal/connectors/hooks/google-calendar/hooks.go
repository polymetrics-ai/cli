// Package googlecalendar bridges the google-calendar quarantine bundle to the legacy connector.
...
import (
	...
	legacy "polymetrics.ai/internal/connectors/google-calendar"
)
...
// New returns a hook set backed by the legacy google-calendar connector.
func New() engine.Hooks { return Hooks{Connector: legacy.New()} }

func (h Hooks) ConnectorName() string { return "google-calendar" }
...
func (h Hooks) ReadStream(ctx context.Context, stream engine.StreamSpec, req connectors.ReadRequest, rt *engine.Runtime, emit func(connectors.Record) error) (bool, error) {
	...
	if req.Stream == "" {
		return true, fmt.Errorf("google-calendar" + " stream name is required")
	}
