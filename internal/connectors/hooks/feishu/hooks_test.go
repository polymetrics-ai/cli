...
import (
	...
	legacy "polymetrics.ai/internal/connectors/feishu"
)

func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("feishu")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "feishu" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "records"}, connectors.ReadRequest{Stream: "records", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
