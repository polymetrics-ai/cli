...
import (
	...
	native "polymetrics.ai/internal/connectors/native/prestashop"
)

func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("prestashop")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "prestashop" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "customers"}, connectors.ReadRequest{Stream: "customers", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
