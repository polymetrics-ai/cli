// Package lokalise bridges the lokalise quarantine bundle to the native connector.
package lokalise
...
import (
	...
	native "polymetrics.ai/internal/connectors/native/lokalise"
)

func init() {
	engine.RegisterHooks("lokalise", func() engine.Hooks { return New() })
}
...
// New returns a hook set backed by the native lokalise connector.
func New() engine.Hooks { return Hooks{Connector: native.New()} }

func (h Hooks) ConnectorName() string { return "lokalise" }
...
func (h Hooks) ReadStream(ctx context.Context, stream engine.StreamSpec, req connectors.ReadRequest, rt *engine.Runtime, emit func(connectors.Record) error) (bool, error) {
	...
	if req.Stream == "" {
		return true, fmt.Errorf("lokalise" + " stream name is required")
	}
