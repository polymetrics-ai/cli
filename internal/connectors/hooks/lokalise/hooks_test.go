...
func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("lokalise")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "lokalise" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "keys"}, connectors.ReadRequest{Stream: "keys", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
