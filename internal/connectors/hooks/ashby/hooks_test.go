...
import (
	...
	legacy "polymetrics.ai/internal/connectors/ashby"
	"polymetrics.ai/internal/connectors/engine"
)

func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("ashby")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "ashby" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "candidates"}, connectors.ReadRequest{Stream: "candidates", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
