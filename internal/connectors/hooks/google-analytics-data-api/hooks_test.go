...
func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("google-analytics-data-api")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "google-analytics-data-api" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "daily_active_users"}, connectors.ReadRequest{Stream: "daily_active_users", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
