// Package googleanalyticsdataapi bridges the google-analytics-data-api quarantine bundle to the legacy connector.
...
func init() {
	engine.RegisterHooks("google-analytics-data-api", func() engine.Hooks { return New() })
}
...
// New returns a hook set backed by the legacy google-analytics-data-api connector.
func New() engine.Hooks { return Hooks{Connector: legacy.New()} }

func (h Hooks) ConnectorName() string { return "google-analytics-data-api" }
...
func (h Hooks) ReadStream(ctx context.Context, stream engine.StreamSpec, req connectors.ReadRequest, rt *engine.Runtime, emit func(connectors.Record) error) (bool, error) {
	...
	if req.Stream == "" {
		return true, fmt.Errorf("google-analytics-data-api" + " stream name is required")
	}
