// Package zohoanalyticsmetadataapi implements the zoho-analytics-metadata-api
// bundle's AuthHook (docs/migration/quarantine.json's
// zoho-analytics-metadata-api AUTH_COMPLEX entry, "OAuth refresh-token
// exchange (hook needed)"): an OAuth 2.0 refresh-token-grant
// connsdk.Authenticator, porting legacy
// internal/connectors/zoho-analytics-metadata-api/zoho_analytics_metadata_api.go's
// refreshToken almost verbatim. Only one hook interface is implemented
// (AuthHook), well under the 2-interface cap and well under the 300-line
// Tier-2 soft target.
//
...
// New returns a fresh zoho-analytics-metadata-api Hooks value as engine.Hooks.
func New() engine.Hooks { return &Hooks{} }

func (h *Hooks) ConnectorName() string { return "zoho-analytics-metadata-api" }

// Authenticator resolves the OAuth2 refresh-token-grant connsdk.Authenticator
// for spec (mode "custom", hook "zoho-analytics-metadata-api"). Templated
// AuthSpec fields are interpolated against cfg here — buildCustomAuth passes
// spec through uninterpolated (engine/auth.go), since interpolation is
// mode-specific engine-side.
//
// spec.Token is interpreted as the refresh token (mirrors gmail's
// documented field mapping): this connector's AuthSpec has no dedicated
...
func interpolateRequired(tmpl, field string, cfg connectors.RuntimeConfig) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("zoho-analytics-metadata-api oauth: resolve %s: %w", field, err)
	}
	if strings.TrimSpace(val) == "" {
		return "", fmt.Errorf("zoho-analytics-metadata-api oauth: %s is required", field)
	}
	return val, nil
}
...
// is intentionally stricter than legacy's validateURL
// (zoho_analytics_metadata_api.go:226-234), which also accepted plain http;
// the tighter rule is documented as a parity deviation in docs.md's Known
// limits (mirrors gmail's identical deviation; never stricter for any real
// Zoho OAuth endpoint, which is always https).
func validateHTTPSURL(raw, field string) error {
	parsed, err := url.Parse(raw)
	if err != nil {
		return fmt.Errorf("zoho-analytics-metadata-api oauth: %s is invalid: %w", field, err)
	}
	if parsed.Scheme != "https" {
		return fmt.Errorf("zoho-analytics-metadata-api oauth: %s must use https, got %q", field, parsed.Scheme)
	}
	if parsed.Host == "" {
		return fmt.Errorf("zoho-analytics-metadata-api oauth: %s must include a host", field)
	}
	return nil
}

// oauthRefreshAuth implements connsdk.Authenticator for the Zoho OAuth 2.0
// refresh-token grant, mirroring legacy
// zoho_analytics_metadata_api.go's refreshToken field-for-field and
// behavior-for-behavior: exchange the refresh token for a short-lived
// access token at tokenURL, cache it until 60s before its declared expiry,
// then set Authorization: Zoho-oauthtoken <token> on each request (Zoho's
// own header scheme, distinct from a plain Bearer prefix). Secret values
// never flow anywhere except the outgoing token-request form or the
// Authorization header; they are never logged.
...
func (a *oauthRefreshAuth) accessToken(ctx context.Context) (string, error) {
	...
	if strings.TrimSpace(a.tokenURL) == "" {
		return "", errors.New("zoho-analytics-metadata-api oauth: token URL is required")
	}
	if strings.TrimSpace(a.refreshToken) == "" {
		return "", errors.New("zoho-analytics-metadata-api oauth: refresh_token is required")
	}
	if strings.TrimSpace(a.clientID) == "" {
		return "", errors.New("zoho-analytics-metadata-api oauth: client_id is required")
	}
	if strings.TrimSpace(a.clientSecret) == "" {
		return "", errors.New("zoho-analytics-metadata-api oauth: client_secret is required")
	}
	...
	if err != nil {
		return "", fmt.Errorf("zoho-analytics-metadata-api oauth: build token request: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("zoho-analytics-metadata-api oauth: token request: %w", err)
	}
	defer func() { _ = resp.Body.Close() }()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return "", fmt.Errorf("zoho-analytics-metadata-api oauth: token endpoint returned %s", resp.Status)
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&out); err != nil {
		return "", fmt.Errorf("zoho-analytics-metadata-api oauth: decode token response: %w", err)
	}
	if strings.TrimSpace(out.AccessToken) == "" {
		return "", errors.New("zoho-analytics-metadata-api oauth: token response missing access_token")
	}
