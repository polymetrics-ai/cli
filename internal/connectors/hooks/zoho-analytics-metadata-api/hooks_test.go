// Package zohoanalyticsmetadataapi implements the
// zoho-analytics-metadata-api AuthHook, ported from legacy
// internal/connectors/zoho-analytics-metadata-api/zoho_analytics_metadata_api.go's
// refreshToken. This test file mirrors internal/connectors/hooks/gmail/
// hooks_test.go's structure (the precedent this hook copies) adapted for
// this connector's own required fields (client_secret and refresh_token
// both mandatory; no scopes/optional client_secret).
...
func baseSpec() engine.AuthSpec {
	return engine.AuthSpec{
		Mode:         "custom",
		Hook:         "zoho-analytics-metadata-api",
...
func TestHooksRegisteredUnderZohoAnalyticsMetadataAPI(t *testing.T) {
	...
	if h.ConnectorName() != "zoho-analytics-metadata-api" {
		t.Fatalf("ConnectorName() = %q, want %q", h.ConnectorName(), "zoho-analytics-metadata-api")
	}
	if _, ok := h.(engine.AuthHook); !ok {
		t.Fatal("registered zoho-analytics-metadata-api hooks does not implement engine.AuthHook")
	}
}
