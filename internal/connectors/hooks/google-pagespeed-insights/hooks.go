// Package googlepagespeedinsights bridges the google-pagespeed-insights quarantine bundle to the native connector.
...
import (
	...
	native "polymetrics.ai/internal/connectors/native/google-pagespeed-insights"
)

func init() {
	engine.RegisterHooks("google-pagespeed-insights", func() engine.Hooks { return New() })
}
...
// New returns a hook set backed by the native google-pagespeed-insights connector.
func New() engine.Hooks { return Hooks{Connector: native.New()} }

func (h Hooks) ConnectorName() string { return "google-pagespeed-insights" }
...
func (h Hooks) ReadStream(ctx context.Context, stream engine.StreamSpec, req connectors.ReadRequest, rt *engine.Runtime, emit func(connectors.Record) error) (bool, error) {
	...
	if req.Stream == "" {
		return true, fmt.Errorf("google-pagespeed-insights" + " stream name is required")
	}
