...
func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("google-pagespeed-insights")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "google-pagespeed-insights" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "pagespeed_reports"}, connectors.ReadRequest{Stream: "pagespeed_reports", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
