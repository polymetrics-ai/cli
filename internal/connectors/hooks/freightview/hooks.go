// Package freightview bridges the freightview quarantine bundle to the native connector.
package freightview
...
import (
	...
	native "polymetrics.ai/internal/connectors/native/freightview"
)

func init() {
	engine.RegisterHooks("freightview", func() engine.Hooks { return New() })
}
...
// New returns a hook set backed by the native freightview connector.
func New() engine.Hooks { return Hooks{Connector: native.New()} }

func (h Hooks) ConnectorName() string { return "freightview" }
...
func (h Hooks) ReadStream(ctx context.Context, stream engine.StreamSpec, req connectors.ReadRequest, rt *engine.Runtime, emit func(connectors.Record) error) (bool, error) {
	...
	if req.Stream == "" {
		return true, fmt.Errorf("freightview" + " stream name is required")
	}
