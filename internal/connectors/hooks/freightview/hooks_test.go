...
func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("freightview")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "freightview" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "shipments"}, connectors.ReadRequest{Stream: "shipments", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
