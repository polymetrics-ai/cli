// Package basecamp bridges the basecamp quarantine bundle to the native connector.
package basecamp
...
import (
	...
	native "polymetrics.ai/internal/connectors/native/basecamp"
)

func init() {
	engine.RegisterHooks("basecamp", func() engine.Hooks { return New() })
}
...
// New returns a hook set backed by the native basecamp connector.
func New() engine.Hooks { return Hooks{Connector: native.New()} }

func (h Hooks) ConnectorName() string { return "basecamp" }
...
func (h Hooks) ReadStream(ctx context.Context, stream engine.StreamSpec, req connectors.ReadRequest, rt *engine.Runtime, emit func(connectors.Record) error) (bool, error) {
	...
	if req.Stream == "" {
		return true, fmt.Errorf("basecamp" + " stream name is required")
	}
