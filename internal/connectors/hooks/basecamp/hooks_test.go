...
func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("basecamp")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "basecamp" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "projects"}, connectors.ReadRequest{Stream: "projects", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
