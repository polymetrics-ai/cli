...
func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("canny")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "canny" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "boards"}, connectors.ReadRequest{Stream: "boards", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
