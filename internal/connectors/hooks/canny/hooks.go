// Package canny bridges the canny quarantine bundle to the native connector.
package canny
...
import (
	...
	native "polymetrics.ai/internal/connectors/native/canny"
)

func init() {
	engine.RegisterHooks("canny", func() engine.Hooks { return New() })
}
...
// New returns a hook set backed by the native canny connector.
func New() engine.Hooks { return Hooks{Connector: native.New()} }

func (h Hooks) ConnectorName() string { return "canny" }
...
func (h Hooks) ReadStream(ctx context.Context, stream engine.StreamSpec, req connectors.ReadRequest, rt *engine.Runtime, emit func(connectors.Record) error) (bool, error) {
	...
	if req.Stream == "" {
		return true, fmt.Errorf("canny" + " stream name is required")
	}
