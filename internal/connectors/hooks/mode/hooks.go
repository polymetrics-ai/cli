// Package mode bridges the mode quarantine bundle to the native connector.
package mode
...
import (
	...
	native "polymetrics.ai/internal/connectors/native/mode"
)

func init() {
	engine.RegisterHooks("mode", func() engine.Hooks { return New() })
}
...
// New returns a hook set backed by the native mode connector.
func New() engine.Hooks { return Hooks{Connector: native.New()} }

func (h Hooks) ConnectorName() string { return "mode" }
...
func (h Hooks) ReadStream(ctx context.Context, stream engine.StreamSpec, req connectors.ReadRequest, rt *engine.Runtime, emit func(connectors.Record) error) (bool, error) {
	...
	if req.Stream == "" {
		return true, fmt.Errorf("mode" + " stream name is required")
	}
