...
func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("mode")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "mode" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "spaces"}, connectors.ReadRequest{Stream: "spaces", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
