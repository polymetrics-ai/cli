...
func TestHooksRegistered(t *testing.T) {
	h := engine.HooksFor("dixa")
	if h == nil {
		t.Fatal("registered hooks = nil")
	}
	if h.ConnectorName() != "dixa" {
...
func TestHooksDelegateFixtureCheckAndRead(t *testing.T) {
	...
	count := 0
	handled, err = h.ReadStream(context.Background(), engine.StreamSpec{Name: "conversations"}, connectors.ReadRequest{Stream: "conversations", Config: cfg}, nil, func(connectors.Record) error {
		count++
		return nil
	})
	if err != nil {
