...
type Requester struct {
	...
	MaxRetries int
	// DisableRetries performs exactly one request attempt. It also suppresses
	// the otherwise automatic one-time 401 auth-refresh attempt, which is a
	// retry even when MaxRetries is zero. The strict rest_write executor sets
	// this explicitly; ordinary reads retain the default policy.
	DisableRetries bool
