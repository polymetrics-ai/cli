// Package workdayrest implements a conservative read-only native Workday REST connector.
package workdayrest
...
func init() { connectors.RegisterFactory("workday-rest", New) }

func New() connectors.Connector { return Connector{} }

type Connector struct{ Client *http.Client }

func (Connector) Name() string { return "workday-rest" }

func (Connector) Metadata() connectors.Metadata {
	return connectors.Metadata{
		Name:            "workday-rest",
		DisplayName:     "Workday REST",
		IntegrationType: "api",
		Description:     "Reads Workday REST API resources with bearer-token authentication. Read-only.",
...
var streamOrder = []string{"workers", "organizations", "jobs"}

var streamEndpoints = map[string]streamEndpoint{
	"workers": {
		resource:     "ccx/api/hcm/v1/{tenant}/workers",
		recordsPath:  "data",
		description:  "Workers from the Workday HCM REST API.",
		fields:       []connectors.Field{{Name: "id", Type: "string"}, {Name: "descriptor", Type: "string"}, {Name: "updated", Type: "string"}},
		cursorFields: []string{"updated"},
	},
	"organizations": {
		resource:     "ccx/api/hcm/v1/{tenant}/organizations",
		recordsPath:  "data",
		description:  "Organizations from the Workday HCM REST API.",
		fields:       []connectors.Field{{Name: "id", Type: "string"}, {Name: "descriptor", Type: "string"}, {Name: "type", Type: "string"}},
		cursorFields: []string{"updated"},
	},
	"jobs": {
		resource:     "ccx/api/hcm/v1/{tenant}/jobs",
		recordsPath:  "data",
		description:  "Job profiles from the Workday HCM REST API.",
		fields:       []connectors.Field{{Name: "id", Type: "string"}, {Name: "descriptor", Type: "string"}, {Name: "updated", Type: "string"}},
		cursorFields: []string{"updated"},
	},
}
...
func (c Connector) Check(ctx context.Context, cfg connectors.RuntimeConfig) error {
	...
	resource, err := resolveResource(streamEndpoints["workers"].resource, cfg)
	if err != nil {
		return err
	}
	if err := r.DoJSON(ctx, http.MethodGet, resource, url.Values{"limit": []string{"1"}}, nil, nil); err != nil {
		return fmt.Errorf("check workday-rest: %w", err)
	}
	return nil
}
...
func (c Connector) Read(ctx context.Context, req connectors.ReadRequest, emit func(connectors.Record) error) error {
	...
	if !ok {
		return fmt.Errorf("workday-rest stream %q not found", stream)
	}
	...
	pageSize, err := boundedInt(req.Config.Config["page_size"], defaultPageSize, maxPageSize, "workday-rest config page_size")
	if err != nil {
		return err
	}
	maxPages, err := readMaxPages(req.Config, "workday-rest")
	if err != nil {
		return err
	}
...
func readFixture(ctx context.Context, connector, stream string, emit func(connectors.Record) error) error {
	for i := 1; i <= 2; i++ {
		...
		rec := connectors.Record{"id": fmt.Sprintf("%s_fixture_%d", stream, i), "descriptor": fmt.Sprintf("Fixture %s %d", stream, i), "updated": "2026-01-01T00:00:00Z", "connector": connector, "stream": stream, "fixture": true}
...
func (c Connector) requester(cfg connectors.RuntimeConfig) (*connsdk.Requester, error) {
	base, err := baseURL(cfg, defaultBaseURL, "workday-rest")
	if err != nil {
		return nil, err
	}
	token := secret(cfg, "access_token")
	if strings.TrimSpace(token) == "" {
		return nil, errors.New("workday-rest connector requires secret access_token")
	}
...
func resolveResource(pattern string, cfg connectors.RuntimeConfig) (string, error) {
	tenant := strings.TrimSpace(cfg.Config["tenant"])
	if tenant == "" || strings.ContainsAny(tenant, "/?#") {
		return "", errors.New("workday-rest config tenant is required and must be a path segment")
	}
	return strings.ReplaceAll(pattern, "{tenant}", url.PathEscape(tenant)), nil
}
