...
Zendesk is an issue #157 metadata inventory bundle for the official Zendesk Support API OAS. It
tracks 617 operations across 429 paths without enabling streams,
...
No Zendesk streams are enabled in issue #157. Later lanes map durable collection reads from the
operation ledger to `streams.json`, schemas, fixtures, pagination, cursors, and primary keys.

## Write actions & risks

No Zendesk write actions are enabled in issue #157. Mutation candidates in `api_surface.json` remain
blocked by default until later lanes add typed reverse-ETL schemas, risk text, approval text,
redaction policy, and `confirm: destructive` where required. Reverse ETL remains plan → preview →
approval → execute.

## Known limits

- This bundle is a metadata scaffold only; it does not claim executable read/write parity yet.
- `api_surface.json` uses blocked-by-default operation rows so the official OAS inventory can be
  reviewed without exposing raw generic HTTP tools.
