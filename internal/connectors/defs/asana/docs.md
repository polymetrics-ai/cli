...
- Provider search/typeahead and CDC/changefeed/audit/webhook routes remain visible only with their actual named request/response foundation in the source ledger; source unavailability and missing runtime foundation are distinct states. The sole binary-upload route is the source-backed bounded Asana attachment action; no binary-download route is claimed.
