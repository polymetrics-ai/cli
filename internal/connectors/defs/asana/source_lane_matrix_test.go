...
func TestAsanaMissingFoundationLedgerSeparatesLegacyAttachmentAliasFromRuntime(t *testing.T) {
	...
	if !ok || events.State != "existing_mapping_backlog" || !strings.Contains(events.Reason, "other 63 source-backed full-refresh collections") || !slices.Contains(events.DoesNotBlock, "full-refresh overwrite and append for all 64 source-backed collection streams") {
