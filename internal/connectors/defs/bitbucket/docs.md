...
Supported JSON/path mutations are named reverse-ETL actions with a record schema for path parameters and a schema-permissive typed payload for official request-body fields. `DELETE` actions use `kind: delete`, `body_type: none`, idempotent `missing_ok_status: [404]`, and `confirm: "destructive"`; they remain in scope under the captain policy correction only through the existing plan -> preview -> explicit approval -> execute path plus typed destructive confirmation. File/multipart operations, including issue imports, issue attachments, repository `/src` commits, and snippet file bodies, are blocked in `api_surface.json`/`operations.json` until a bounded file-transfer foundation can bind file snapshots, byte caps, and approvals without generic raw bytes. The issue-import ZIP operation is also destructive and must require destructive confirmation if it is later implemented.
