...
Implemented connector-local inventory summary: `api_surface.json` has 331 official rows (GET 179, POST 50, PUT 48, DELETE 54); 145 rows are declarative streams, 148 rows are typed reverse-ETL writes, and 38 rows are blocked connector-local operation metadata for binary/direct-read/provider-query/upload foundations. All 54 DELETE writes carry `confirm: "destructive"`.
...
| Runtime help/inspect spot checks | `pm connectors inspect bitbucket --json`, `pm bitbucket --help`, `pm bitbucket repositories delete --help` via `go run ./cmd/pm` | passed; inspect reported 145 streams and 148 write actions, delete help shows destructive confirmation path |
