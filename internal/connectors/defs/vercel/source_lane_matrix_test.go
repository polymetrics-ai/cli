...
func TestVercelSourceLaneMatrixRetainsEveryLockedOperationAndLane(t *testing.T) {
	...
	if err := validateVercelSourceLaneMatrix(matrix, lock, streams); err != nil {
	...
	t.Run("rejects missing lane cell", func(t *testing.T) {
		...
		if err := validateVercelSourceLaneMatrix(broken, lock, streams); err == nil || !strings.Contains(err.Error(), "missing lane cell") {
	...
	t.Run("rejects hidden source row", func(t *testing.T) {
		...
		if err := validateVercelSourceLaneMatrix(broken, lock, streams); err == nil || !strings.Contains(err.Error(), "source rows matrix=") {
	...
	t.Run("rejects legacy ETL backlink drift", func(t *testing.T) {
		...
		if err := validateVercelSourceLaneMatrix(broken, lock, streams); err == nil || !strings.Contains(err.Error(), "legacy stream backlink") {
	...
	t.Run("rejects crosswalk boundary drop", func(t *testing.T) {
		...
		if err := validateVercelSourceLaneMatrix(broken, lock, streams); err == nil || !strings.Contains(err.Error(), "crosswalk boundary identities") {
	...
	t.Run("rejects invalid executable disposition", func(t *testing.T) {
		...
		if err := validateVercelSourceLaneMatrix(broken, lock, streams); err == nil || !strings.Contains(err.Error(), "lane direct_read") {
	...
	t.Run("rejects a crosswalk-only identity that overlaps the retained source lock", func(t *testing.T) {
		broken := cloneVercelObject(t, matrix)
		boundary := mustVercelObject(t, broken["source_boundary_reconciliation"])
		identity := mustVercelObject(t, mustVercelArray(t, boundary["crosswalk_only_source_identities"])[0])
		operation := mustVercelObject(t, mustVercelArray(t, objectAt(lock, "rest")["operations"])[0])
		identity["method"] = stringAt(operation, "method")
		identity["path"] = stringAt(operation, "path")
		entry := mustVercelObject(t, identity["crosswalk_entry"])
		entry["method"] = stringAt(operation, "method")
		entry["path"] = stringAt(operation, "path")
		if err := validateVercelSourceLaneMatrix(broken, lock, streams); err == nil || !strings.Contains(err.Error(), "incorrectly a source row") {
...
func validateVercelSourceLaneMatrix(matrix, lock, streams map[string]any) error {
	...
	if len(lockedMethodPaths) != len(locked) {
		return fmt.Errorf("source lock method/path identities=%d, want %d", len(lockedMethodPaths), len(locked))
	}
	...
	if err := validateVercelCrosswalkBoundary(objectAt(matrix, "source_boundary_reconciliation"), lockedMethodPaths); err != nil {
		return err
	}
...
// The retained matrix carries the complete reviewed API-surface-only sidecar.
// Every source row is independently bound to the immutable source lock; the
// sidecar preserves the 22 non-source rows without importing the historical
// crosswalk artifact into this mapping-only Batch R1 integration.
func validateVercelCrosswalkBoundary(boundary map[string]any, lockedMethodPaths map[string]struct{}) error {
	...
	if len(lockedMethodPaths) != 400 {
		return fmt.Errorf("source lock method/path boundary=%d, want 400", len(lockedMethodPaths))
	}

	identities := mustMapSlice(boundary["crosswalk_only_source_identities"])
	if len(identities) != 22 {
		return fmt.Errorf("crosswalk boundary identities=%d, want 22", len(identities))
	}
	...
	for _, identity := range identities {
		...
		seen[key] = struct{}{}
		entry := objectAt(identity, "crosswalk_entry")
		if key == vercelMethodPathKey("", "") ||
			stringAt(identity, "disposition") != "not_source_row" ||
			strings.TrimSpace(stringAt(identity, "reason")) == "" ||
			stringAt(entry, "method") != stringAt(identity, "method") ||
			stringAt(entry, "path") != stringAt(identity, "path") {
		...
		coveredBy, excluded := optionalVercelObject(entry, "covered_by"), optionalVercelObject(entry, "excluded")
		if (len(coveredBy) == 0) == (len(excluded) == 0) {
			return fmt.Errorf("crosswalk boundary entry %q must have exactly one coverage classification", key)
		}
		if len(coveredBy) != 0 {
			stream, write := strings.TrimSpace(stringAt(coveredBy, "stream")), strings.TrimSpace(stringAt(coveredBy, "write"))
			if len(coveredBy) != 1 || (stream == "") == (write == "") {
				return fmt.Errorf("crosswalk boundary covered entry %q is malformed", key)
			}
			if stream != "" {
				classificationCounts["covered_by.stream"]++
			} else {
				classificationCounts["covered_by.write"]++
			}
			continue
		}
		category, reason := strings.TrimSpace(stringAt(excluded, "category")), strings.TrimSpace(stringAt(excluded, "reason"))
		if len(excluded) != 2 || category == "" || reason == "" {
			return fmt.Errorf("crosswalk boundary excluded entry %q is malformed", key)
		}
		classificationCounts["excluded."+category]++
	}
	if !equalVercelCounts(classificationCounts, map[string]int{
		"covered_by.stream":                2,
		"covered_by.write":                 3,
		"excluded.duplicate_of":            2,
		"excluded.out_of_scope":            9,
		"excluded.requires_elevated_scope": 6,
	}) {
		return fmt.Errorf("crosswalk boundary classification counts=%v", classificationCounts)
	}
	return nil
}
...
func optionalVercelObject(value map[string]any, key string) map[string]any {
