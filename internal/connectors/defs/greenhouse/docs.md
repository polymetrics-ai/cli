...
Current v2 parity is implemented for Greenhouse job post updates, scheduled interview create/update replacements, user edit/disable/enable, and job-opening destroy operations where the official docs moved behavior from deprecated v1 endpoints to v2. `anonymize_candidate` accepts a non-empty array of documented anonymize field names and serializes it with the engine's `join:,` template filter to Greenhouse's comma-delimited `fields` query parameter. Hiring-team writes follow the official HTTP Request entries (`PUT /v1/jobs/{id}/hiring_team`, `POST /v1/jobs/{id}/hiring_team`, and `DELETE /v1/jobs/{id}/hiring_team`) with closed member-list schemas; the same Greenhouse pages show conflicting cURL snippets that omit `/hiring_team`, so this bundle keeps the canonical HTTP Request path. Connector-local validation rejects empty hiring-team member lists during plan, preview, and execute. `destroy_openings` uses a connector-local write hook to fail the operation when Greenhouse returns a non-empty `not_deleted` array.
