...
func TestJiraSourceLaneMatrixRetainsEveryLockedOperationAndLane(t *testing.T) {
	...
	if err := validateJiraSourceLaneMatrix(matrix, lock, streams); err != nil {
	...
	t.Run("rejects missing lane cell", func(t *testing.T) {
		...
		if err := validateJiraSourceLaneMatrix(broken, lock, streams); err == nil || !strings.Contains(err.Error(), "missing lane cell") {
	...
	t.Run("rejects hidden source row", func(t *testing.T) {
		...
		if err := validateJiraSourceLaneMatrix(broken, lock, streams); err == nil || !strings.Contains(err.Error(), "source rows matrix=") {
	...
	t.Run("rejects legacy ETL backlink drift", func(t *testing.T) {
		...
		if err := validateJiraSourceLaneMatrix(broken, lock, streams); err == nil || !strings.Contains(err.Error(), "legacy stream backlink") {
	...
	t.Run("rejects invalid executable disposition", func(t *testing.T) {
		...
		if err := validateJiraSourceLaneMatrix(broken, lock, streams); err == nil || !strings.Contains(err.Error(), "lane direct_read") {
...
func TestJiraETLRequiresDocumentedContinuationRatherThanMaxResults(t *testing.T) {
	...
	t.Run("rejects legacy criterion and maxResults field substitution", func(t *testing.T) {
		...
		if err := validateJiraSourceLaneMatrix(broken, lock, streams); err == nil || !strings.Contains(err.Error(), "documented continuation ETL source mapping drift") {
...
func validateJiraSourceLaneMatrix(matrix jiraSourceLaneMatrix, lock jiraSourceLaneLock, streams jiraStreamsDefinition) error {
	...
	if err := validateJiraCrosswalkBoundary(matrix.SourceBoundaryReconciliation, lockedMethodPaths); err != nil {
		return err
	}
...
// Jira's reviewed boundary is an exact method/path reconciliation: all 617
// retained source operations are rows and no API-surface-only entries exist.
// The source-lane matrix intentionally retains that result as evidence instead
// of importing the historical crosswalk artifact into this Batch R1 change.
func validateJiraCrosswalkBoundary(boundary jiraSourceBoundaryReconciliation, lockedMethodPaths map[string]struct{}) error {
	...
	if len(lockedMethodPaths) != 617 {
		return fmt.Errorf("source lock method/path reconciliation=%d, want 617", len(lockedMethodPaths))
	}
	return nil
}
