...
reached if `identifier` itself is absent, which the real API never does) and every other stream
(`categories`/`families`/`attributes`/`channels`/`product_models`/`family_variants`/
`attribute_groups`/`association_types`/`locales`/`currencies`/`measure_families`) uses
`{{ record.code }}` (their real natural key; legacy's identical fallback chain never advances past
`code` for the original 4 resources in practice, and every Pass B addition's own documented API
shape uses `code` as its natural key too). See Known limits for the documented,
never-diverging-for-real-input simplification the original `id`-derivation choice represents.
