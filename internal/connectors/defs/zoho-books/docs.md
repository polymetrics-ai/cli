...
## Streams notes
Reads use Zoho's page-number pagination with `page` and `per_page`, matching the legacy 200-record page size. `organization_id` remains optional for read requests to preserve legacy behavior, and path or required-query parameters for detail/sub-resource streams are declared as config fields for conformance and targeted reads. Streams use passthrough projection so Zoho's raw accounting fields are preserved; `id`, `name`, and `updated_at` are synthesized from documented response fields when available.
