...
Bounded direct-read commands cover the executable official GET detail/query endpoints and all 13 POST read-query endpoints with the `json_redacted` output policy. `GET /v2/crm/entities` is explicitly blocked because Gong requires the `objectsCrmIds` OpenAPI FORM/explode array query parameter and the shared direct-read query runner currently serializes one string per key. POST reads execute through typed operation metadata with connector-authored flags and schema-gated JSON bodies; no raw body flag exists. Call transcripts are available through `pm gong calls transcript` with call ID, time range, workspace, and cursor filters and a 16 MiB response cap. JSON mutations, typed multipart uploads, target assignment CSV uploads, and top-level array CRM schema uploads are modeled as reverse-ETL write actions where the engine supports the request shape.
...
The original incremental streams remain `users`, `calls`, and `scorecards`; additional list streams are full-refresh stream runners over public Gong list endpoints. `pm gong calls list` requires `--from` and `--to` ISO-8601/RFC3339 bounds mapped to Gong `fromDateTime` and `toDateTime`; the upper bound is exclusive. Gong list endpoints with official required query fields enforce typed flags: `permissions profiles list --workspaceId`, `flows list --flowOwnerEmail`, `flows folders list --flowFolderOwnerEmail`, and `targets list --workspaceId`. Gong GET direct reads with official required query fields also enforce typed flags before HTTP dispatch, including permission profile IDs, CRM entity-schema selectors, log type/from time, entity brief/ask selectors, privacy identifiers, CRM request status IDs, and coaching workspace/manager/time bounds. `pm gong crm entities get` remains a blocked command until the shared runner supports repeated query values for the required `objectsCrmIds` array. Commands such as `pm gong workspaces list --json` use the same credential and bounded PM output `--limit` behavior as other connector stream commands. Global `--limit` caps emitted PM records only; it does not claim to control Gong page size or total provider-side results.
...
- Executable/dispositioned coverage after the 2026-08-01 checkpoint: 12 stream endpoints, 29 executable bounded direct reads (16 GET plus all 13 typed POST read-query commands), 27 typed reverse-ETL write actions, and 1 blocked direct-read operation for `GET /v2/crm/entities`; `api_surface.json` has 68 executable covered rows, 1 blocked operation row, and 0 excluded rows.
- The checkpoint found 0 missing/stale official operation rows, 0 required write parameter/schema gaps, and 0 required executable direct-read flag gaps after fixing connector-local metadata. Certification remains 0 because this work used fixture/local validation only and no live Gong credentials or provider calls.
