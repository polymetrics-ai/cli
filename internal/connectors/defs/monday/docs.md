...
The Monday connector reads monday.com GraphQL data and exposes connector-owned metadata for the official monday.com GraphQL reference. Existing ETL streams remain `boards`, `items`, `users`, `teams`, and `tags`. Additional documented query operations are represented as fixed, bounded planned GraphQL operations until shared direct-read error-envelope handling and duplicate POST `/` endpoint classifier validation can make them safely executable.
...
Additional docs-sourced query operations are modeled in `operations.json` and `cli_surface.json` as planned fixed GraphQL documents. Scalar query commands are not advertised as executable in this branch because shared endpoint coverage currently collapses duplicate Monday `POST /` rows by method and path, and the existing operation direct-read path does not fail closed on GraphQL `errors[]` envelopes. Operations with array/object GraphQL arguments also stay planned/blocked until a connector-local or shared typed variable contract can pass those inputs without becoming a raw GraphQL escape hatch.
...
- Fixed query operations remain capped/redacted `graphql_query` ledger entries, but their CLI commands are planned until shared duplicate GraphQL endpoint classifier matching and GraphQL `errors[]` failure semantics land.
