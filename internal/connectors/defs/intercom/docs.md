...
- The generated write schemas use the engine's supported draft-07 subset (`type`, `properties`, `required`, `items`, `enum`, `format`, `description`, `minProperties`, `additionalProperties`, `oneOf`, `anyOf`, `allOf`) and encode official `oneOf`/`anyOf` variant-required fields for destructive redaction, conversation/ticket replies, conversation management, data attributes, contacts, events, messages, and visitors.
