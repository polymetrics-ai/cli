// Package workday implements a conservative read-only native Workday connector.
package workday
...
func init() { connectors.RegisterFactory("workday", New) }

func New() connectors.Connector { return Connector{} }

type Connector struct{ Client *http.Client }

func (Connector) Name() string { return "workday" }
...
func (Connector) Metadata() connectors.Metadata {
	return connectors.Metadata{
		...
		DisplayName:     "Workday",
		IntegrationType: "api",
		Description:     "Reads Workday tenant data through conservative Workday API endpoints. Read-only.",
...
var streamOrder = []string{"workers", "organizations", "positions"}

var streamEndpoints = map[string]streamEndpoint{
	"workers": {
		resource:     "ccx/api/v1/{tenant}/workers",
		recordsPath:  "data",
		description:  "Workday workers exposed by the tenant API.",
		fields:       []connectors.Field{{Name: "id", Type: "string"}, {Name: "name", Type: "string"}, {Name: "updated_at", Type: "string"}},
		cursorFields: []string{"updated_at"},
	},
	"organizations": {
		resource:     "ccx/api/v1/{tenant}/organizations",
		recordsPath:  "data",
		description:  "Workday organizations exposed by the tenant API.",
		fields:       []connectors.Field{{Name: "id", Type: "string"}, {Name: "name", Type: "string"}, {Name: "type", Type: "string"}, {Name: "updated_at", Type: "string"}},
		cursorFields: []string{"updated_at"},
	},
	"positions": {
		resource:     "ccx/api/v1/{tenant}/positions",
		recordsPath:  "data",
		description:  "Workday positions exposed by the tenant API.",
		fields:       []connectors.Field{{Name: "id", Type: "string"}, {Name: "title", Type: "string"}, {Name: "worker_id", Type: "string"}, {Name: "updated_at", Type: "string"}},
		cursorFields: []string{"updated_at"},
	},
}
...
func (c Connector) Check(ctx context.Context, cfg connectors.RuntimeConfig) error {
	...
	if err := r.DoJSON(ctx, http.MethodGet, resource, url.Values{"limit": []string{"1"}}, nil, nil); err != nil {
		return fmt.Errorf("check workday: %w", err)
	}
	return nil
}
...
func (c Connector) Read(ctx context.Context, req connectors.ReadRequest, emit func(connectors.Record) error) error {
	...
	if !ok {
		return fmt.Errorf("workday stream %q not found", stream)
	}
	...
	pageSize, err := boundedInt(req.Config.Config["page_size"], defaultPageSize, maxPageSize, "workday config page_size")
	if err != nil {
		return err
	}
	maxPages, err := readMaxPages(req.Config, "workday")
	if err != nil {
		return err
	}
...
func (c Connector) requester(cfg connectors.RuntimeConfig) (*connsdk.Requester, error) {
	base, err := baseURL(cfg, defaultBaseURL, "workday")
	if err != nil {
		return nil, err
	}
	...
	if strings.TrimSpace(username) == "" || strings.TrimSpace(password) == "" {
		return nil, errors.New("workday connector requires secrets username and password")
	}
...
func resolveResource(pattern string, cfg connectors.RuntimeConfig) (string, error) {
	...
	if tenant == "" || strings.ContainsAny(tenant, "/?#") {
		return "", errors.New("workday config tenant is required and must be a path segment")
	}
