...
import (
	...
	"polymetrics.ai/internal/connectors/todoist"
)

func TestReadTasksAuthenticatesAndMapsRootArray(t *testing.T) {
	var sawAuth string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`[{"id":"1","content":"Write tests","project_id":"p1"},{"id":"2","content":"Ship","project_id":"p1"}]`))
	}))
	defer srv.Close()

	c := todoist.New()
	cfg := connectors.RuntimeConfig{Config: map[string]string{"base_url": srv.URL}, Secrets: map[string]string{"token": "todo-token"}}
	...
	if sawAuth != "Bearer todo-token" {
		t.Fatalf("Authorization = %q", sawAuth)
	}
	if len(got) != 2 || got[0]["content"] == nil {
