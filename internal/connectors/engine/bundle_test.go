...
func TestBundleLoadEmbeddedJiraCLISurface(t *testing.T) {
	b, err := Load(defs.FS, "jira")
	if err != nil {
		t.Fatalf("Load(defs.FS, jira): %v", err)
	}
	if b.CLISurface == nil {
		t.Fatalf("Jira CLISurface is nil; defs.FS must embed cli_surface.json")
	}
	if b.CLISurface.Usage != "pm jira <command> [flags]" {
		t.Fatalf("Jira CLISurface usage = %q", b.CLISurface.Usage)
	}
	if len(b.CLISurface.Commands) == 0 {
		t.Fatalf("Jira CLISurface has no commands")
	}
}
