// Package zonkafeedback implements a read-only native Zonka Feedback API connector.
...
const (
	connectorName  = "zonka-feedback"
	defaultBaseURL = "https://api.zonkafeedback.com"
...
func (Connector) Metadata() connectors.Metadata {
	return connectors.Metadata{Name: connectorName, DisplayName: "Zonka Feedback", IntegrationType: "api", Description: "Reads Zonka Feedback responses, surveys, and contacts through the Zonka Feedback REST API.", Capabilities: connectors.Capabilities{Check: true, Catalog: true, Read: true, Write: false}}
}
...
var streamOrder = []string{"responses", "surveys", "contacts"}

var streams = map[string]streamEndpoint{
	"responses": {path: "responses", recordsPath: "responses", desc: "Zonka Feedback survey responses.", idKeys: []string{"id", "response_id"}, nameKeys: []string{"respondent_name", "email", "name"}, cursorKeys: []string{"updated_at", "modified_at", "created_at"}},
	"surveys":   {path: "surveys", recordsPath: "surveys", desc: "Zonka Feedback surveys.", idKeys: []string{"id", "survey_id"}, nameKeys: []string{"name", "title"}, cursorKeys: []string{"updated_at", "modified_at", "created_at"}},
	"contacts":  {path: "contacts", recordsPath: "contacts", desc: "Zonka Feedback contacts.", idKeys: []string{"id", "contact_id"}, nameKeys: []string{"name", "email"}, cursorKeys: []string{"updated_at", "modified_at", "created_at"}},
}
...
func readFixture(ctx context.Context, stream string, ep streamEndpoint, req connectors.ReadRequest, emit func(connectors.Record) error) error {
	for i := 1; i <= 2; i++ {
		...
		rec := mapRecord(ep, map[string]any{"id": fmt.Sprintf("%s_fixture_%d", stream, i), "name": fmt.Sprintf("%s fixture %d", strings.TrimSuffix(stream, "s"), i), "rating": 10 - i, "updated_at": fixtureTime})
...
func (c Connector) requester(cfg connectors.RuntimeConfig) (*connsdk.Requester, error) {
	...
	token := secret(cfg, "auth_token")
	if token == "" {
		token = secret(cfg, "access_token")
	}
	if token == "" {
		return nil, errors.New("zonka-feedback connector requires secret auth_token")
	}
...
func catalogFields() []connectors.Field {
	return []connectors.Field{{Name: "id", Type: "string"}, {Name: "name", Type: "string"}, {Name: "rating", Type: "number"}, {Name: "updated_at", Type: "timestamp"}}
}

func pageQuery(page, size int) url.Values {
	return url.Values{"page": []string{strconv.Itoa(page)}, "per_page": []string{strconv.Itoa(size)}}
}
