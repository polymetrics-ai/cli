...
import (
	...
	"polymetrics.ai/internal/connectors/workramp"
)
...
func TestFixtureCatalogRegistrationAndWrite(t *testing.T) {
	...
	if cat.Connector != "workramp" || len(cat.Streams) < 3 {
		t.Fatalf("catalog = %+v, want workramp streams", cat)
	}
	var rows []connectors.Record
	err = c.Read(context.Background(), connectors.ReadRequest{Stream: "users", Config: fixture}, func(rec connectors.Record) error {
	...
	if _, ok := connectors.NewRegistry().Get("workramp"); !ok {
		t.Fatal("registry did not resolve workramp")
	}
...
func TestReadLiveUsesBearerAuthAndMapsData(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"data":[{"id":"user-1","email":"user@example.test","updated_at":"2026-01-01T00:00:00Z"}]}`))
	}))
	defer srv.Close()
	...
	if len(got) != 1 || got[0]["id"] != "user-1" {
		t.Fatalf("records = %+v, want user-1", got)
	}
}
