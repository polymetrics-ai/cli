...
import (
	...
	"polymetrics.ai/internal/connectors/tinyemail"
)

func TestReadSubscribersPaginatesAndAuthenticates(t *testing.T) {
	var sawKey string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "1":
			_, _ = w.Write([]byte(`{"data":[{"id":"s1","email":"a@example.com"},{"id":"s2","email":"b@example.com"}]}`))
		case "2":
			_, _ = w.Write([]byte(`{"data":[{"id":"s3","email":"c@example.com"}]}`))
		default:
	...
	c := tinyemail.New()
	cfg := connectors.RuntimeConfig{Config: map[string]string{"base_url": srv.URL, "page_size": "2"}, Secrets: map[string]string{"api_key": "tiny-key"}}
	var got []connectors.Record
	if err := c.Read(context.Background(), connectors.ReadRequest{Stream: "subscribers", Config: cfg}, func(rec connectors.Record) error {
		got = append(got, rec)
		return nil
	}); err != nil {
		t.Fatalf("Read: %v", err)
