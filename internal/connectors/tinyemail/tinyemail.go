// Package tinyemail implements a read-only native Go connector for tinyEmail.
package tinyemail
...
const (
	connectorName   = "tinyemail"
	defaultBaseURL  = "https://api.tinyemail.com/v1"
	defaultPageSize = 100
...
func (Connector) Metadata() connectors.Metadata {
	return connectors.Metadata{Name: connectorName, DisplayName: "TinyEmail", IntegrationType: "api", Description: "Reads subscribers, lists, and campaigns from the tinyEmail API.", Capabilities: connectors.Capabilities{Check: true, Catalog: true, Read: true, Write: false}}
}
...
func (c Connector) Check(ctx context.Context, cfg connectors.RuntimeConfig) error {
	...
	if err := r.DoJSON(ctx, http.MethodGet, "subscribers", url.Values{"page": {"1"}, "limit": {"1"}}, nil, nil); err != nil {
...
func (c Connector) requester(cfg connectors.RuntimeConfig) (*connsdk.Requester, error) {
	...
	if key == "" {
		return nil, errors.New("tinyemail connector requires secret api_key")
	}
...
var streamSpecs = map[string]streamSpec{
	"subscribers": {"subscribers", "tinyEmail subscribers.", "subscribers", "data", fields("id", "email", "first_name", "last_name", "status", "created_at")},
	"lists":       {"lists", "tinyEmail lists.", "lists", "data", fields("id", "name", "subscriber_count", "created_at")},
	"campaigns":   {"campaigns", "tinyEmail campaigns.", "campaigns", "data", fields("id", "name", "subject", "status", "created_at")},
}

func streams() []connectors.Stream {
	order := []string{"subscribers", "lists", "campaigns"}
