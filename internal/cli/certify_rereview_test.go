...
func TestSeventhCertifyValueRequiredFlagsRejectBeforeLoggerTelemetry(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			assertCertifyRequiredValueHasNoProjectEffects(t, tc.args)
		})
	}
}

func TestEighthCertifyWhitespaceValueRequiredFlagsRejectBeforeLoggerTelemetry(t *testing.T) {
	credsPath := filepath.Join(t.TempDir(), "creds.yaml")
	if err := os.WriteFile(credsPath, []byte("version: 1\nconnectors:\n  sample: {}\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	for _, value := range []string{"", "   "} {
		...
		for _, tc := range cases {
			t.Run(label+" "+tc.name, func(t *testing.T) {
				assertCertifyRequiredValueHasNoProjectEffects(t, append(append([]string{}, tc.args...), value))
			})
		}
	}
}

func assertCertifyRequiredValueHasNoProjectEffects(t *testing.T, commandArgs []string) {
	t.Helper()
	root := t.TempDir()
	t.Setenv("PM_TELEMETRY", "file")
	args := append(append([]string{}, commandArgs...), "--root", root, "--json")
	var stdout, stderr bytes.Buffer
	if code := Run(args, &stdout, &stderr); code != 2 {
		t.Fatalf("required-value flag exit=%d, want usage 2; stdout=%q stderr=%q", code, stdout.String(), stderr.String())
	}
	if !strings.Contains(stdout.String()+stderr.String(), "requires a value") {
		t.Fatalf("required-value error missing message; stdout=%q stderr=%q", stdout.String(), stderr.String())
	}
	if _, err := os.Stat(filepath.Join(root, ".polymetrics")); !os.IsNotExist(err) {
		t.Fatalf("required-value flag created project effects under .polymetrics: %v", err)
	}
}
