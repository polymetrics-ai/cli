	...
	docs := []skillDoc{
		...
		},
		{
			...
			Body: skillBody("pm-etl", "Run bounded ETL syncs from configured connections.", []string{
				...
				"Incremental modes and deduped compatibility names require a cursor. Deduped modes require a primary key; the deduped compatibility names refuse before source I/O until a matching transport is admitted.",
