...
**Purpose:** Runtime-neutral prompt for upstream GSD Core brownfield onboarding of connector parity planning.
...
Use this prompt with any agent runtime, including Pi. The agent must follow the installed upstream GSD Core workflow source files listed by `scripts/gsd sources issue-122-rebootstrap` and the repo-local safety/context files listed below.
...
## Required upstream GSD workflow sources

Use the repo command to list and validate installed workflow sources:
